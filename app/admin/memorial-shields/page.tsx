...
import { Search, CheckCircle2, Shield, Loader2, UserCircle, GraduationCap, Crown, Sparkles, Send, Award, Medal, UploadCloud, ImagePlus, X } from 'lucide-react';
...
export default function MemorialShieldsMaker() {
  ...
  // 🚀 وضعية صناعة الدرع (بناء ذكي 3D أو رفع خارجي)
  ...
  // 🚀 دالة الرفع السحابية (Cloudinary)
  ...
  const handleSaveShield = async () => {
    ...
    try {
       const payload = {
          ...
          title: title.trim() || 'درع خارجي',
          message: message.trim() || 'تهانينا',
          custom_logo_url: creationMode === 'build' ? customLogoUrl : null,
          external_shield_url: creationMode === 'upload' ? externalShieldUrl : null
       };

       if (activeTab === 'students') {
          await supabase.from('student_memorials').insert({ student_id: selectedUser.id, ...payload });
       } else {
          await supabase.from('teacher_memorials').insert({ teacher_id: selectedUser.id, ...payload });
       }
       alert('تم إرسال الدرع التذكاري بنجاح! سيظهر للشخص كاحتفال مفاجئ فور دخوله لحسابه. 🎇');
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
           ...
           <div className="lg:col-span-6 xl:col-span-7 space-y-6 flex flex-col">
              ...
              {/* 🚀 خيارات الصناعة (بناء 3D أو رفع خارجي) */}
