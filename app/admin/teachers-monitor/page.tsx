...
import {
  CheckCircle2, AlertTriangle, Users, Calendar, Clock, Search, Send, ShieldAlert, BarChart2, RefreshCw, Zap, Info, School, Folder, Crown
} from "lucide-react";
...
const MONTH_MAP: Record<number, string> = { 0: "يناير", 1: "فبراير", 2: "مارس", 3: "أبريل", 4: "مايو", 5: "يونيو", 6: "يوليو", 7: "أغسطس", 8: "سبتمبر", 9: "أكتوبر", 10: "نوفمبر", 11: "ديسمبر" };
...
export default function TeachersMonitorPage() {
  ...
  useEffect(() => {
    ...
    setTodayStr(getLocalDateString(st));
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const results: TeacherMonitor[] = allTeachers.map((teacher: any) => {
        ...
        let expectedTotal = 0; let actualRecorded = 0; let actualMissed = 0;
        ...
        daySchedules.forEach(sch => {
          ...
          if (isPassed && isSystemActive) {
            expectedTotal++;
            const hasRecord = safeAttendance.find((a: any) => String(a.section_id) === String(sch.section_id) && String(a.period) === String(sch.period));
            if (hasRecord) {
              actualRecorded++;
        ...
        return {
          id: teacher.id, name: teacherName,
          specialization: teacher.specialization || "عام",
          department: getParentDepartment(teacher.specialization),
          isHOD: teacher.department_heads?.length > 0,
          stage: getTeacherStage(teacher),
          recorded: actualRecorded, missed: actualMissed, expected: expectedTotal, scheduled: scheduledTotal, percent, lastRecorded, status, assignmentsCount, examsCount,
        };
      });
  ...
  useEffect(() => { if (todayStr && allTeachers.length > 0) fetchData(); }, [fetchData, todayStr, allTeachers]);
  ...
  const sendWarning = async (teacherId: string) => {
    ...
    } catch (e) { alert("حدث خطأ أثناء إرسال التنبيه"); } 
    finally { setSendingWarning(null); }
  };
  ...
  const groupedTeachers = useMemo(() => {
    const groups = filteredAndStagedTeachers.reduce((acc, t) => {
      ...
      return acc;
    }, {} as Record<string, TeacherMonitor[]>);

    const sortedGroups: Record<string, TeacherMonitor[]> = {};
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 sm:space-y-8 pb-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 font-cairo" dir="rtl">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6 sm:gap-8">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 shrink-0 w-full lg:w-auto">
            <Link href="/admin/teachers-report" className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 sm:px-8 py-3.5 sm:py-4 rounded-[1.5rem] bg-white text-indigo-600 hover:bg-indigo-50 text-sm sm:text-base font-black shadow-lg shadow-indigo-500/30 transition-all active:scale-95">
              <BarChart2 className="w-4 h-4 sm:w-5 sm:h-5" /> التقارير والمطبوعات
            </Link>
      ...
      {isWeekend && !isDataLoading && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-sky-50 border border-sky-200 p-4 rounded-2xl flex items-center gap-3 text-sky-800 shadow-sm">
          ...
          <p className="text-sm font-bold">اليوم عطلة رسمية، الإحصائيات الحالية تظهر صفرياً. للانتقال للأيام السابقة استخدم التقارير الشاملة.</p>
        </motion.div>
      )}

      {/* 🚀 أدوات الفلترة والبحث للمراقبة */}
      <div className="bg-white/90 backdrop-blur-xl p-5 sm:p-6 rounded-[2rem] shadow-sm border border-slate-200 flex flex-col xl:flex-row gap-6 justify-between items-start xl:items-center">
        ...
        <div className="relative w-full xl:w-72 shrink-0">
          ...
          <input type="text" placeholder="بحث بالاسم أو التخصص..." value={search} onChange={e => setSearch(e.target.value)} className="w-full rounded-[1.5rem] bg-slate-50 border border-slate-200 py-3 pr-12 pl-4 text-sm font-bold text-slate-900 focus:ring-2 focus:ring-indigo-500 outline-none shadow-sm transition-all" />
        </div>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
        {[
          { label: "انضباط ممتاز", count: filteredAndStagedTeachers.filter(t => t.status === "ممتاز").length, color: "emerald", icon: CheckCircle2 },
          { label: "أداء جيد", count: filteredAndStagedTeachers.filter(t => t.status === "جيد").length, color: "blue", icon: Users },
          { label: "يستدعي التحذير", count: filteredAndStagedTeachers.filter(t => t.status === "تحذير").length, color: "amber", icon: AlertTriangle },
          { label: "وضع حرج (تأخير)", count: filteredAndStagedTeachers.filter(t => t.status === "حرج").length, color: "rose", icon: ShieldAlert },
        ].map(item => (
      ...
      <div className="bg-white rounded-[2rem] sm:rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-5 sm:p-8 border-b border-slate-100 bg-slate-50/50 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 sm:gap-6">
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div>
              <h3 className="text-lg sm:text-xl lg:text-2xl font-black text-slate-900 tracking-tight">رادار الرصد الحي للأقسام</h3>
              <p className="text-[10px] sm:text-xs lg:text-sm text-slate-500 font-bold mt-1">يتغير لون المعلم تلقائياً إذا تأخر عن موعد الحصة.</p>
            </div>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-100">
            <thead className="bg-slate-50/30">
              <tr>
                <th className="py-4 sm:py-5 pr-6 sm:pr-8 pl-4 text-right text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest">المعلم / المنصب</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {isDataLoading ? (
                <tr><td colSpan={6} className="py-20 text-center"><div className="h-10 w-10 sm:h-12 sm:w-12 border-4 border-indigo-600/20 border-t-indigo-600 rounded-full animate-spin mx-auto" /></td></tr>
              ) : Object.keys(groupedTeachers).length === 0 ? (
                <tr><td colSpan={6} className="py-20 text-center"><p className="text-slate-400 font-bold text-sm sm:text-lg">لا توجد نتائج مطابقة</p></td></tr>
              ) : (
                Object.entries(groupedTeachers).map(([dept, teachers]) => (
                  <React.Fragment key={`monitor-${dept}`}>
                    {/* 🚀 عنوان القسم كفاصل */}
                    <tr className="bg-slate-100/50 border-y border-slate-200">
                      <td colSpan={6} className="py-3 px-6">
                        <div className="flex items-center gap-2 text-indigo-900 font-black">
                          <Folder className="w-5 h-5 text-indigo-500" /> قسم {dept} 
                          <span className="text-xs bg-indigo-100 text-indigo-600 px-2 py-0.5 rounded-md mr-2">{teachers.length} معلمين</span>
                        </div>
                      </td>
                    </tr>
                    {/* 🚀 المعلمين */}
                    {teachers.map((teacher, idx) => {
                      ...
                      return (
                        <motion.tr key={teacher.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.02 }} className={`group transition-all hover:bg-slate-50/50 ${teacher.status === "حرج" ? "bg-rose-50/10" : ""} ${teacher.isHOD ? "bg-amber-50/5" : ""}`}>
                          <td className="whitespace-nowrap py-3 sm:py-4 pr-6 sm:pr-8 pl-4">
                            <div className="flex items-center gap-3 sm:gap-4">
                              <div className={`h-10 w-10 sm:h-12 sm:w-12 rounded-xl sm:rounded-2xl border flex items-center justify-center font-black text-base sm:text-lg shadow-sm shrink-0 ${teacher.isHOD ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-indigo-50 text-indigo-600 border-indigo-100'}`}>
                              ...
                              <div className="flex flex-col min-w-0">
                                ...
                                <div className="text-[9px] sm:text-[10px] text-slate-400 font-bold truncate mt-0.5">
                                  {teacher.isHOD ? <span className="text-amber-600">رئيس القسم</span> : teacher.specialization} <span className="mx-0.5 text-slate-300">|</span> {teacher.stage === 'middle' ? 'متوسط' : teacher.stage === 'high' ? 'ثانوي' : teacher.stage === 'both' ? 'مشترك' : ''}
                                </div>
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="px-4 py-3 sm:py-4 text-center">
                            <span className={`inline-flex items-center px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-[9px] sm:text-[10px] font-black border shadow-sm ${statusColor(teacher.status)}`}>{teacher.status} {teacher.expected > 0 ? `(${teacher.percent}%)` : ''}</span>
                            {teacher.lastRecorded && <div className="text-[8px] sm:text-[9px] font-bold text-slate-400 mt-1 flex items-center justify-center gap-1"><Clock className="w-2.5 h-2.5" /> {new Date(teacher.lastRecorded).toLocaleTimeString("ar-EG", { hour: '2-digit', minute: '2-digit' })}</div>}
                          </td>
                          <td className="px-6 py-3 sm:py-4 text-center">
                            <button onClick={() => sendWarning(teacher.id)} disabled={sendingWarning === teacher.id || !hasAlert || teacher.expected === 0} className={`inline-flex items-center justify-center gap-1.5 w-full sm:w-auto px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl text-[10px] sm:text-xs font-black transition-all active:scale-95 ${!hasAlert || teacher.expected === 0 ? "bg-slate-50 text-slate-300 border border-slate-100 cursor-not-allowed" : "bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white border border-rose-200 shadow-sm"}`}>
