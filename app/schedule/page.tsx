                  ...
                  {slot ? (
                    ...
                    >
                      ...
                      {slot.teachers?.zoom_link && (
                        ...
                        >
                          🎥 {slot.teachers.zoom_link.length > 28
                            ? slot.teachers.zoom_link.slice(0, 28) + '...'
                            : slot.teachers.zoom_link}
                        </a>
                      )}
                    </div>
...
// ─── طباعة جماعية ───
const handleBulkPrint = async (type: ‘all-teachers’ | ‘all-sections’) => {
setIsPrintingBulk(true);
// نضمن تحميل كامل البيانات أولاً
if (!showAllSchedules) {
...
```
  ...
  {/* ─── واجهة الموقع (تُخفى عند الطباعة) ─── */}
  <div className="web-content space-y-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
    ...
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
      ...
      <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
        ...
        {/* زرا الطباعة الجماعية — للمدير فقط */}
        {isAdmin && authRole !== 'teacher' && (
          <>
            <button
              onClick={() => handleBulkPrint('all-teachers')}
            ...
            >
              ...
              {isPrintingBulk ? 'جاري التحضير...' : 'طباعة كل المعلمين'}
            </button>
            <button
              onClick={() => handleBulkPrint('all-sections')}
              disabled={isPrintingBulk}
            ...
            >
              ...
              {isPrintingBulk ? 'جاري التحضير...' : 'طباعة كل الفصول'}
            </button>
          </>
        )}
      </div>
  ...
  {periods.length > 0 && (
    <div id="print-area" style={{ display: 'none' }} dir="rtl">
      ...
      {/* ── وضع الطباعة الجماعية: كل الفصول ── */}
      {printMode === 'all-sections' && sections.map((section, sIdx) => {
        ...
        const sectionSchedule = scheduleData.filter(
      ...
      {/* ── وضع الطباعة الفردية (الجدول الحالي) ── */}
        ...
        <PrintScheduleBlock
          label={
            viewType === 'teacher'
              ? teachers.find(t => String(t.id) === String(selectedId))?.users?.full_name || ''
              : `${
                  (Array.isArray(sections.find(s => String(s.id) === String(selectedId))?.classes)
                    ? sections.find(s => String(s.id) === String(selectedId))?.classes[0]
                    : sections.find(s => String(s.id) === String(selectedId))?.classes
                  )?.name
                } - ${sections.find(s => String(s.id) === String(selectedId))?.name}`
          }
