...
export default function SchedulePage() {
  ...
  const executePDF = async (mode: 'single' | 'all-teachers' | 'all-sections') => {
    try {
      ...
      if (!containers || containers.length === 0) {
        throw new Error('لم يتم العثور على جداول صالحة للطباعة.');
      }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20" dir="rtl">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" dir="rtl">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-[2rem] p-8 w-full max-w-lg shadow-2xl border border-slate-100">
              ...
              <div className="space-y-5">
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-2">{viewType === 'teacher' ? 'اختيار الفصل' : 'اختيار المعلم'}</label>
                  {viewType === 'teacher' ? (
                                ...
                                {isAdmin && slot && (
                                  <div className="absolute inset-0 bg-white/90 backdrop-blur-sm flex flex-col items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl">
                                    <div className="flex gap-1.5">
                                      <button className="text-[10px] font-black px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-600 hover:bg-emerald-500 hover:text-white transition-colors shadow-sm" onClick={(e) => { e.stopPropagation(); setCopiedLesson(displaySlot); }}>نسخ</button>
                                      <button className="text-[10px] font-black px-3 py-1.5 rounded-lg bg-amber-50 text-amber-600 hover:bg-amber-500 hover:text-white transition-colors shadow-sm" onClick={(e) => { e.stopPropagation(); setSwappingFrom(displaySlot); }}>نقل</button>
                                    </div>
                                    <div className="flex gap-1.5">
                                      <button className="text-[10px] font-black px-3 py-1.5 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-500 hover:text-white transition-colors shadow-sm" onClick={(e) => { e.stopPropagation(); setEditingId(String(displaySlot.id)); setFormData({ teacher_id: displaySlot.teacher_id || '', section_id: displaySlot.section_id || '', subject_id: displaySlot.subject_id || '' }); setSelectedSlot({day: day.id, period: period}); setIsModalOpen(true); }}>تعديل</button>
                                      <button className="text-[10px] font-black px-3 py-1.5 rounded-lg bg-rose-50 text-rose-600 hover:bg-rose-500 hover:text-white transition-colors shadow-sm" onClick={(e) => { e.stopPropagation(); handleDeleteSchedule(String(displaySlot.id)); }}>حذف</button>
                                    </div>
                                  </div>
                                )}
      ...
      {/* ======================================================== */}
      {/* 🖨️ منطقة تجهيز الـ PDF (لا تظهر في الواجهة أبداً) */}
      {/* تم استخدام مصفوفة مفلترة لتجاهل الجداول الفارغة كلياً */}
      {/* ======================================================== */}
                            ...
                            <td key={p.id} className={`border border-slate-300 p-2 align-middle text-center ${index % 2 === 0 ? 'bg-slate-50/50' : 'bg-white'}`}>
                              {slot ? (
                                <div className="flex flex-col items-center justify-center h-full gap-2 p-2 bg-white rounded-lg shadow-sm border border-slate-200">
                                  ...
                                  {slot.teachers?.zoom_link && (
                                    <a href={normalizeUrl(slot.teachers.zoom_link)} className="zoom-link inline-flex items-center justify-center gap-1.5 text-xs font-black text-emerald-800 bg-emerald-100 border border-emerald-300 px-4 py-2 rounded-lg mt-1 w-[90%] shadow-sm" style={{ WebkitPrintColorAdjust: 'exact' }}>
                                      <Video className="w-4 h-4" /> <span>دخول البث</span>
                                    </a>
                                  )}
                                </div>
                              ) : (
