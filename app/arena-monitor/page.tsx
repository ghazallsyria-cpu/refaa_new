...
export default function ArenaMonitorDashboard() {
  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={containerVariants} className="min-h-[100dvh] bg-transparent text-slate-100 py-8 px-4 font-sans relative overflow-x-hidden pt-6" dir="rtl">
                      ...
                      displayedStudents.map((student) => (
                        <tr key={student.student_id} className={`hover:bg-white/5 transition-colors group ${!student.has_started ? 'opacity-50 grayscale' : ''}`}>
                          ...
                          <td className="p-4 px-6 align-middle">
                            {/* 🚀 إزالة الإخفاء على الشاشات الصغيرة لجعل الأزرار ظاهرة دائماً */}
                            <div className="flex items-center justify-center gap-2 sm:gap-3 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                              {student.has_started && (
                                <button 
                                  onClick={() => handleResetStudentProgress(student.student_id, student.student_name)}
                                  className="bg-white/5 border border-rose-500/30 hover:border-rose-400 hover:text-rose-100 hover:bg-rose-500 p-2 sm:p-2.5 rounded-xl text-rose-400 transition-all shadow-inner active:scale-90 shrink-0"
                  ...
                  <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 pb-32 space-y-6 sm:space-y-8 bg-transparent custom-scrollbar relative">
                    ...
                    <div className="relative z-10 space-y-6 sm:space-y-8 max-w-6xl mx-auto">
                      {assignmentQuestions.map((q, idx) => {
                        ...
                        // 🚀 معالجة ذكية للإجابات لاستخراج النصوص أو الخيارات المتعددة (JSON or String)
                        let studentText = 'لم يقم الطالب بالإجابة على هذا السؤال.';
                        if (studentAnsObj) {
                          if (studentAnsObj.answer_text) {
                            studentText = String(studentAnsObj.answer_text);
                          } else if (studentAnsObj.selected_options) {
                            let opts = studentAnsObj.selected_options;
                            if (typeof opts === 'string') {
                               try { opts = JSON.parse(opts); } catch (e) { /* ابقه كنص عادي */ }
                            }
                            if (Array.isArray(opts) && opts.length > 0) {
                               ...
                               studentText = String(opts); // حماية إضافية
                            }
                          } else if (studentAnsObj.text_answer) {
                            studentText = String(studentAnsObj.text_answer);
                          }
                        }
