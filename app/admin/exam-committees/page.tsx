...
// 🔥 توزيع رؤساء اللجان بالتساوي (Round-Robin) باختيار الأقل مشاركة
         const sortedHeads = [...allHeadsList].sort((a, b) => {
             const shiftsA = teacherTotalShifts.get(String(a.id)) || 0;
             const shiftsB = teacherTotalShifts.get(String(b.id)) || 0;
             if (shiftsA === shiftsB) return Math.random() - 0.5; // تعادل؟ عشوائي
             return shiftsA - shiftsB; // الأقل يأتي أولاً
         });
         const selectedHeadsForToday = sortedHeads.slice(0, 5);
                                       ...
                                       {commInvigs.map((inv, iIdx) => (
                                          <li key={`inv-${iIdx}`} className="flex items-center justify-between text-sm font-bold text-slate-700 bg-slate-50 p-3 rounded-lg border border-slate-100">
                                             <div className="flex items-center gap-2">
                                              ...
                                              <span className={inv.role === 'locked' ? 'font-black text-amber-700' : ''}>{getInvName(inv)}</span>
                                             </div>
                                             <div className="shrink-0 text-left">
                                                {inv?.status === 'signed' ? (
                                                   <span className="text-emerald-600 font-black text-[10px] sm:text-xs flex items-center gap-1"><CheckCircle2 className="w-3 h-3 sm:w-4 sm:h-4"/> وقّع إلكترونياً</span>
                                                ) : inv?.status === 'excused' ? (
