...
export default function AutoScheduleGenerator() {
  ...
  // 🚀 إضافة حالة للأقسام الأكاديمية الرسمية
  ...
  const fetchMasterData = async () => {
    try {
      ...
      // 🚀 جلب الأقسام الأكاديمية الرسمية من المنصة
      ...
      let tsData = [];
      // 🚀 جلب department_id لكل معلم لربطه بالقسم الرسمي
    ...
    try {
      ...
      const formatted = (slots || []).map(s => {
        ...
        return {
          ...
          zoom_link: zoomL, teacher_id: s.teacher_id
        };
      });
  ...
  // 🚀 المحرك الديناميكي الذكي للبحث عن القسم الرسمي من قاعدة البيانات أو التخمين كخط دفاع أخير
  const getTeacherDept = (tId: string) => {
    // 1. البحث عن القسم الرسمي من جدول teachers
    ...
    if (assignment?.teachers?.department_id) {
        ...
        if (dept) return dept.name; // القسم الحقيقي بنسبة 100%
    }

    // 2. خط الدفاع الأخير: تخمين متقدم للقسم في حال لم يتم ربط المعلم
  ...
  // 🚀 تحديث قائمة الأقسام لتعرض فقط الأقسام النشطة بدلاً من قائمة صلبة
  ...
  }, [uniqueTeachersInSchedule, departments, rawTeacherAssignments]);
