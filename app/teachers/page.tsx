...
import { 
  ...
  Users, GraduationCap, Folder, ChevronLeft, School, Layers, Award, Crown
} from 'lucide-react';
...
export default function TeachersPage() {
  const { user } = useAuth(); 
  const {
    teachers, subjects, sections, departments, loading: usersLoading,
    fetchTeachers, fetchSections, fetchSubjects, fetchDepartments, addTeacher, updateTeacher, deleteUser, resetPassword
  ...
  const [editForm, setEditForm] = useState({
    full_name: '', national_id: '', email: '', phone: '', specialization: '', zoom_link: '', department_id: '',
  ...
  const handleAddSubmit = async () => { 
    ...
    if (!addForm.full_name || !addForm.national_id || !addForm.department_id) { showNotification('error', 'يرجى تعبئة الحقول الإلزامية واختيار القسم'); return; } 
    try { 
      setSubmitting(true); 
  ...
  const handleEditClick = (teacher: any) => {
    ...
    const isHod = teacher.isHOD; // 👈 نعتمد على الكشف المبرمج في الهوك
    const hodData = teacher.department_heads && teacher.department_heads.length > 0 ? teacher.department_heads[0] : null;
    setEditForm({ 
      ...
      department_id: teacher.department_id || '', // 👈 جلب القسم المحفوظ
  ...
  // 🚀 الفلترة مبنية على القسم المحفوظ في القاعدة
  const filteredTeachers = useMemo(() => {
    return (teachers as any[]).filter((teacher: any) => {
      const stageInfo = getTeacherStageInfo(teacher);
      const parentDept = teacher.academic_departments?.id;
  ...
  const departmentHeads = filteredTeachers.filter((t: any) => t.isHOD);
  const departmentMembers = filteredTeachers.filter((t: any) => !t.isHOD);
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`p-6 rounded-[2rem] bg-white border ${isHOD ? 'border-amber-200 shadow-lg shadow-amber-50' : 'border-slate-100 shadow-sm'} relative overflow-hidden group hover:-translate-y-1 transition-all`}>
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              <div className={`h-14 w-14 rounded-2xl flex items-center justify-center font-black text-xl ${isHOD ? 'bg-amber-100 text-amber-600' : 'bg-indigo-50 text-indigo-600'}`}>
                {teacher.users?.avatar_url ? <img src={teacher.users.avatar_url} className="w-full h-full rounded-2xl object-cover" alt="T"/> : teacher.users?.full_name?.charAt(0)}
              </div>
        ...
        <div className="mt-4 flex flex-wrap gap-1.5">
           {isHOD && <span className="px-2 py-1 bg-amber-500 text-white text-[9px] font-black rounded-md">رئيس قسم {teacher.department_heads?.[0]?.subjects?.name}</span>}
  ...
  return (
    <div className="space-y-8 pb-20 font-cairo" dir="rtl">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-slate-900 tracking-tight">{selectedDepartment ? `قسم ${departments.find((d:any)=>d.id===selectedDepartment)?.name}` : 'إدارة المعلمين والأقسام'}</h1>
      ...
      {!selectedDepartment ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {departments?.map((dept:any) => (
            <motion.div key={dept.id} whileHover={{ y: -5 }} onClick={() => setSelectedDepartment(dept.id)} className="bg-white p-8 rounded-[2.5rem] cursor-pointer border-2 border-slate-50 hover:border-indigo-100 transition-all text-center group">
              ...
              <form className="space-y-8">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  ...
                  <div className="sm:col-span-2 space-y-1.5">
                    <label className="text-xs font-black text-slate-400 mr-2">القسم الأكاديمي المعتمد <span className="text-rose-500">*</span></label>
                    <select value={editForm.department_id} onChange={e => setEditForm({...editForm, department_id: e.target.value})} className="w-full px-4 py-3 bg-slate-50 rounded-xl font-bold outline-none cursor-pointer">
