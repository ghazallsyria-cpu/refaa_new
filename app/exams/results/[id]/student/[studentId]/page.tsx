  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-10 pb-24" dir="rtl">
      ...
      <div className="flex items-center justify-between">
        <button onClick={() => router.back()} className="flex items-center gap-3 px-6 py-3 rounded-2xl bg-white border border-slate-100 text-slate-500 font-black hover:text-indigo-600 transition-all shadow-sm">
          <ArrowRight className="h-5 w-5" /> العودة للنتائج
        </button>
      </div>

      {/* بطاقة معلومات الطالب والاختبار */}
      <div className="glass-card p-10 rounded-[40px] border-t-[16px] border-t-indigo-600 shadow-2xl relative overflow-hidden bg-white">
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="bg-indigo-50/50 px-6 py-5 rounded-[28px] border border-indigo-100 shadow-sm transition-all hover:bg-indigo-50">
            ...
            <div className="bg-slate-50 px-6 py-5 rounded-[28px] border border-slate-100 shadow-sm">
              <span className="text-[10px] text-slate-400 block font-black uppercase tracking-widest mb-1 flex items-center gap-2">
                <Clock size={12} className="text-slate-500" /> مدة الاختبار
              </span>
              <span className="text-xl font-black text-slate-800">
      ...
      <div className="space-y-8">
        <h2 className="text-2xl font-black text-slate-900 flex items-center gap-3">
          <CheckCircle className="text-emerald-500" /> إجابات الطالب بالتفصيل
        </h2>
        ...
        {answers.length === 0 ? (
          ...
          answers.map((ans, idx) => (
            <motion.div 
              ...
              viewport={{ once: true }}
              key={ans.id} 
              className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-xl relative overflow-hidden"
            >
