 ...
 * * 🛠️ التحديث الحالي:
 * - إضافة فلتر تطهير نصوص (cleanNameForSort) يتجاهل النقاط والمسافات والهمزات
 * في بداية الأسماء لمنع اختلال الترتيب الأبجدي (إصلاح ثغرة الطالب "يوسف").
 * - خوارزمية السحّاب الأبجدي (Zipper Algorithm): تدمج طلاب العاشر والحادي عشر.
 * - نظام "لجنة الفائض" (Overflow Room) للتوزيع اليدوي لاحقاً.
 * ============================================================================
 */
  ...
  const generateSeatingAndDistribute = useCallback(async (academicYear: string, semester: string) => {
    ...
    try {
      ...
      for (const committee of committees) {
        // تخطي "لجنة الفائض" إذا كانت موجودة مسبقاً لتجنب التعبئة التلقائية فيها
      ...
      // إذا تبقى طلاب ولم تكفِ اللجان، يتم وضعهم في "لجنة فائض"
      if (studentPointer < zippedStudents.length) {
        setProgressMsg('توليد لجنة الفائض للطلاب المتبقين...');
        
        // التحقق مما إذا كانت لجنة الفائض موجودة مسبقاً
