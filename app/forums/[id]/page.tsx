    ...
    <div className="min-h-screen bg-slate-50/50 pb-24" dir="rtl">
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 py-4 sm:px-6 lg:px-8 flex items-center justify-between gap-4">
          ...
          >
            <Plus className="w-4 h-4" /> <span className="hidden sm:inline">نشر موضوع</span>
          </button>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden divide-y divide-slate-100">
            {filteredTopics.map((topic) => (
              <Link key={topic.id} href={`/forums/topic/${topic.id}`} className="block hover:bg-slate-50 transition-colors group">
                <div className="p-5 sm:p-6 flex items-start gap-4">
                  {/* أيقونة حالة الموضوع */}
                  <div className="shrink-0 pt-1">
                    {topic.is_pinned ? (
                      <div className="p-2 bg-rose-50 text-rose-500 rounded-lg"><Pin className="w-5 h-5" /></div>
                    ) : topic.is_locked ? (
                      <div className="p-2 bg-slate-100 text-slate-400 rounded-lg"><Lock className="w-5 h-5" /></div>
                    ) : (
                      <div className="p-2 bg-indigo-50 text-indigo-400 rounded-lg group-hover:bg-indigo-600 group-hover:text-white transition-colors"><MessageSquare className="w-5 h-5" /></div>
                    )}
                  </div>
                  
                  {/* تفاصيل الموضوع */}
                  <div className="flex-1 min-w-0">
                    <h3 className="text-base sm:text-lg font-black text-slate-900 mb-1.5 truncate group-hover:text-indigo-600 transition-colors">
                      {topic.title}
                    </h3>
                    <div className="flex flex-wrap items-center gap-3 sm:gap-4 text-[10px] sm:text-xs font-bold text-slate-500">
                      <span className="flex items-center gap-1.5 bg-slate-100 px-2 py-1 rounded-md text-slate-600">
                        <User className="w-3.5 h-3.5" /> {topic.author_name}
                      </span>
                  ...
                  <div className="hidden sm:flex flex-col items-end gap-1 shrink-0 text-slate-400">
                    <div className="flex items-center gap-1.5 text-xs font-black">
                      <span>{topic.replies_count}</span> <MessageSquare className="w-3.5 h-3.5" />
                    </div>
                  </div>
                </div>
              </Link>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
            <motion.div 
              ...
              className="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl overflow-hidden border border-slate-100 my-auto"
            >
              <div className="bg-slate-50 p-6 border-b border-slate-100 flex items-center justify-between">
                <div>
                  <h2 className="text-xl font-black text-slate-900">موضوع جديد</h2>
              ...
              <form onSubmit={handleCreateTopic} className="p-6 space-y-5">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">محتوى الموضوع</label>
                  <textarea 
                    rows={6} required value={newContent} onChange={e => setNewContent(e.target.value)}
                    placeholder="اكتب تفاصيل نقاشك أو سؤالك هنا..."
                ...
                <div className="flex items-center justify-end pt-2">
                  <button type="submit" disabled={isSubmitting} className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3.5 rounded-xl font-black text-sm transition-all shadow-md active:scale-95 flex items-center gap-2">
