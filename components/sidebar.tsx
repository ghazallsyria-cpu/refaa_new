    ...
    <div className={cn(
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-1 relative z-20">
          {onToggleCollapse && (
            <button onClick={onToggleCollapse} className="hidden lg:flex p-2 text-slate-400 hover:text-white hover:bg-white/10 rounded-xl transition-all active:scale-90">
              {isCollapsed ? <ChevronLeft className="h-5 w-5" /> : <ChevronRight className="h-5 w-5" />}
            </button>
          )}
          {onClose && (
            <button onClick={onClose} className="lg:hidden p-2 text-slate-400 hover:text-white hover:bg-rose-500/20 hover:text-rose-400 rounded-xl transition-all active:scale-90">
              <X className="h-6 w-6" />
            </button>
          )}
        </div>
              ...
              <motion.div key={item.name} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.02 }}>
                <Link
                  ...
                  className={cn(
                    ...
                    isActive 
                      ? isSpecialBtn ? "bg-gradient-to-r from-rose-600 to-red-600 text-white" 
                        : isRadarBtn ? "bg-gradient-to-r from-emerald-500 to-teal-600 text-white"
                        : isBadgeBtn ? "bg-gradient-to-r from-amber-500 to-orange-500 text-white"
                        : isGoldBtn ? "bg-gradient-to-r from-amber-400 to-yellow-600 text-slate-900 shadow-lg shadow-amber-500/20"
                        : "bg-gradient-to-r from-indigo-600 to-violet-600 text-white shadow-lg shadow-indigo-600/30" 
                      : "hover:bg-white/5 hover:text-white text-slate-400"
                  )}
                >
                  <item.icon className={cn("h-5 w-5 shrink-0 transition-all", !isCollapsed && "ml-3.5", isActive ? "scale-110" : "group-hover:text-indigo-400")} />
                  <span className={cn("relative z-10 transition-all duration-500 whitespace-nowrap font-black", isCollapsed ? "w-0 opacity-0 scale-0 overflow-hidden" : "w-auto opacity-100 scale-100")}>
                    {item.name}
                  </span>
                  {isActive && (
      ...
      <div className={cn("p-4 border-t border-slate-800/80 relative z-10 bg-slate-900/50 backdrop-blur-md", isCollapsed ? "items-center" : "")}>
        <div className={cn("bg-white/5 backdrop-blur-sm rounded-[1rem] flex items-center border border-white/10 hover:bg-white/10 transition-all cursor-pointer group overflow-hidden shadow-inner", isCollapsed ? "justify-center p-2" : "gap-3 p-3")}>
          <div className="relative shrink-0">
            <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white font-black text-xs shadow-md ring-2 ring-slate-800">
