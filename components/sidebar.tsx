...
// ... (نفس مصفوفة navigation السابقة بدون تغيير) ...
const navigation = [
...
export function Sidebar({ onClose, authRole = 'admin', isCollapsed = false, onToggleCollapse }: { onClose?: () => void, authRole?: string, isCollapsed?: boolean, onToggleCollapse?: () => void }) {
  ...
  return (
    <div className={cn("flex h-full flex-col bg-[#0a0d16]/80 backdrop-blur-3xl text-slate-300 border-l border-white/5 relative overflow-hidden transition-all duration-500 z-50", isCollapsed ? "w-20" : "w-72", "group/sidebar")}>
      ...
      <div className={cn("flex h-24 shrink-0 items-center border-b border-white/5 relative z-10 transition-all duration-500 bg-[#02040a]/40", isCollapsed ? "justify-center px-0" : "justify-between px-6")}>
        <motion.div initial={false} animate={{ opacity: isCollapsed ? 0 : 1, scale: isCollapsed ? 0.8 : 1, x: isCollapsed ? 20 : 0 }} className={cn("flex items-center gap-3 transition-all duration-300", isCollapsed ? "pointer-events-none absolute" : "relative")}>
          <div className="flex h-12 w-12 items-center justify-center rounded-[1rem] bg-[#0f1423] shadow-[0_0_15px_rgba(245,158,11,0.15)] ring-1 ring-amber-500/30 group-hover/sidebar:rotate-3 transition-transform duration-300 overflow-hidden relative border border-white/10">
          ...
          <div className="flex flex-col">
            <span className="text-lg font-black text-white tracking-tight leading-none drop-shadow-md">{schoolData.name}</span>
            <span className="text-[9px] text-amber-500 font-bold uppercase tracking-[0.2em] mt-1.5">المنصة الرقمية</span>
          </div>
        </motion.div>
        
        {isCollapsed && (
          <motion.div initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} className="flex h-12 w-12 items-center justify-center rounded-[1rem] bg-[#0f1423] shadow-[0_0_15px_rgba(245,158,11,0.15)] ring-1 ring-amber-500/30 relative z-20 overflow-hidden border border-white/10">
            {schoolData.logo_url ? <Image src={schoolData.logo_url} alt="Logo" fill className="object-contain p-1.5" /> : <School className="h-6 w-6 text-amber-500" />}
          </motion.div>
        )}

        <div className="flex items-center gap-1 relative z-20">
          {onToggleCollapse && <button onClick={onToggleCollapse} className="hidden lg:flex p-2 text-slate-400 hover:text-amber-400 hover:bg-white/5 rounded-xl transition-all active:scale-90">{isCollapsed ? <ChevronLeft className="h-5 w-5" /> : <ChevronRight className="h-5 w-5" />}</button>}
          {onClose && <button onClick={onClose} className="lg:hidden p-2 text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-xl transition-all active:scale-90"><X className="h-6 w-6" /></button>}
        </div>
      </div>
          ...
          {filteredNavigation.map((item, idx) => {
            ...
            return (
              <motion.div key={item.name} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.02 }}>
                <Link href={itemHref} onClick={onClose} className={cn("flex items-center rounded-xl text-sm font-bold transition-all duration-300 group relative overflow-hidden", isCollapsed ? "justify-center p-3" : "px-4 py-3.5", isActive ? "sidebar-link-active" : "hover:bg-white/5 hover:text-white text-slate-400 hover:shadow-sm")}>
                  <item.icon className={cn("h-5 w-5 shrink-0 transition-all", !isCollapsed && "ml-3.5", isActive ? "scale-110 text-slate-950" : "text-slate-400 group-hover:text-amber-400")} />
      ...
      <div className={cn("p-4 border-t border-white/5 relative z-10 bg-[#02040a]/40", isCollapsed ? "items-center" : "")}>
        <div className={cn("bg-[#0f1423] rounded-[1rem] flex items-center border border-white/5 hover:border-amber-500/30 transition-all cursor-pointer group overflow-hidden shadow-inner", isCollapsed ? "justify-center p-2" : "gap-3 p-3")}>
          <div className="relative shrink-0">
            <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-amber-500 to-yellow-400 flex items-center justify-center text-slate-950 font-black text-xs shadow-[0_0_15px_rgba(245,158,11,0.4)] ring-1 ring-white/20">{roleDisplayName.substring(0, 2)}</div>
            <div className="absolute -bottom-1 -left-1 w-3.5 h-3.5 bg-emerald-500 border-2 border-[#0f1423] rounded-full shadow-[0_0_5px_rgba(16,185,129,0.5)]" />
          </div>
          {!isCollapsed && (
            <div className="flex flex-col overflow-hidden">
              <span className="text-xs font-black text-white truncate drop-shadow-md">{roleDisplayName}</span>
              <span className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">لوحة التحكم</span>
            </div>
          )}
        </div>
      </div>
