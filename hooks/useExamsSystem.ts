...
export const clearExamsCache = () => { globalCache.clear(); };
...
export function useExamsSystem() {
      ...
      const fetchedData = await withCache(cacheKey, async () => {
        ...
        if (currentRole === 'student') {
          ...
          if (!teacherProfile && user.user_metadata?.role === 'teacher') {
            const { data: newTeacher } = await supabase.from('teachers').insert({ id: user.id, national_id: 'TEMP_' + user.id.substring(0, 8), specialization: 'غير محدد' }).select('id').single();
        ...
        if (['teacher', 'admin', 'management'].includes(currentRole || '')) {
          const examsWithStats = await Promise.all(mappedData.map(async (e) => {
            const [attemptsRes, questionsRes] = await Promise.all([ supabase.from('exam_attempts').select('score, status').eq('exam_id', e.id), supabase.from('questions').select('id', { count: 'exact', head: true }).eq('exam_id', e.id) ]);
        ...
        if (currentRole === 'student' && currentStudentProfileId) {
          ...
          mappedData = mappedData.map(exam => {
            ...
            return { ...exam, submission_status: attempt ? (attempt.status === 'completed' || attempt.status === 'graded' ? 'submitted' : 'pending') : 'pending', score: attempt?.score };
          });
        }
        return mappedData;
      }, forceRefresh);
      setData(fetchedData);
    } catch (err: unknown) { setError(err instanceof Error ? err.message : 'Error fetching exams'); } finally { setLoading(false); }
  }, [user, currentRole]);
  ...
  const fetchExamDetails = useCallback(async (examId: string): Promise<ExamDetails> => {
    try {
      ...
      return { exam: { ...examData, section_ids: examSectionsData ? examSectionsData.map((es: any) => es.section_id) : [] }, questions: mapQuestionsWithMedia(questionsData || []) };
    } catch (err) { throw err; }
  }, []);
  ...
  const saveExam = useCallback(async (examData: Partial<Exam> & { section_ids?: string[] }, questions: Question[], isNew: boolean): Promise<string> => {
    ...
    try {
      const response = await fetch('/api/exams/save', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ examData, questions, isNew, userId: user.id }) });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || 'Failed to save exam');
      clearExamsCache(); await fetchExams(true);
  ...
  const fetchExamForStudent = useCallback(async (examId: string): Promise<ExamForStudent> => {
    ...
    try {
      if (currentRole === 'student') {
        ...
        if (!sp) throw new Error('حساب الطالب غير مكتمل');
      }
      ...
      return {
        exam: { ...examData, subject_name: Array.isArray(examData.subject) ? examData.subject[0]?.name : examData.subject?.name, teacher_name: Array.isArray(examData.teacher?.users) ? examData.teacher.users[0]?.full_name : examData.teacher?.users?.full_name },
  ...
  const submitExam = useCallback(async (examId: string, answers: Record<string, any>, score: number, status: string, timeSpent: number): Promise<string> => {
    if (!user) throw new Error('User not authenticated');
    try {
      const response = await fetch('/api/exams/submit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ examId, answers, score, status, timeSpent, userId: user.id }) });
      const result = await response.json();
  ...
  const deleteExamWithMedia = useCallback(async (examId: string): Promise<{ success: boolean }> => {
    ...
    try {
      ...
      if (questions && questions.length > 0) { for (const q of questions) { if (q.media_url) { try { await deleteFromCloudinary(q.media_url); } catch (e) {} } } }
      const response = await fetch('/api/exams/delete', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ examId, userId: user.id }) });
      if (!response.ok) throw new Error('Failed to delete exam');
      clearExamsCache(); await fetchExams(true);
      return { success: true };
  ...
  const fetchExamResults = useCallback(async (examId: string): Promise<ExamResults> => {
    try {
      await fetch(`/api/exams/results-data?examId=${examId}`, { cache: 'no-store' }).catch(() => null);
      ...
      if (examData?.section_ids && examData.section_ids.length > 0) {
        ...
        if (students) {
          studentsData = students.map((s: any) => ({ id: s.id, full_name: s.users?.full_name || 'طالب غير معروف', email: s.users?.email || '', section_name: s.section?.name ? `${Array.isArray(s.section?.classes) ? s.section?.classes[0]?.name : s.section?.classes?.name} - ${s.section.name}` : 'غير محدد' }));
        }
      }
      ...
      return { exam: examData, students: studentsData, attempts: attemptsData || [], questions: mapQuestionsWithMedia(qData || []), answers: aData || [] };
    } catch (err) { throw err; }
  }, []);
  ...
  const deleteAttempt = useCallback(async (attemptId: string): Promise<{ success: boolean }> => {
    ...
    try {
      ...
      clearExamsCache(); return { success: true };
    } catch (err) { throw err; }
  }, [user]);

  const fetchStudentExamResult = useCallback(async (examId: string, studentId: string): Promise<StudentExamResult> => {
    try {
      const response = await fetch('/api/exams/student-result', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ examId, studentId }), cache: 'no-store' });
      const result = await response.json();
  ...
  const gradeAnswer = useCallback(async (attemptId: string | null, questionId: string, pointsEarned: number, examId?: string, studentId?: string): Promise<void> => {
    try {
      const response = await fetch('/api/exams/grade', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ attemptId, questionId, pointsEarned, examId, studentId }) });
      const result = await response.json();
