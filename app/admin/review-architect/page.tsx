  ...
  const handleIngestData = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const questionsToInsert = parsedData.map((q: any) => ({
        ...
        image_url: q.image_url || null // إضافة حقل الصورة
      }));
      ...
      setStatus({ type: 'success', msg: `تم حقن ${questionsToInsert.length} سؤالاً بنجاح.` });
      setJsonInput('');
      setTitle('');
    } catch (err: any) {
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative glass-panel p-6 sm:p-10 rounded-[2.5rem] sm:rounded-[3rem] border border-white/10 overflow-hidden shadow-2xl">
          <div className="mb-8 border-b border-white/5 pb-6">
            <h2 className="text-xl sm:text-3xl font-black text-white flex items-center gap-2">
              <Database className="w-6 h-6 text-amber-400" /> مهندس المراجعات الوزارية
            </h2>
          </div>
          ...
          <form onSubmit={handleIngestData} className="space-y-6 relative z-10">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div className="space-y-2">
                ...
                <input type="text" required value={title} onChange={e => setTitle(e.target.value)} className="w-full bg-[#02040a]/60 border border-white/10 rounded-xl p-3.5 text-white outline-none focus:border-amber-500" />
              </div>
              <div className="space-y-2">
                <label className="text-xs font-black text-slate-400"><Layers className="inline w-3.5 h-3.5" /> المرحلة</label>
              ...
              <div className="space-y-2">
                <label className="text-xs font-black text-slate-400"><BookOpen className="inline w-3.5 h-3.5" /> المادة</label>
                <input type="text" required value={subject} onChange={e => setSubject(e.target.value)} className="w-full bg-[#02040a]/60 border border-white/10 rounded-xl p-3.5 text-white outline-none" />
              </div>
            </div>

            <div className="bg-indigo-500/5 border border-indigo-500/20 rounded-2xl p-5 relative">
              <div className="flex justify-between items-center mb-2">
                <label className="text-xs font-black text-indigo-300"><Bot className="inline w-4 h-4" /> البرومبت الذهبي (انسخه للذكاء الاصطناعي)</label>
            ...
            <div className="space-y-2">
              <label className="text-xs font-black text-emerald-400"><ClipboardCopy className="inline w-3.5 h-3.5" /> الصق الـ JSON هنا</label>
              <textarea required rows={8} value={jsonInput} onChange={e => setJsonInput(e.target.value)} className="w-full bg-[#02040a]/80 border border-white/10 rounded-2xl p-4 text-emerald-400 font-mono text-xs outline-none focus:border-emerald-500 custom-scrollbar" dir="ltr" />
            </div>

            {status && (
              <div className={`p-4 rounded-xl font-bold text-sm ${status.type === 'success' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
                {status.msg}
              </div>
            )}

            <button type="submit" disabled={loading} className="w-full bg-amber-500 text-black font-black py-4 rounded-xl hover:opacity-90">
              {loading ? 'جاري المعالجة...' : 'حقن البيانات'}
            </button>
          </form>
        </motion.div>
      </div>
