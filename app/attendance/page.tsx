...
import { 
  ...
  Bug, RefreshCw, Calculator, Layers, PieChart, ChevronDown, Loader2 
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import Link from 'next/link';

// تم تعديل مسارات الاستيراد
...
// 🚀 حل مشكلة Tailwind Purge: تعريف الفئات بالكامل لمنع اختفائها في الإنتاج
const ATTENDANCE_OPTIONS = [
  { 
    ...
    label: 'حاضر',
    mobileLabel: 'حاضر',
    activeClasses: 'bg-emerald-50 border-emerald-500 text-emerald-700 shadow-md shadow-emerald-100',
    inactiveClasses: 'bg-white border-slate-100 text-slate-400 hover:border-emerald-200 hover:bg-emerald-50'
  },
  { 
    status: 'absent', 
    icon: XCircle, 
    label: 'غائب',
    mobileLabel: 'غائب',
    activeClasses: 'bg-rose-50 border-rose-500 text-rose-700 shadow-md shadow-rose-100',
    inactiveClasses: 'bg-white border-slate-100 text-slate-400 hover:border-rose-200 hover:bg-rose-50'
  },
  { 
    status: 'late', 
    icon: Clock, 
    label: 'متأخر',
    mobileLabel: 'تأخر',
    activeClasses: 'bg-amber-50 border-amber-500 text-amber-700 shadow-md shadow-amber-100',
    inactiveClasses: 'bg-white border-slate-100 text-slate-400 hover:border-amber-200 hover:bg-amber-50'
  },
  { 
    status: 'excused', 
    icon: AlertCircle, 
    label: 'مستأذن',
    mobileLabel: 'عذر',
    activeClasses: 'bg-blue-50 border-blue-500 text-blue-700 shadow-md shadow-blue-100',
    inactiveClasses: 'bg-white border-slate-100 text-slate-400 hover:border-blue-200 hover:bg-blue-50'
  }
];
...
export default function AttendancePage() {
  ...
  // 🚀 تحسين الـ useEffect لمنع الـ Infinite Loop
  useEffect(() => {
  ...
  // 🚀 شاشة حماية الوصول (Security Guard) لمنع الوميض
  if (isChecking) {
    return (
  ...
  // ==========================================
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 sm:space-y-8 max-w-7xl mx-auto pb-24 px-4 sm:px-6 lg:px-8 overflow-x-hidden font-cairo" dir="rtl">
                    ...
                    {ATTENDANCE_OPTIONS.map((opt) => (
                      <td key={opt.status} className="whitespace-nowrap px-1.5 sm:px-2 py-3 sm:py-4 text-center">
                        <label className="relative inline-flex cursor-pointer group/radio w-full justify-center">
                          ...
                          <div className={`px-2 sm:px-4 py-2 sm:py-2.5 rounded-xl sm:rounded-2xl border-2 transition-all flex items-center justify-center gap-1.5 sm:gap-2 font-bold text-[10px] sm:text-xs shadow-sm active:scale-95 ${attendance[student.id] === opt.status ? opt.activeClasses : opt.inactiveClasses}`}>
                            <opt.icon className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${attendance[student.id] === opt.status ? 'animate-in zoom-in-50' : ''}`} />
            ...
            students.map((student: any) => (
              <div key={student.id} className="p-4 space-y-3 bg-white hover:bg-slate-50/50 transition-colors">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {ATTENDANCE_OPTIONS.map((opt) => (
                    <button key={opt.status} onClick={() => handleStatusChange(student.id, opt.status as any)} className={`flex flex-col items-center justify-center gap-1 py-2 px-1 rounded-xl border-2 transition-all active:scale-95 ${attendance[student.id] === opt.status ? opt.activeClasses : opt.inactiveClasses}`}>
