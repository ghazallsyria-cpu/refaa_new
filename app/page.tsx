...
/**
 * ============================================================================
 * 🏗️ التوثيق الهندسي (Gemini Style Edition - Ultra Performance)
 * ============================================================================
 * @file        app/page.tsx
 * @version     8.0.0 (Alive UI - GPU Optimized Bento Campus)
...
// إعدادات الحركة الموحدة للأداء العالي
...
export default function DigitalCampusPage() {
  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={containerVariants} className="min-h-[100dvh] bg-[#02040a] text-slate-200 font-sans overflow-x-hidden relative pb-20 sm:pb-32 pt-2 sm:pt-6" dir="rtl">
      ...
      {/* 🚀 وحدة الكونسول الطافية تظهر فقط في حالة الانهيار البرمجي */}
      {hasCrashed && (
      ...
      {/* 🌌 الإضاءة الخلفية المحيطية (آمنة للجوال - بدون Blur ضخم مدمر للـ GPU) */}
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
         {/* استخدام Radial Gradient نقي بدلاً من CSS Blur الثقيل */}
         <div className="absolute top-[-20%] right-[-10%] w-[80vw] h-[80vw] max-w-[600px] max-h-[600px] bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-indigo-900/40 to-transparent opacity-60"></div>
         <div className="absolute bottom-[-20%] left-[-10%] w-[70vw] h-[70vw] max-w-[500px] max-h-[500px] bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-blue-900/30 to-transparent opacity-60"></div>
      ...
      {breakingNews && (
        <div className="fixed bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 w-[95%] max-w-5xl z-50 pointer-events-none">
          <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 1, type: "spring" }} className="w-full bg-[#0f1423]/90 backdrop-blur-md border border-white/10 shadow-lg rounded-full flex items-center overflow-hidden pointer-events-auto p-1.5">
             <div className="bg-indigo-600 px-4 sm:px-6 py-2.5 sm:py-3 rounded-full font-black text-[10px] sm:text-xs text-white flex items-center gap-1.5 sm:gap-2 shrink-0 z-10 shadow-inner">
               <BellRing className="w-3.5 h-3.5 sm:w-4 sm:h-4 animate-pulse drop-shadow-md" /> عاجل
             </div>
             <div className="flex-1 overflow-hidden h-full flex items-center">
               <div className="marquee-content whitespace-nowrap font-bold text-indigo-100 text-[11px] sm:text-sm tracking-wide flex gap-12 sm:gap-16 py-2 sm:py-3 opacity-90">
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10 space-y-6 sm:space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-6 min-h-[60vh] sm:min-h-[70vh]">
           ...
           <motion.div style={{ opacity: opacityHero }} variants={itemVariants} className="lg:col-span-8 xl:col-span-9 bg-[#0f1423]/60 backdrop-blur-md rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-10 lg:p-14 border border-white/10 shadow-lg relative overflow-hidden flex flex-col justify-center group">
              {/* بديل آمن للإضاءة الفائقة */}
              <div className="absolute top-[-10%] right-[-10%] w-64 h-64 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-indigo-500/20 to-transparent pointer-events-none"></div>
              ...
              <AnimatePresence mode="wait">
                <motion.div key={currentSlideData?.id || 'default'} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5 }} className="relative z-10">
                  {currentSlideData?.badge_text && (
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 sm:px-5 sm:py-2 rounded-full bg-indigo-500/20 border border-indigo-500/40 text-indigo-200 text-[10px] sm:text-sm font-black mb-6 sm:mb-8">
                      <SlideIcon className="w-4 h-4 text-emerald-400 drop-shadow-sm" /> {currentSlideData.badge_text}
                    </div>
                  )}
                  
                  <h1 className={`text-4xl sm:text-6xl md:text-7xl lg:text-[6rem] font-black tracking-tighter mb-4 sm:mb-6 leading-[1.1] text-transparent bg-clip-text bg-gradient-to-l ${currentSlideData?.color_gradient || 'from-indigo-300 via-white to-blue-300'}`}>
                    {currentSlideData?.title || 'مدرسة الرفعة'}
                  </h1>
                  
                  {currentSlideData?.description && (
                    <p className="text-slate-300 text-sm sm:text-lg md:text-xl font-bold max-w-3xl leading-relaxed sm:leading-loose mb-8 sm:mb-10 opacity-90">
                      {currentSlideData.description}
                    </p>
                  )}

                  <div className="flex flex-wrap items-center gap-3 sm:gap-4">
                    <Link href={portal?.href || '/login'} className="group/btn relative inline-flex items-center gap-3 px-6 sm:px-8 py-3.5 sm:py-4 bg-indigo-600 border border-indigo-500 text-white rounded-2xl sm:rounded-[1.5rem] font-black text-xs sm:text-sm transition-all hover:bg-indigo-500 shadow-md active:scale-95">
                       <span>{portal?.text || 'الدخول للمنصة'}</span>
                       <div className="w-6 h-6 sm:w-8 sm:h-8 bg-white/20 text-white rounded-xl flex items-center justify-center group-hover/btn:-translate-x-1 transition-transform">
                         <PortalIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4 rotate-180" />
                       </div>
                    </Link>
                    {!user && (
                      <Link href="/about" className="inline-flex items-center gap-2 px-6 sm:px-8 py-3.5 sm:py-4 bg-white/5 border border-white/10 text-slate-300 rounded-2xl sm:rounded-[1.5rem] font-black text-xs sm:text-sm transition-all hover:bg-white/10 hover:text-white shadow-sm active:scale-95">
              ...
              {heroSlides && heroSlides.length > 1 && (
                <div className="absolute bottom-6 sm:bottom-8 right-6 sm:right-8 flex gap-2 z-30">
                  {heroSlides.map((_, i) => (
                    <button key={i} onClick={() => setCurrentSlide(i)} className={`h-1.5 sm:h-2 rounded-full transition-all duration-500 ${currentSlide === i ? 'w-6 sm:w-8 bg-indigo-400' : 'w-2 bg-white/30 hover:bg-white/50'}`} />
                  ))}
                </div>
              )}
           </motion.div>
           ...
           <div className="lg:col-span-4 xl:col-span-3 flex flex-col gap-5 sm:gap-6">
              <motion.div variants={itemVariants} className="bg-[#0f1423]/60 backdrop-blur-md p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border border-white/10 shadow-lg flex-1 relative overflow-hidden flex flex-col justify-center">
                 <div className="w-12 h-12 sm:w-14 sm:h-14 bg-blue-500/20 text-blue-300 border border-blue-500/40 rounded-[1rem] sm:rounded-2xl flex items-center justify-center shrink-0 mb-4 relative z-10"><Activity className="w-6 h-6 sm:w-7 sm:h-7"/></div>
                 <div className="relative z-10">
                   <div className="text-3xl sm:text-4xl font-black text-white leading-none mb-2">منصة حية</div>
              ...
              <motion.div variants={itemVariants} className="bg-[#0f1423]/60 backdrop-blur-md p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border border-white/10 shadow-lg flex-1 relative overflow-hidden flex flex-col justify-center">
                 <div className="w-12 h-12 sm:w-14 sm:h-14 bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 rounded-[1rem] sm:rounded-2xl flex items-center justify-center shrink-0 mb-4 relative z-10"><Target className="w-6 h-6 sm:w-7 sm:h-7"/></div>
                 <div className="relative z-10">
                   <div className="text-3xl sm:text-4xl font-black text-white leading-none mb-2">تقييم ذكي</div>
        ...
        {announcements && announcements.length > 0 && (
          <motion.div variants={itemVariants} className="bg-[#0f1423]/60 backdrop-blur-md p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border border-white/10 shadow-lg relative overflow-hidden">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-5 relative z-10 border-b border-white/10 pb-5 mb-5">
               <div className="flex items-center gap-3 sm:gap-4">
                 <div className="p-2 sm:p-3 bg-rose-500/20 border border-rose-500/40 rounded-xl sm:rounded-2xl shadow-inner"><Megaphone className="w-5 h-5 sm:w-6 sm:h-6 text-rose-300" /></div>
                 <h2 className="text-xl sm:text-2xl font-black text-white tracking-tight">إعلانات <span className="text-rose-400">الإدارة</span></h2>
               </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 relative z-10">
              {announcements.map((ann) => (
                <div key={ann.id} className="bg-[#02040a]/40 p-5 sm:p-6 rounded-2xl sm:rounded-[1.5rem] border border-white/5 hover:border-white/10 transition-colors shadow-inner flex flex-col justify-between group cursor-default">
                  <div>
                    ...
                    <h3 className="text-sm sm:text-base font-black text-white leading-relaxed group-hover:text-rose-200 transition-colors">{ann.title}</h3>
                  </div>
                </div>
              ))}
            </div>
        ...
        {studioItems && studioItems.length > 0 && (
          <motion.div variants={itemVariants} className="space-y-5 sm:space-y-6 pt-6 border-t border-white/10">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
               <div className="flex items-center gap-3 sm:gap-4">
                 <div className="w-12 h-12 sm:w-14 sm:h-14 bg-indigo-500/20 border border-indigo-500/40 shadow-inner rounded-xl sm:rounded-2xl flex items-center justify-center backdrop-blur-md"><Video className="w-6 h-6 text-indigo-300" /></div>
                 <div>
                   <h2 className="text-2xl sm:text-3xl font-black text-white tracking-tight">المكتبة البصرية</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
               {studioItems.slice(0, 3).map((media, index) => (
                 <div key={media.id} className={`relative rounded-[1.5rem] sm:rounded-[2rem] overflow-hidden group bg-white/5 p-1.5 cursor-pointer shadow-lg border border-white/10 ${index === 0 ? 'sm:col-span-2 lg:col-span-2 aspect-[16/9] sm:aspect-[21/9]' : 'aspect-[16/9] sm:aspect-auto'}`} onClick={() => setActiveMedia(media)}>
                   <div className="relative w-full h-full rounded-[1.25rem] sm:rounded-[1.75rem] overflow-hidden bg-[#02040a]">
                      <img src={media.media_type === 'video' ? media.thumbnail_url : media.media_url} alt={media.title} className="absolute inset-0 w-full h-full object-cover z-0 group-hover:scale-105 transition-transform duration-700 opacity-80 group-hover:opacity-100" />
                      ...
                      <div className="absolute top-4 right-4 sm:top-5 sm:right-5 z-20 px-3 py-1.5 rounded-lg bg-[#02040a]/80 backdrop-blur-md text-white text-[9px] sm:text-[10px] font-black flex items-center gap-1.5 border border-white/10 shadow-inner">
                      ...
                      <div className="absolute bottom-0 left-0 w-full p-5 sm:p-6 z-20">
                        <h3 className="text-sm sm:text-lg lg:text-xl font-black text-white leading-snug line-clamp-2">{media.title}</h3>
                      </div>
                   </div>
                 </div>
               ))}
        ...
        {magazineItems && magazineItems.length > 0 && (
          <motion.div variants={itemVariants} className="space-y-5 sm:space-y-6 pt-10 border-t border-white/10">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
              <div className="flex items-center gap-3 sm:gap-4">
                <div className="w-12 h-12 sm:w-14 sm:h-14 bg-emerald-500/20 border border-emerald-500/40 shadow-inner rounded-xl sm:rounded-2xl flex items-center justify-center backdrop-blur-md"><Newspaper className="w-6 h-6 text-emerald-300" /></div>
                <div>
                  <h2 className="text-2xl sm:text-3xl font-black text-white tracking-tight mb-1 sm:mb-2">المركز الإخباري</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6">
              {pinnedArticle && (
                <div onClick={() => setActiveArticle(pinnedArticle)} className="lg:col-span-8 group cursor-pointer relative min-h-[350px] sm:min-h-[450px] rounded-[1.5rem] sm:rounded-[2.5rem] overflow-hidden bg-white/5 p-1.5 flex flex-col shadow-lg border border-white/10">
                  <div className="relative w-full h-full rounded-[1.25rem] sm:rounded-[2.25rem] overflow-hidden bg-[#02040a]">
                     <img src={pinnedArticle.cover_image} alt={pinnedArticle.title} className="absolute inset-0 w-full h-full object-cover z-0 group-hover:scale-105 transition-transform duration-1000 opacity-80 group-hover:opacity-100" />
                     ...
                     <div className="relative z-20 p-6 sm:p-8 lg:p-10 flex flex-col justify-end h-full w-full">
                       <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                         ...
                         <span className="text-slate-300 text-[9px] sm:text-[10px] font-bold flex items-center gap-1.5 bg-[#02040a]/80 px-3 py-1.5 rounded-lg border border-white/10 shadow-inner backdrop-blur-sm"><User className="w-3 h-3 text-emerald-400 opacity-70" /> {pinnedArticle.author_name}</span>
                       </div>
                       <h3 className="text-xl sm:text-3xl lg:text-4xl font-black text-white leading-tight mb-3 sm:mb-4">{pinnedArticle.title}</h3>
                       <p className="text-slate-300 font-bold text-xs sm:text-sm lg:text-base max-w-2xl line-clamp-2 mb-5 sm:mb-6 leading-relaxed opacity-90">{pinnedArticle.excerpt}</p>
              ...
              {sideArticles && sideArticles.length > 0 && (
                <div className="lg:col-span-4 flex flex-col gap-4 sm:gap-6">
                  {sideArticles.map((article, idx) => (
                    <div onClick={() => setActiveArticle(article)} key={article.id} className="flex-1 group cursor-pointer relative rounded-[1.5rem] sm:rounded-[2rem] overflow-hidden bg-white/5 p-1.5 min-h-[160px] sm:min-h-[200px] flex flex-col shadow-md border border-white/10">
                       <div className="relative w-full h-full rounded-[1.25rem] sm:rounded-[1.75rem] overflow-hidden bg-[#02040a]">
                          <img src={article.cover_image} alt={article.title} className="absolute inset-0 w-full h-full object-cover z-0 group-hover:scale-105 transition-transform duration-700 opacity-80 group-hover:opacity-100" />
                          ...
                          <div className="relative z-20 p-5 sm:p-6 flex flex-col justify-end h-full">
                            <span className="text-emerald-300 text-[9px] sm:text-[10px] font-black mb-2 flex items-center gap-1.5 bg-[#02040a]/80 w-fit px-2 py-1 rounded-md border border-white/10 backdrop-blur-sm"><User className="w-3 h-3 opacity-70" /> {article.author_name}</span>
                            <h3 className="text-sm sm:text-base lg:text-lg font-black text-white leading-snug line-clamp-3">{article.title}</h3>
                          </div>
                       </div>
                    </div>
                  ))}
        ...
        {(studentMemorials.length > 0 || teacherMemorials.length > 0) && (
          <motion.div variants={itemVariants} className="py-10 sm:py-16 relative z-10 border-t border-white/10 bg-transparent overflow-hidden mt-10">
             <div className="flex flex-col items-center text-center mb-8 sm:mb-10">
                ...
                <h2 className="text-2xl sm:text-4xl lg:text-5xl font-black text-white tracking-tight leading-tight">معرض التتويج <span className="text-transparent bg-clip-text bg-gradient-to-l from-amber-300 to-amber-500">والتميز</span></h2>
             </div>
             ...
             <div className="w-full overflow-hidden relative">
                <div className="flex overflow-x-auto gap-4 sm:gap-6 px-4 sm:px-[10vw] pb-10 pt-4 snap-x snap-mandatory custom-scrollbar min-h-[350px]">
                   {displayedMemorials && displayedMemorials.length > 0 ? displayedMemorials.map((memorial, i) => {
                      ...
                      return (
                        <div key={memorial?.id || i} className="snap-center shrink-0 w-[220px] sm:w-[260px] group cursor-pointer relative z-0 hover:z-20">
                           {isExternal ? (
                              ...
                              <div className={cn("relative p-[2px] rounded-t-full rounded-b-[2.5rem] shadow-2xl overflow-hidden bg-gradient-to-br transition-all duration-500 group-hover:-translate-y-4", theme.border)}>
                                 ...
                                 <div className="relative h-full w-full bg-[#02040a]/90 backdrop-blur-2xl border border-white/10 rounded-t-full rounded-b-[2.4rem] p-5 sm:p-6 flex flex-col items-center text-center overflow-hidden z-10 shadow-inner">
                                    <div className="mt-4 mb-4 p-2.5 sm:p-3 rounded-full bg-white/10 border border-white/20 shadow-inner flex items-center justify-center backdrop-blur-md">
                                       {memorial?.custom_logo_url ? <img src={memorial.custom_logo_url} crossOrigin="anonymous" className="w-8 h-8 sm:w-10 sm:h-10 object-contain" /> : theme.icon}
                                    </div>
                                    ...
                                    <h2 className={cn("text-base sm:text-lg font-black leading-tight mb-4 min-h-[40px] flex items-center justify-center", theme.textPrimary)}>{memorial?.title}</h2>
                                    ...
                                    <p className={cn("text-sm sm:text-base font-black truncate w-full", theme.textPrimary)}>{memorial?.personName}</p>
                      ...
                      <div className="w-full text-center py-16 flex flex-col items-center justify-center bg-white/5 rounded-3xl border border-white/10 shadow-inner backdrop-blur-md">
                         <Award className="w-10 h-10 sm:w-12 sm:h-12 text-slate-500 mb-4 opacity-60" />
        ...
        <motion.div variants={itemVariants} className="py-20 sm:py-32 relative z-10 text-center border-t border-white/10 mt-10">
           <div className="w-16 h-16 sm:w-24 sm:h-24 bg-indigo-500/20 border border-indigo-500/40 rounded-[1.5rem] sm:rounded-[2rem] flex items-center justify-center mx-auto mb-6 sm:mb-8 shadow-lg backdrop-blur-md">
             <Compass className="w-8 h-8 sm:w-12 sm:h-12 text-indigo-300" />
           </div>
           <h2 className="text-3xl sm:text-5xl md:text-7xl font-black text-white mb-4 sm:mb-6 tracking-tight">جاهز للانطلاق؟</h2>
           <p className="text-slate-300 font-bold text-xs sm:text-lg md:text-xl mb-8 sm:mb-12 max-w-2xl mx-auto leading-relaxed px-4">انضم الآن إلى مجتمع مدرستك في منصة التعليم الرقمي الأقوى والأكثر تطوراً على مستوى البلاد.</p>
           <Link href={portal?.href || '/login'} className="inline-flex items-center justify-center gap-2 sm:gap-3 px-8 py-3.5 sm:px-12 sm:py-5 bg-indigo-600 border border-indigo-500 text-white rounded-2xl sm:rounded-[1.5rem] font-black text-xs sm:text-lg hover:bg-indigo-500 transition-all active:scale-95 shadow-lg">
      ...
      <AnimatePresence>
        {activeMedia && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-[#02040a]/90 backdrop-blur-xl p-2 sm:p-10" onClick={() => setActiveMedia(null)}>
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} transition={{ type: "spring", damping: 25 }} className="w-full max-w-6xl bg-[#0f1423] rounded-[2rem] sm:rounded-[3rem] overflow-hidden shadow-2xl border border-white/20 relative flex flex-col" onClick={e => e.stopPropagation()}>
              ...
              <div className="p-5 sm:p-6 bg-[#0f1423] border-t border-white/10 relative z-10"><h3 className="text-white font-black text-base sm:text-2xl text-center sm:text-right">{activeMedia?.title}</h3></div>
            </motion.div>
          </motion.div>
        )}

        {activeArticle && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-[#02040a]/90 backdrop-blur-xl p-2 sm:p-6" onClick={() => setActiveArticle(null)}>
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} transition={{ type: "spring", damping: 25 }} className="w-full max-w-4xl bg-[#0f1423] rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/20 relative flex flex-col max-h-[95vh] sm:max-h-[90vh]" onClick={e => e.stopPropagation()}>
              ...
              <div className="relative h-48 sm:h-[350px] shrink-0 bg-[#02040a] rounded-t-[2rem] sm:rounded-t-[2.5rem] overflow-hidden m-1.5 sm:m-2">
                ...
                <img src={activeArticle?.cover_image} alt={activeArticle?.title} className="absolute inset-0 w-full h-full object-cover rounded-[1.25rem] sm:rounded-[2rem] opacity-80" />
              ...
              <div className="px-5 sm:px-10 pb-10 sm:pb-12 pt-4 sm:pt-6 overflow-y-auto custom-scrollbar relative z-10 bg-transparent flex-1">
                <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-5 sm:mb-8 border-b border-white/10 pb-4 sm:pb-5">
                  <span className="text-slate-300 bg-white/10 px-3 py-1.5 rounded-lg border border-white/20 shadow-inner flex items-center gap-1.5 text-[10px] sm:text-xs font-black"><Calendar className="w-3.5 h-3.5 opacity-80" /> {activeArticle?.created_at ? new Date(activeArticle.created_at).toLocaleDateString('ar-SA') : ''}</span>
                  <span className="text-emerald-300 bg-emerald-500/20 px-3 py-1.5 rounded-lg border border-emerald-500/40 shadow-inner flex items-center gap-1.5 text-[10px] sm:text-xs font-black"><User className="w-3.5 h-3.5 opacity-80" /> {activeArticle?.author_name}</span>
                </div>
                <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black text-white mb-6 sm:mb-8 leading-tight sm:leading-tight tracking-tight">{activeArticle?.title}</h2>
                <div className="prose prose-invert prose-sm sm:prose-base max-w-none text-slate-300 font-bold leading-relaxed">
                  <div className="text-sm sm:text-base lg:text-lg text-indigo-200 font-black mb-6 sm:mb-8 p-5 sm:p-6 bg-indigo-500/20 rounded-2xl sm:rounded-3xl border border-indigo-500/30 border-r-4 border-r-indigo-400 shadow-inner leading-relaxed">{activeArticle?.excerpt}</div>
                </div>
              </div>
            </motion.div>
          </motion.div>
