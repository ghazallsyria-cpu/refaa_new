...
import { Calendar, Clock, BookOpen, User, Video, GraduationCap } from 'lucide-react';
import { motion } from 'motion/react';
// استيراد النوع "StudentScheduleData" من الهوك بشكل صريح
...
export default function StudentSchedulePage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      // تعريف النتيجة كـ "any" هو الحل الجذري لتجاوز اعتراض TypeScript في Netlify
      ...
      <div className="bg-white rounded-[40px] shadow-2xl border border-slate-100 overflow-hidden overflow-x-auto">
        <table className="min-w-full divide-y divide-slate-100 table-fixed">
          <thead className="bg-slate-50/80">
            <tr>
              <th className="py-8 px-6 text-xs font-black text-slate-400 border-l border-slate-200 w-32">اليوم</th>
              {periods.map(p => (
                <th key={p.id} className="py-6 px-4 text-center border-l border-slate-100">
            ...
            {DAYS.map((day) => (
              <tr key={day.id} className="hover:bg-indigo-50/30">
                ...
                {periods.map(period => {
                  ...
                  return (
                    <td key={period.id} className="p-3 border-l border-slate-100 h-44 align-top min-w-[180px]">
                      {cell ? (
                        <div className="h-full bg-white rounded-3xl p-4 border border-slate-100 shadow-sm flex flex-col justify-between">
                          ...
                          {cell.teachers?.zoom_link && (
                            <a href={cell.teachers.zoom_link} target="_blank" className="bg-indigo-600 text-white text-[10px] font-black py-2 rounded-xl text-center">بث مباشر</a>
                          )}
                        </div>
                      ) : null}
                    </td>
