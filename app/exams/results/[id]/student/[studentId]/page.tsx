...
export default function StudentExamResult() {
  ...
  const handleSaveGrade = async (questionId: string) => {
    ...
    try {
      if(gradeAnswer) {
          ...
          await fetchData(); // إعادة تحميل البيانات لتحديث النتيجة النهائية
  ...
  // ✅ التحقق الذكي من حالة الاختبار (هل يحتاج تصحيح يدوي؟)
  const isPendingGrading = attempt?.status === 'submitted' || attempt?.status === 'pending';
  
  const totalEarned = attempt?.score !== undefined ? attempt.score : answers.reduce((sum, ans) => sum + (ans?.points_earned || 0), 0);
  ...
  const renderStudentAnswer = (answer: any) => {
    ...
    if (question.type === 'multi_select') {
      try {
        ...
        return question.options?.filter((o: any) => selectedIds.includes(o.id)).map((o: any) => o.content).join('، ') || 'لم يتم الإجابة';
      } catch { return answer.text_answer || 'لم يتم الإجابة'; }
    }
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-8 space-y-8 pb-24" dir="rtl">
      ...
      {/* ✅ لافتة التنبيه بوجود أسئلة بانتظار التصحيح (تظهر للطالب والمعلم) */}
      {isPendingGrading && (
        <div className="bg-amber-50 border-2 border-amber-200 p-6 rounded-3xl flex items-center gap-4 animate-in fade-in slide-in-from-top-4 shadow-sm">
           <div className="bg-amber-200/50 p-3 rounded-2xl text-amber-600 shrink-0"><Clock className="w-8 h-8" /></div>
           <div>
              <h3 className="text-xl font-black text-amber-800 mb-1">الاختبار قيد المراجعة</h3>
              <p className="text-amber-700 font-bold text-sm leading-relaxed">
                {currentRole === 'teacher' 
                  ? 'هناك أسئلة مقالية في هذا الاختبار تتطلب إدخال الدرجة يدوياً من قبلك (في الأسفل) لتكتمل النتيجة ويراها الطالب.' 
                  : 'يحتوي هذا الاختبار على أسئلة مقالية أو نصوص. ستظهر نتيجتك النهائية بعد أن يقوم المعلم بمراجعتها وتصحيحها.'}
              </p>
           </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="relative z-10">
            ...
            <div className="flex flex-wrap items-center gap-4 mt-6 text-slate-600 font-bold">
              ...
              <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border ${isPendingGrading ? 'bg-amber-50 text-amber-700 border-amber-100' : 'bg-emerald-50 text-emerald-700 border-emerald-100'}`}>
                ...
                <span>{isPendingGrading ? 'قيد التقييم والتصحيح' : 'مكتمل ومقيّم'}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className={`p-8 rounded-3xl shadow-xl flex flex-col items-center justify-center text-white relative overflow-hidden transition-all duration-500 ${isPendingGrading ? 'bg-gradient-to-br from-amber-500 to-orange-500 shadow-amber-200' : 'bg-gradient-to-br from-indigo-600 to-purple-700 shadow-indigo-200'}`}>
          ...
          <div className="text-5xl font-black tracking-tighter relative z-10 flex items-baseline gap-2">
            {/* ✅ إخفاء العلامة تماماً ووضع كلمة (قيد المراجعة) إذا كان الاختبار لم يُصحح بعد */}
            {isPendingGrading ? (
               <span className="text-3xl text-white">قيد المراجعة</span>
            ) : (
      ...
      <div className="space-y-6 mt-8">
        ...
        {answers && answers.length > 0 ? answers.map((answer, index) => {
          const question = answer?.question;
          const isManual = question?.type === 'essay' || question?.type === 'fill_in_blank' || question?.type === 'open' || question?.type === 'paragraph';
          
          // السؤال يعتبر "مصحح" إذا لم يكن مقالياً، أو إذا تم وضع درجة له أكبر من 0 (ويمكن للمعلم تعديلها لاحقاً)
          // في حال أخذ الطالب صفر بالسؤال المقالي، سيعتبر مصححاً لأن المعلم سيضغط "حفظ الدرجة" وتتحول حالة الاختبار إلى Graded.
              ...
              <div className="p-6 sm:p-8">
                <div className="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
                  ...
                  <div className="flex items-center gap-1.5 bg-slate-50 px-4 py-1.5 rounded-xl font-black text-sm text-slate-600 border border-slate-100">
                    {/* ✅ إخفاء درجة السؤال للطالب إذا كان مقالياً والاختبار لم يُقيّم بعد */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {/* قسم الإجابة النموذجية أو تصحيح المعلم */}
                  {(!isCorrect || isManual) && (
                    <div className="p-5 rounded-2xl border bg-slate-50 border-slate-200">
                      ...
                      {isManual && currentRole === 'teacher' && question && gradingState[question.id] && (
                        <div className="mt-2 bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center gap-4">
                          ...
                          <button 
                             ...
                             className="flex justify-center items-center gap-2 bg-indigo-600 text-white px-5 py-2.5 rounded-lg font-bold hover:bg-indigo-700 disabled:opacity-50 transition-all"
                          >
                      ...
                      {/* ✅ رسالة للطالب بدلاً من واجهة التصحيح */}
                      {isManual && currentRole === 'student' && (
                        <p className="text-base font-bold text-amber-600 mt-2 flex items-center gap-2 bg-amber-50 p-3 rounded-xl border border-amber-100">
                           <Clock className="w-5 h-5"/> بانتظار تصحيح المعلم لوضع الدرجة.
                        </p>
                      )}
                    </div>
                  )}
          ...
          <div className="text-center py-20 bg-white rounded-3xl border border-slate-100 shadow-sm">
            ...
            <p className="text-slate-500 font-medium max-w-md mx-auto leading-relaxed">
              لم يتم العثور على إجابات لهذا الطالب. <br/> يرجى التأكد من أن الطالب قام بتسليم الاختبار بنجاح.
            </p>
          </div>
        )}
      </div>
