        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
                ...
                <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-white rounded-[40px] p-10 shadow-2xl z-50 animate-in fade-in zoom-in-95 duration-300 border border-white/20">
                  <div className="flex items-center justify-between mb-10">
                    <div>
                      ...
                      <p className="text-slate-500 font-bold">تخصيص تجربة الاختبار</p>
                    </div>
                      ...
                      <div className="space-y-4">
                        {[
                          ...
                          <div key={setting.key} className="flex items-center justify-between p-4 rounded-3xl bg-slate-50/50 border border-slate-100">
                            ...
                            >
                              <Switch.Thumb className="block w-6 h-6 bg-white rounded-full shadow-xl transition-transform duration-200 translate-x-1 data-[state=checked]:translate-x-[26px]" />
                            </Switch.Root>
                          </div>
                        ))}
                      </div>
            ...
            >
              ...
              <span>{exam.status === 'published' ? 'حفظ ونشر' : 'حفظ كمسودة'}</span>
            </button>
          </div>
        </div>
      </header>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pt-10 border-t border-slate-100">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                <input 
                  ...
                  className="w-full px-3 py-4 rounded-2xl bg-slate-50 border-0 ring-1 ring-inset ring-slate-100 focus:ring-2 focus:ring-indigo-600 outline-none font-bold text-slate-700 transition-all appearance-none cursor-pointer"
                />
            ...
            <div className="space-y-3 lg:col-span-2">
              ...
              <div className="flex flex-wrap gap-2">
                <label className={`flex items-center gap-2 px-4 py-2 rounded-xl cursor-pointer transition-colors border ${exam.section_ids.length === 0 ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                  <input 
                    type="checkbox"
                ...
                {sections.map(s => (
                  <label key={s.id} className={`flex items-center gap-2 px-4 py-2 rounded-xl cursor-pointer transition-colors border ${exam.section_ids.includes(s.id) ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                    <input 
                      type="checkbox"
