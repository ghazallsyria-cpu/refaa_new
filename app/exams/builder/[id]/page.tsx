...
const QuestionCard = memo(({ 
  ...
  <Reorder.Item 
    value={q} 
    className="glass-card rounded-[40px] border border-white/60 shadow-2xl group relative overflow-hidden mb-10 p-10 space-y-10"
  >
    <div className="absolute top-0 left-1/2 -translate-x-1/2 p-2 opacity-0 group-hover:opacity-100 transition-opacity cursor-grab active:cursor-grabbing">
    ...
    <div className="flex flex-col md:flex-row gap-10">
      <div className="flex-1 space-y-6">
        <div className="space-y-3">
          ...
          <textarea 
            ...
            className="w-full bg-slate-50/50 px-6 py-5 rounded-3xl border-0 ring-1 ring-slate-100 focus:ring-2 focus:ring-indigo-600 text-xl font-black transition-all outline-none resize-none" 
      ...
      <div className="w-full md:w-64 space-y-3">
        ...
        <select 
          ...
          className="w-full px-6 py-5 rounded-3xl bg-white border-0 ring-1 ring-slate-200 focus:ring-2 focus:ring-indigo-600 font-black appearance-none cursor-pointer shadow-sm"
        >
      ...
      <div className="space-y-4">
        ...
        {q.options?.map((opt: any, optIdx: number) => (
          <div key={opt.id} className="flex items-center gap-5 p-4 rounded-3xl bg-slate-50/50 border border-slate-100 group/opt hover:bg-white transition-all">
            ...
            <button 
              ...
              className="h-10 w-10 opacity-0 group-hover/opt:opacity-100 text-slate-400 hover:text-red-600 transition-all flex items-center justify-center"
            >
              <X size={18} />
            </button>
          </div>
        ...
        {q.type !== 'true_false' && (
          <button 
            ...
            className="flex items-center gap-3 px-6 py-4 rounded-2xl border-2 border-dashed border-slate-200 text-slate-500 hover:border-indigo-600 hover:bg-white transition-all font-black text-sm"
          >
    ...
    <div className="flex items-center justify-between pt-10 border-t border-slate-100">
      ...
      <div className="flex gap-2">
        <button type="button" onClick={() => duplicateQuestion(q.id)} className="h-12 w-12 flex items-center justify-center rounded-2xl bg-slate-50 text-slate-400 hover:text-indigo-600 transition-all"><Copy size={20} /></button>
        <button type="button" onClick={() => deleteQuestion(q.id)} className="h-12 w-12 flex items-center justify-center rounded-2xl bg-red-50 text-red-400 hover:text-red-600 transition-all"><Trash2 size={20} /></button>
      </div>
    </div>
  </Reorder.Item>
));
...
export default function QuizBuilder() {
  ...
  const [exam, setExam] = useState<any>({ 
    ...
    subject_id: '', 
    section_ids: [], 
    settings: { shuffle_questions: false, shuffle_options: false, show_results: true, allow_backtrack: true, browser_lock: false } 
  });
  ...
  useEffect(() => {
    if (params.id !== 'new') {
      fetchExamDetails(params.id as string).then(res => { 
        setExam((prev: any) => ({ ...prev, ...res.exam, settings: res.exam.settings || prev.settings })); 
  ...
  const handleSave = async () => {
    if (!exam.title || !exam.subject_id || !exam.section_ids || exam.section_ids.length === 0) {
      return alert('يرجى التأكد من إدخال العنوان والمادة واختيار صف واحد على الأقل');
    }
    setSaving(true);
    try { 
      ...
      alert('خطأ في عملية الحفظ، يرجى المحاولة لاحقاً'); 
    } finally { 
      setSaving(false); 
    }
  };
    ...
    <div className="min-h-screen bg-slate-50/50 pb-24" dir="rtl">
      <header className="sticky top-0 z-40 glass-card border-b border-white/60 px-6 py-4 shadow-xl flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button type="button" onClick={() => router.back()} className="h-12 w-12 flex items-center justify-center rounded-2xl bg-white border border-slate-100 active:scale-95 shadow-sm text-slate-500 hover:text-indigo-600 transition-all"><ArrowRight size={20} /></button>
          <h1 className="text-xl font-black truncate max-w-[200px] md:max-w-[400px] text-slate-800">{exam.title || 'اختبار جديد'}</h1>
        </div>
        <div className="flex items-center gap-3">
          <Dialog.Root>
            <Dialog.Trigger asChild>
              <button className="h-12 px-5 flex items-center gap-3 rounded-2xl bg-white border border-slate-200 text-slate-600 font-black hover:text-indigo-600 transition-all active:scale-95 shadow-sm">
            ...
            <Dialog.Portal>
              ...
              <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90vw] max-w-md bg-white rounded-[40px] p-10 z-[101] shadow-2xl focus:outline-none">
                <Dialog.Title className="text-2xl font-black mb-8 flex items-center gap-3">
                   <SettingsIcon className="text-indigo-600" /> إعدادات الاختبار
                </Dialog.Title>
                <div className="space-y-6">
                  {[
                    ...
                    { key: 'show_results', label: 'إظهار النتيجة فوراً', icon: Eye },
                    { key: 'allow_backtrack', label: 'السماح بالعودة للخلف', icon: ArrowRight },
                    { key: 'browser_lock', label: 'حماية الغش (منع التبويب)', icon: ShieldCheck }
                  ].map((s) => (
                    <div key={s.key} className="flex items-center justify-between p-4 bg-slate-50 rounded-3xl border border-slate-100 transition-all hover:bg-white hover:shadow-sm">
                      <div className="flex items-center gap-3 text-slate-700 font-bold"><s.icon size={18} className="text-slate-400" /><span>{s.label}</span></div>
                      <Switch.Root checked={exam.settings?.[s.key]} onCheckedChange={(v) => updateSetting(s.key, v)} className={`w-12 h-7 rounded-full relative transition-colors shadow-inner ${exam.settings?.[s.key] ? 'bg-indigo-600' : 'bg-slate-200'}`}><Switch.Thumb className={`block w-5 h-5 bg-white rounded-full transition-all shadow-md ${exam.settings?.[s.key] ? 'translate-x-6' : 'translate-x-1'}`} /></Switch.Root>
                    </div>
                  ))}
                </div>
                <Dialog.Close asChild><button className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black mt-8 hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100">تم الحفظ</button></Dialog.Close>
              </Dialog.Content>
            </Dialog.Portal>
          </Dialog.Root>
      ...
      <main className="max-w-4xl mx-auto px-6 py-12 space-y-10">
        <div className="glass-card rounded-[40px] border-t-[16px] border-t-indigo-600 p-10 space-y-10 shadow-2xl bg-white transition-all hover:shadow-indigo-50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-t border-slate-100 pt-10">
            <div className="space-y-3">
              <label className="text-xs font-black uppercase text-slate-400 tracking-widest flex items-center gap-2"><BookOpen size={14} /> المادة الدراسية</label>
            ...
            <div className="space-y-3">
              <label className="text-xs font-black uppercase text-slate-400 tracking-widest flex items-center gap-2"><Target size={14} /> درجة النجاح (%)</label>
              <input type="number" value={exam.passing_score} onChange={(e) => setExam({ ...exam, passing_score: parseInt(e.target.value) })} className="w-full p-5 rounded-3xl bg-slate-50 border-0 ring-1 ring-slate-100 font-bold focus:ring-2 focus:ring-indigo-600 transition-all" />
            </div>
          </div>

          <div className="space-y-4 border-t border-slate-100 pt-10">
            <label className="text-xs font-black uppercase text-slate-400 tracking-widest flex items-center gap-2"><Users size={14} /> الصفوف المستهدفة</label>
              ...
              {formData?.sections?.map((s: any) => {
                ...
                return (
                  <button 
                    key={s.id} 
                    type="button" 
                    onClick={() => setExam({...exam, section_ids: active ? exam.section_ids.filter((id: string) => id !== s.id) : [...(exam.section_ids || []), s.id]})} 
                    className={`px-6 py-3 rounded-2xl text-sm font-black transition-all border-2 ${active ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg shadow-indigo-100' : 'bg-white border-slate-100 text-slate-500 hover:border-indigo-400 hover:text-indigo-600'}`}
                  >
                    {s.name}
                  </button>
                )
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 border-t border-slate-100 pt-10">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <input type="time" value={exam.start_time} onChange={(e) => setExam({...exam, start_time: e.target.value})} className="w-1/2 p-4 rounded-2xl bg-slate-50 border-0 ring-1 ring-slate-100 font-bold text-center" />
                <input type="time" value={exam.end_time} onChange={(e) => setExam({...exam, end_time: e.target.value})} className="w-1/2 p-4 rounded-2xl bg-slate-50 border-0 ring-1 ring-slate-100 font-bold text-center" />
              </div>
            </div>
          </div>
        </div>
        ...
        <button 
          type="button"
          onClick={() => setQuestions([...questions, createNewQuestion('multiple_choice')])} 
