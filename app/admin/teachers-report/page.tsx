...
export default function TeachersReportPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const results: TeacherReport[] = data.map((item: any) => {
        ...
        // حساب إجمالي الحصص المطلوبة
        let total = 0;
        if (reportType === "day") {
            total = scheduleData?.filter((s: any) => s.day_of_week === dbDay).length || 0;
        } else {
            // للتبسيط في الأسبوعي نعتمد إجمالي الجدول المسجل (افتراض أنه يمثل أسبوعاً دراسياً)
            total = scheduleData?.length || 0;
        }

        // حساب الحصص التي تم رصدها (Deduplication)
        let recorded = 0;
        if (attendanceData && attendanceData.length > 0) {
            // نستخدم البصمة المركبة: التاريخ + الفصل + الحصة
            const uniqueSlots = new Set(attendanceData.map((a: any) => `${a.date}-${a.section_id}-${a.period}`));
            recorded = uniqueSlots.size;
        }
        
        // التأكد من أن الرصد لا يتجاوز الجدول منطقياً
        ...
        const lastRecorded = attendanceData && attendanceData.length > 0
          ? [...attendanceData].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())[0].created_at
          : null;
      ...
      // ترتيب المعلمين للطباعة بشكل منسق
  ...
  return (
    <>
      ...
      {/* 🚀 واجهة المستخدم الويب (تختفي عند الطباعة) */}
      ...
      {/* ========================================== */}
      {/* 🚀 PRINTABLE PDF VIEW (تظهر فقط عند الطباعة) */}
      {/* ========================================== */}
