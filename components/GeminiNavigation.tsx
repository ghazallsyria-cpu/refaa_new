...
export default function GeminiNavigation() {
  ...
  const isItemVisible = (item: any) => {
    ...
    // 2. 👑 روابط حصرية للمدير والإدارة العليا فقط (ممنوع دخول المعلم أو الطالب)
    const adminOnly = [
    ...
    // 5. روابط عامة ومفتوحة للطالب (والمعلم كمنشئ)
    ...
    // 6. 👨‍🎓 روابط مخصصة للطالب فقط
    if (['ساحة التدريب', 'سجل الأداء'].includes(n)) return (r === 'student' || r === 'parent');

    // الإغلاق الأمني: أي رابط غير مذكور يتم إخفاؤه
    return false;
  };
