...
export default function CampusControlPage() {
  ...
  const handleFileUpload = async (file: File): Promise<{ url: string, type: 'image' | 'video', thumb?: string } | null> => {
    ...
    try {
      ...
      formData.append('upload_preset', 'ml_default'); // التجاوز الإجباري
  ...
  const handleDeleteStudio = async (item: any) => {
    if (!confirm('سيتم حذف الملف نهائياً من السحابة. هل أنت متأكد؟')) return;
    try {
      ...
      showToast('تم حذف الوسيط بنجاح', 'success');
    } catch (error) {
  ...
  const handleDeleteMagazine = async (item: any) => {
    if (!confirm('سيتم حذف المقال وغلافه نهائياً. هل أنت متأكد؟')) return;
    try {
  ...
  const onStudioFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (uploaded) {
      setStudioForm(prev => ({ ...prev, media_url: uploaded.url, media_type: uploaded.type, thumbnail_url: uploaded.thumb || uploaded.url }));
      showToast('تم تجهيز الملف بنجاح', 'success');
    }
  };
  const handleSaveStudio = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!studioForm.media_url) return showToast('يرجى رفع ملف أولاً', 'error');
    setIsSubmitting(true);
    try {
      const finalTitle = studioForm.title.trim() || `لقطة جديدة - ${new Date().toLocaleDateString('ar-SA')}`;
      await supabase.from('school_studio').insert([{ ...studioForm, title: finalTitle }]);
      setIsStudioModalOpen(false);
      setStudioForm({ title: '', media_type: 'image', media_url: '', thumbnail_url: '' });
      fetchData();
      showToast('تم نشر الميديا بنجاح', 'success');
  ...
  const onMagazineFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (file.type.startsWith('video/') || file.name.toLowerCase().endsWith('.mov')) return showToast('الغلاف يجب أن يكون صورة فقط', 'error');
    const uploaded = await handleFileUpload(file);
    if (uploaded) {
  ...
  const handleSaveMagazine = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!magazineForm.cover_image) return showToast('يرجى رفع غلاف للمقال', 'error');
    setIsSubmitting(true);
    try {
      const finalTitle = magazineForm.title.trim() || `خبر عاجل - ${new Date().toLocaleDateString('ar-SA')}`;
      await supabase.from('school_magazine').insert([{ 
        ...magazineForm, title: finalTitle, excerpt: magazineForm.excerpt || 'لا توجد تفاصيل إضافية.', author_name: magazineForm.author_name || 'إدارة المدرسة'
      }]);
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 relative z-10">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-slate-400 font-bold text-sm sm:text-base">تحكم كامل في هوية ومحتوى الحرم الرقمي</p>
              </div>
            </div>

            {/* 🎛️ التبويبات الأنيقة (Apple Style) */}
            <div className="flex bg-black/40 backdrop-blur-md p-1.5 rounded-[1.5rem] border border-white/5 relative shadow-inner">
              {['studio', 'magazine'].map((tab) => (
                <button key={tab} onClick={() => setActiveTab(tab as any)} className="relative px-8 py-3.5 rounded-xl text-sm font-black transition-colors w-1/2 md:w-auto z-10">
                  {activeTab === tab && <motion.div layoutId="activeTab" className="absolute inset-0 bg-white/10 rounded-xl shadow-lg border border-white/10" />}
                  <span className={`relative flex items-center justify-center gap-2 ${activeTab === tab ? 'text-white' : 'text-slate-500 hover:text-slate-300'}`}>
                    {tab === 'studio' ? <Video className="w-4 h-4" /> : <Newspaper className="w-4 h-4" />}
                    {tab === 'studio' ? 'الاستوديو' : 'المجلة'}
                  </span>
                </button>
              ))}
            </div>
              ...
              <motion.div key="studio" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-8">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-slate-500 text-xs font-bold mt-1">الصور والفيديوهات المعروضة في الرئيسية</p>
                  </div>
                  <button onClick={() => setIsStudioModalOpen(true)} className="bg-white hover:bg-slate-200 text-[#02040a] px-6 py-3.5 rounded-2xl font-black text-sm flex items-center gap-2 transition-all active:scale-95 shadow-xl">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                  {studioItems.length === 0 ? (
                    <div className="col-span-full py-32 text-center rounded-[3rem] border border-white/5 bg-[#0f1423]/30">
                      <UploadCloud className="w-16 h-16 text-slate-700 mx-auto mb-4" />
                      <p className="text-slate-400 font-black text-lg">المكتبة فارغة.</p>
                    </div>
                  ) : (
                      ...
                      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} key={item.id} className="group relative aspect-[4/5] rounded-[2rem] overflow-hidden bg-[#0f1423] border border-white/5 shadow-lg">
                        ...
                        <div className="absolute top-4 right-4 flex gap-2">
                          <span className="bg-black/50 backdrop-blur-md px-3 py-1.5 rounded-xl border border-white/10 text-[10px] font-black text-white shadow-sm flex items-center gap-1.5">
                            {item.media_type === 'video' ? <Video className="w-3.5 h-3.5 text-amber-400" /> : <ImageIcon className="w-3.5 h-3.5 text-indigo-400" />}
                            {item.media_type === 'video' ? 'فيديو' : 'صورة'}
                          </span>
                        </div>

                        <button onClick={() => handleDeleteStudio(item)} className="absolute top-4 left-4 w-9 h-9 bg-rose-500/80 hover:bg-rose-500 text-white rounded-xl flex items-center justify-center backdrop-blur-md transition-all border border-white/10 active:scale-95 shadow-xl">
              ...
              <motion.div key="magazine" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-8">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-slate-500 text-xs font-bold mt-1">إدارة الأخبار والمقالات الأكاديمية</p>
                  </div>
                  <button onClick={() => setIsMagazineModalOpen(true)} className="bg-emerald-500 hover:bg-emerald-400 text-[#02040a] px-6 py-3.5 rounded-2xl font-black text-sm flex items-center gap-2 transition-all active:scale-95 shadow-[0_0_20px_rgba(16,185,129,0.3)]">
                    <Plus className="w-5 h-5" /> صياغة خبر
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {magazineItems.length === 0 ? (
                    <div className="col-span-full py-32 text-center rounded-[3rem] border border-white/5 bg-[#0f1423]/30">
                      ...
                      <p className="text-slate-400 font-black text-lg">لا توجد أخبار منشورة.</p>
                    </div>
                  ) : (
                    magazineItems.map((article) => (
                      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} key={article.id} className="group p-4 rounded-[2rem] bg-[#0f1423] border border-white/5 hover:border-white/10 transition-colors flex gap-6 shadow-lg">
                        ...
                        <div className="flex-1 min-w-0 py-2 flex flex-col justify-between">
                          ...
                          <div className="flex items-center justify-between mt-4">
                            ...
                            <button onClick={() => handleDeleteMagazine(article)} className="text-rose-500 p-2 hover:bg-rose-500/20 rounded-xl transition-colors">
                              <Trash2 className="w-4 h-4" />
                            </button>
                          </div>
                        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#02040a]/90 backdrop-blur-2xl">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#0f1423] w-full max-w-xl rounded-[3rem] border border-white/10 overflow-hidden shadow-2xl relative">
              ...
              <div className="p-8 pb-4 flex justify-between items-center">
                ...
                <button onClick={() => !isUploading && setIsStudioModalOpen(false)} className="bg-white/5 hover:bg-white/10 p-2 rounded-full transition-colors"><XCircle className="w-6 h-6 text-slate-400" /></button>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#02040a]/90 backdrop-blur-2xl overflow-y-auto">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#0f1423] w-full max-w-2xl rounded-[3rem] border border-white/10 overflow-hidden shadow-2xl relative my-auto">
              ...
              <div className="p-8 pb-4 flex justify-between items-center">
                ...
                <button onClick={() => !isUploading && setIsMagazineModalOpen(false)} className="bg-white/5 hover:bg-white/10 p-2 rounded-full transition-colors"><XCircle className="w-6 h-6 text-slate-400" /></button>
              </div>
