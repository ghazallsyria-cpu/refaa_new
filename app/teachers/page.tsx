...
import { 
  ...
  Users, Crown, Folder, ChevronLeft, ChevronRight, 
  Award, CheckCircle2, LayoutGrid, List, Loader2 
} from 'lucide-react';
    ...
    <tr className="hover:bg-slate-50/80 transition-colors group border-b border-slate-50">
      ...
      <td className="whitespace-nowrap py-4 pl-8 pr-4 text-left">
        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <button onClick={() => onResetPassword(teacher)} className="p-2 text-slate-400 hover:text-indigo-600 bg-white hover:bg-indigo-50 rounded-lg shadow-sm border border-slate-200 transition-all" title="تغيير كلمة المرور"><Key className="w-4 h-4" /></button>
...
export default function TeachersPage() {
  ...
  // 🚀 حالات العمليات (Submitting States)
  ...
  useEffect(() => {
    const initPage = async () => {
      ...
      if (data) {
        setStats({ 
          total: data.length, 
          hods: data.filter((t: any) => t.department_heads?.length > 0).length, 
          unassigned: data.filter((t: any) => !t.teacher_sections || t.teacher_sections.length === 0).length 
        });
      }
    };
    initPage();
  ...
  const handleAddSubmit = async () => { 
    if (!addForm.full_name || !addForm.national_id || !addForm.department_id) return showNotification('error', 'يرجى تعبئة الحقول الإلزامية'); 
    setSubmitting(true);
    try { 
  ...
  // 🚀 هنا الدالة التي سببت الخطأ وتم إرجاعها الآن:
    ...
    <div className="relative min-h-screen bg-slate-50 font-cairo pb-20" dir="rtl">
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
          <div>
            <h1 className="text-4xl font-black text-slate-900 tracking-tight text-right">إدارة المعلمين والأقسام</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-4 text-right">
            ...
            <div><p className="text-2xl font-black text-slate-900">{stats.unassigned}</p><p className="text-[10px] font-bold text-slate-400 uppercase">غير معينين بجدول</p></div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-[2rem] shadow-sm border border-slate-200 flex flex-col lg:flex-row items-center justify-between gap-4">
          ...
          <div className="flex w-full lg:w-auto gap-2 flex-1 max-w-2xl">
            <div className="relative flex-1 group">
              ...
              <input type="text" className="w-full rounded-xl border-0 py-3.5 pr-12 text-right bg-slate-50 focus:bg-white focus:ring-2 focus:ring-indigo-500 font-bold transition-all" placeholder="بحث بالاسم أو الرقم المدني..." value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setCurrentPage(1); }} />
            </div>
            <select value={selectedDepartment} onChange={(e) => { setSelectedDepartment(e.target.value); setCurrentPage(1); }} className="w-48 bg-slate-50 border-0 rounded-xl py-3.5 px-4 font-bold text-slate-700 outline-none cursor-pointer">
        ...
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden">
          {viewMode === 'table' ? (
            ...
            <div className="p-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {departments?.map((dept: any) => (
                <div key={dept.id} onClick={() => { setViewMode('table'); setSelectedDepartment(dept.id); setCurrentPage(1); }} className="bg-slate-50 p-8 rounded-[2.5rem] cursor-pointer border border-slate-100 hover:border-indigo-200 hover:shadow-md text-center group transition-all">
          ...
          {totalPages > 1 && viewMode === 'table' && (
            <div className="flex items-center justify-between border-t border-slate-200 bg-white px-6 py-4">
              <p className="text-sm font-bold text-slate-500">إظهار <span className="text-slate-900 font-black">{((currentPage - 1) * itemsPerPage) + 1}</span> إلى <span className="text-slate-900 font-black">{Math.min(currentPage * itemsPerPage, totalTeachers)}</span> من أصل <span className="text-slate-900 font-black">{totalTeachers}</span> معلم</p>
              <div className="flex gap-2">
                <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1 || isLoading} className="p-2 rounded-xl bg-slate-50 hover:bg-slate-100 disabled:opacity-50 transition-colors"><ChevronRight size={20}/></button>
                ...
                <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages || isLoading} className="p-2 rounded-xl bg-slate-50 hover:bg-slate-100 disabled:opacity-50 transition-colors"><ChevronLeft size={20}/></button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto" dir="rtl">
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden my-8">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              ...
              <button onClick={() => setShowAddModal(false)} className="text-slate-400 hover:text-rose-500 transition-colors"><X size={24}/></button>
            </div>
            <div className="p-8 space-y-6">
              <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <input type="text" placeholder="الاسم الرباعي *" value={addForm.full_name} onChange={e => setAddForm({...addForm, full_name: e.target.value})} className="w-full rounded-2xl border-0 py-3.5 px-4 bg-slate-50 font-bold focus:ring-2 focus:ring-indigo-500 outline-none" />
                <input type="text" placeholder="الرقم المدني *" value={addForm.national_id} onChange={e => setAddForm({...addForm, national_id: e.target.value})} className="w-full rounded-2xl border-0 py-3.5 px-4 bg-slate-50 font-bold focus:ring-2 focus:ring-indigo-500 outline-none" />
                <input type="text" placeholder="رقم الهاتف" value={addForm.phone} onChange={e => setAddForm({...addForm, phone: e.target.value})} className="w-full rounded-2xl border-0 py-3.5 px-4 bg-slate-50 font-bold text-left focus:ring-2 focus:ring-indigo-500 outline-none" dir="ltr" />
                <input type="email" placeholder="البريد الإلكتروني" value={addForm.email} onChange={e => setAddForm({...addForm, email: e.target.value})} className="w-full rounded-2xl border-0 py-3.5 px-4 bg-slate-50 font-bold text-left focus:ring-2 focus:ring-indigo-500 outline-none" dir="ltr" />
                <select value={addForm.department_id} onChange={e => setAddForm({...addForm, department_id: e.target.value})} className="sm:col-span-2 w-full rounded-2xl border-0 py-3.5 px-4 bg-slate-50 font-bold focus:ring-2 focus:ring-indigo-500 outline-none cursor-pointer">
                  <option value="">-- اختر القسم الأكاديمي المعتمد --</option>
                  {departments?.map((d:any) => <option key={d.id} value={d.id}>{d.name}</option>)}
                </select>
                <input type="text" placeholder="التخصص الدقيق (اختياري)" value={addForm.specialization} onChange={e => setAddForm({...addForm, specialization: e.target.value})} className="sm:col-span-2 w-full rounded-2xl border-0 py-3.5 px-4 bg-slate-50 font-bold focus:ring-2 focus:ring-indigo-500 outline-none" />
              </div>
              <div className="bg-slate-50 px-6 py-6 flex flex-row-reverse gap-3 border-t border-slate-100">
                <button disabled={submitting} onClick={handleAddSubmit} className="flex-1 bg-indigo-600 text-white py-4 rounded-2xl font-black shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
                  {submitting && <Loader2 className="w-5 h-5 animate-spin" />} إضافة واعتماد المعلم
                </button>
                <button onClick={() => setShowAddModal(false)} className="flex-1 bg-white border border-slate-200 text-slate-600 py-4 rounded-2xl font-black hover:bg-slate-50 transition-colors">إلغاء</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showEditModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto" dir="rtl">
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden my-8">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h3 className="text-2xl font-black text-slate-900">تعديل الملف الإداري</h3>
              <button onClick={() => setShowEditModal(false)} className="text-slate-400 hover:text-rose-500 transition-colors"><X size={24}/></button>
            </div>
            <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
              <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <input type="text" value={editForm.full_name} onChange={e => setEditForm({...editForm, full_name: e.target.value})} className="w-full rounded-2xl border-0 py-3.5 px-4 bg-slate-50 font-bold focus:ring-2 focus:ring-indigo-500 outline-none" />
                <input type="text" value={editForm.national_id} onChange={e => setEditForm({...editForm, national_id: e.target.value})} className="w-full rounded-2xl border-0 py-3.5 px-4 bg-slate-50 font-bold focus:ring-2 focus:ring-indigo-500 outline-none" />
                <select value={editForm.department_id || ''} onChange={e => setEditForm({...editForm, department_id: e.target.value})} className="sm:col-span-2 w-full rounded-2xl border-0 py-3.5 px-4 bg-slate-50 font-bold focus:ring-2 focus:ring-indigo-500 outline-none cursor-pointer">
                  <option value="">-- يرجى اختيار القسم --</option>
                  {departments?.map((d:any) => <option key={d.id} value={d.id}>{d.name}</option>)}
                </select>
                <div className="sm:col-span-2 bg-amber-50/50 p-6 rounded-3xl border border-amber-100 space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <label htmlFor="isHOD" className="font-black text-sm text-slate-800 cursor-pointer">تعيين كرئيس قسم (يمنح صلاحيات إشرافية)</label>
                  </div>
                  {editForm.isHOD && (
                    <div className="grid grid-cols-2 gap-3 animate-in slide-in-from-top-2">
                      <select value={editForm.hod_subject_id} onChange={e => setEditForm({...editForm, hod_subject_id: e.target.value})} className="w-full rounded-xl border-0 py-3.5 px-4 bg-white font-bold outline-none border border-amber-100">
                        <option value="">-- مادة الإشراف --</option>
                        {subjects?.map((s:any) => <option key={s.id} value={s.id}>{s.name}</option>)}
                      </select>
                      <select value={editForm.hod_stage} onChange={e => setEditForm({...editForm, hod_stage: e.target.value})} className="w-full rounded-xl border-0 py-3.5 px-4 bg-white font-bold outline-none border border-amber-100">
                        <option value="الكل">الكل</option><option value="متوسط">متوسط</option><option value="ثانوي">ثانوي</option>
                      </select>
                    </div>
                  )}
                </div>
              ...
              <div className="bg-slate-50 px-6 py-6 flex flex-row-reverse gap-3 border-t border-slate-100">
                <button disabled={submittingEdit} onClick={handleEditSubmit} className="flex-1 bg-indigo-600 text-white py-4 rounded-2xl font-black shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
                  {submittingEdit && <Loader2 className="w-5 h-5 animate-spin" />} حفظ التعديلات
                </button>
                <button onClick={() => setShowEditModal(false)} className="flex-1 bg-white border border-slate-200 text-slate-600 py-4 rounded-2xl font-black hover:bg-slate-50 transition-colors">إلغاء</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto" dir="rtl">
          <div className="bg-white w-full max-w-4xl rounded-[2.5rem] shadow-2xl overflow-hidden my-8">
            <div className="p-8 border-b flex justify-between items-center bg-slate-50/50">
              <h3 className="text-2xl font-black">توزيع الفصول لـ: {selectedTeacherForAssign?.users?.full_name}</h3>
              <button onClick={() => setShowAssignmentModal(false)} className="text-slate-400 hover:text-rose-500 transition-colors"><X size={24}/></button>
            </div>
            <div className="p-8 max-h-[60vh] overflow-y-auto space-y-8 custom-scrollbar text-right">
              <div className="bg-amber-50 p-8 rounded-[2rem] border border-amber-100 shadow-sm">
                <h4 className="font-black text-amber-900 mb-6">أداة التعيين المتعدد السريع</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div>
                    <p className="text-xs font-black text-amber-800 mb-3 block">1. اختر المادة (أو المواد)</p>
                    <div className="flex flex-wrap gap-2 justify-end">
                      {subjects?.map((s:any) => <button key={s.id} onClick={() => setBulkAssignData(prev => ({ ...prev, subject_ids: prev.subject_ids.includes(s.id) ? prev.subject_ids.filter(id => id !== s.id) : [...prev.subject_ids, s.id] }))} className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${bulkAssignData.subject_ids.includes(s.id) ? 'bg-amber-500 text-white shadow-md border-amber-500' : 'bg-white text-amber-700 border-amber-200 hover:border-amber-400'}`}>{s.name}</button>)}
                    </div>
                  </div>
                  <div>
                    <p className="text-xs font-black text-amber-800 mb-3 block">2. اختر الفصول المستهدفة</p>
                    <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto custom-scrollbar justify-end pr-1">
                      {sections?.map((s:any) => <button key={s.id} onClick={() => setBulkAssignData(prev => ({ ...prev, section_ids: prev.section_ids.includes(s.id) ? prev.section_ids.filter(id => id !== s.id) : [...prev.section_ids, s.id] }))} className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${bulkAssignData.section_ids.includes(s.id) ? 'bg-amber-500 text-white shadow-md border-amber-500' : 'bg-white text-amber-700 border-amber-200 hover:border-amber-400'}`}>{s.classes?.name} - {s.name}</button>)}
                    </div>
                  </div>
                </div>
                <button onClick={handleBulkAssign} className="w-full mt-8 py-4 bg-amber-500 text-white rounded-2xl font-black shadow-lg shadow-amber-500/20 hover:bg-amber-600 transition-all active:scale-95">حفظ وتطبيق التعيين</button>
              </div>

              <div>
                <h4 className="text-sm font-black text-slate-800 mb-4 block border-b border-slate-100 pb-2">التعديل اليدوي الدقيق</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {sections?.map((sec:any) => (
                    <div key={sec.id} className="bg-slate-50 p-5 rounded-2xl border border-slate-200 hover:border-indigo-200 transition-colors">
                      <h4 className="font-black text-sm mb-3 text-slate-800">{sec.classes?.name} - {sec.name}</h4>
                      <div className="flex flex-wrap gap-1.5 justify-end">
                        {subjects?.map((sub:any) => {
                          const isAssigned = teacherSections.some(ts => ts.section_id === sec.id && ts.subject_id === sub.id);
                          return <button key={sub.id} onClick={() => toggleAssignment(sec.id, sub.id)} className={`px-2.5 py-1.5 rounded-lg text-[10px] font-bold border transition-all ${isAssigned ? 'bg-emerald-500 text-white border-emerald-500 shadow-sm' : 'bg-white text-slate-500 border-slate-200 hover:border-indigo-300 hover:text-indigo-600'}`}>{sub.name}</button>
                        })}
                      </div>
                    </div>
                  ))}
            ...
            <div className="bg-slate-50 p-6 flex justify-end border-t border-slate-100">
              <button onClick={() => setShowAssignmentModal(false)} className="px-10 py-4 bg-slate-900 text-white rounded-2xl font-black shadow-lg hover:bg-slate-800 transition-all">إغلاق النافذة</button>
            </div>
          </div>
        </div>
      )}

      {/* 🚀 هذه هي الدالة التي سببت الخطأ في التحديث السابق وتم إرجاعها واستخدامها هنا بنجاح */}
      {showPasswordModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" dir="rtl">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white w-full max-w-md rounded-[2.5rem] p-8 text-center shadow-2xl border border-slate-100">
            <div className="mx-auto w-16 h-16 bg-sky-50 text-sky-600 flex items-center justify-center rounded-2xl mb-6 shadow-inner border border-sky-100">
              <Key size={32} />
            </div>
            <h3 className="text-2xl font-black mb-2 text-slate-900">تغيير كلمة المرور</h3>
            <p className="text-sm font-bold text-slate-500 mb-8">يجب أن تحتوي كلمة المرور على 6 خانات على الأقل.</p>
            <input type="text" placeholder="اكتب كلمة المرور الجديدة..." value={resetPasswordForm.newPassword} onChange={e => setResetPasswordForm({...resetPasswordForm, newPassword: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 font-bold text-center text-xl mb-8 outline-none focus:ring-2 focus:ring-sky-500 shadow-inner transition-all" dir="ltr"/>
            <div className="flex gap-3">
              <button onClick={handleResetPasswordSubmit} className="flex-1 bg-sky-600 text-white py-4 rounded-2xl font-black shadow-lg shadow-sky-200 hover:bg-sky-700 transition-all active:scale-95">حفظ التغيير</button>
              <button onClick={() => { setShowPasswordModal(false); setResetPasswordForm({ userId: '', newPassword: '' }); }} className="flex-1 bg-white border border-slate-200 text-slate-600 py-4 rounded-2xl font-black hover:bg-slate-50 transition-all">إلغاء</button>
            </div>
          </motion.div>
        </div>
      )}
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" dir="rtl">
          <div className="bg-white p-8 rounded-[2.5rem] text-center max-w-sm w-full shadow-2xl border border-slate-100">
            <div className="mx-auto w-16 h-16 bg-rose-50 text-rose-500 rounded-full flex items-center justify-center mb-6 shadow-inner border border-rose-100"><Trash2 size={32}/></div>
            <h3 className="text-2xl font-black mb-3 text-slate-900">حذف نهائي؟</h3>
            <p className="text-slate-500 font-bold text-sm mb-8 leading-relaxed">هذا الإجراء سيقوم بحذف المعلم وجميع سجلاته وحصصه نهائياً من النظام. هل أنت متأكد؟</p>
            <div className="flex gap-3">
              <button onClick={confirmDelete} className="bg-rose-600 text-white py-4 rounded-2xl font-black flex-1 shadow-lg shadow-rose-200 hover:bg-rose-700 transition-all active:scale-95">نعم، احذف</button>
              <button onClick={() => setShowDeleteModal(false)} className="bg-white border border-slate-200 text-slate-600 py-4 rounded-2xl font-black flex-1 hover:bg-slate-50 transition-all">تراجع</button>
            </div>
          </div>
        </div>
      )}
