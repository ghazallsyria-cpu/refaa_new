...
export default function StudentEvaluationsDashboard() {
  ...
  // 🚀 دوال إدارة بنود التقييم (تم إصلاحها لمنع الـ ReferenceError)
  const addCriterion = () => {
  ...
  const deleteEvaluation = async (id: string, name: string) => {
      if (!confirm(`هل أنت متأكد من حذف تقييم الطالب (${name}) نهائياً؟`)) return;
      try {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 sm:p-8 font-cairo text-slate-800 pb-32" dir="rtl">
        ...
        <div className="bg-white border border-slate-200 p-4 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
           ...
           <div className="flex items-center gap-3 w-full sm:w-auto">
              ...
              <select 
                 ...
                 className="w-full sm:w-80 bg-slate-50 border border-slate-200 text-slate-800 font-black text-sm rounded-xl px-4 py-3 outline-none focus:border-indigo-500 transition-all"
              >
             ...
             <div className="overflow-x-auto custom-scrollbar p-6">
                <table className="w-full text-right border-collapse min-w-[900px]">
                   <thead className="bg-slate-50 text-slate-500 text-[10px] font-black uppercase tracking-widest border-b border-slate-200">
                      <tr>
                         <th className="p-5 rounded-r-xl">الطالب / الفصل</th>
                         <th className="p-5">المعلم / المادة</th>
                         <th className="p-5">التقييم</th>
                         <th className="p-5 min-w-[200px]">التعليق</th>
                         <th className="p-5 rounded-l-xl text-center">إجراءات</th>
                      </tr>
                   </thead>
                   <tbody>
                      {filteredRaw.map(ev => (
                         <tr key={ev.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors group">
                            <td className="p-5">
                            ...
                            <td className="p-5">
                               <div className="flex items-center gap-2">
                                 <div className="w-8 h-8 rounded-full bg-slate-200 border border-slate-300 overflow-hidden shrink-0">{ev.teacher_avatar ? <img src={ev.teacher_avatar} className="w-full h-full object-cover" crossOrigin="anonymous"/> : <UserCircle className="w-full h-full text-slate-400"/>}</div>
                                 <div><p className="font-bold text-slate-700 text-sm leading-tight">{ev.teacher_name}</p><p className="text-[10px] text-slate-400 font-bold">{ev.subject}</p></div>
                               </div>
                            </td>
                            <td className="p-5">
                               <div className="flex flex-col gap-1.5 w-48">
                                 {ev.dynamic_ratings && typeof ev.dynamic_ratings === 'object' && Object.keys(ev.dynamic_ratings).length > 0 ? Object.entries(ev.dynamic_ratings).map(([k, v]) => (
                                    <div key={k} className="flex justify-between items-center text-[9px] font-bold bg-amber-50 text-amber-700 px-2 py-1 rounded-lg border border-amber-100 shadow-sm">
                                       <span className="truncate w-24" title={k}>{k}</span>
                                       <span className="font-black flex items-center gap-0.5">{v} <Star className="w-3 h-3 fill-amber-500"/></span>
                                    </div>
                                 )) : <span className="text-[10px] font-bold text-slate-400 italic bg-slate-100 px-2 py-1 rounded-lg border border-slate-200">نظام قديم: {ev.avg_score.toFixed(1)}⭐</span>}
                               </div>
                            </td>
                            <td className="p-5">
                               <p className="text-xs font-bold text-slate-600 italic whitespace-pre-wrap leading-relaxed max-w-sm">{ev.feedback ? `"${ev.feedback}"` : <span className="text-slate-400 not-italic">بدون تعليق</span>}</p>
                            </td>
                            <td className="p-5 text-center">
                               <button onClick={() => deleteEvaluation(ev.id, ev.student_name)} className="p-2.5 text-rose-500 bg-white hover:bg-rose-500 hover:text-white rounded-xl transition-all border border-rose-200 shadow-sm opacity-50 group-hover:opacity-100"><Trash2 className="w-4 h-4"/></button>
                            </td>
                         </tr>
                      ))}
                      {filteredRaw.length === 0 && <tr><td colSpan={5} className="p-10 text-center text-slate-500 font-bold bg-slate-50 rounded-xl border border-dashed border-slate-200">لا يوجد سجلات مطابقة للبحث.</td></tr>}
                   </tbody>
                </table>
             </div>
          </div>
      ...
      {/* 🚀 نافذة الإعدادات (Pure, Lightweight & Safe Modal) */}
      {isSettingsOpen && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
           {/* خلفية غامقة خفيفة على الرام */}
           ...
           <div className="bg-white rounded-[2rem] w-full max-w-lg relative z-10 flex flex-col max-h-[85vh] overflow-hidden shadow-2xl">
              ...
              <div className="p-6 overflow-y-auto flex-1 space-y-6 custom-scrollbar bg-white">
