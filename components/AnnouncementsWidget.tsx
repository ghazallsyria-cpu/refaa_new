...
interface AnnouncementsWidgetProps {
  authRole: string; // 'student', 'teacher', 'parent', 'admin', 'management'
}
...
export default function AnnouncementsWidget({ authRole }: AnnouncementsWidgetProps) {
  ...
  useEffect(() => {
    const fetchWidgetAnnouncements = async () => {
      try {
        setLoading(true);
        // سحب أحدث 4 إعلانات فقط للويدجت
        let query = supabase
          .from('announcements')
        ...
        // 🛡️ جدار الرفعة الناري: فلترة الإعلانات حسب رتبة المستخدم
        if (authRole !== 'admin' && authRole !== 'management') {
          // جلب الإعلانات الموجهة للجميع ('all') أو الموجهة لرتبة المستخدم المحددة
