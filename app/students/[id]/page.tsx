    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
            {[
              { label: 'حضور', count: attendance.filter((a:any) => a.status === 'present').length, color: 'emerald' },
              { label: 'غياب', count: attendance.filter((a:any) => a.status === 'absent').length, color: 'rose' },
              { label: 'تأخير', count: attendance.filter((a:any) => a.status === 'late').length, color: 'amber' },
              { label: 'عذر', count: attendance.filter((a:any) => a.status === 'excused').length, color: 'blue' }
            ].map((item, idx) => (
...
const AssignmentsTab = ({ assignments }: any) => (
...
const PrivateNotesTab = ({ privateNotes, newNote, setNewNote, handleAddPrivateNote, isSubmittingNote }: any) => (
