...
import { 
  ...
  CheckCircle2, Edit3, UserCheck, Fingerprint
} from 'lucide-react';
...
export default function ControlTeamManagement() {
  ...
  // 🚀 محرك الطباعة الأصلي (Native CSS Print) - أسرع وأدق ولا يقص الصفحات!
  const printBadges = () => {
  ...
  return (
    <div className={`min-h-screen p-4 sm:p-6 md:p-10 font-cairo pb-20 relative ${isPrinting ? 'bg-white' : 'bg-slate-50'}`} dir="rtl">
      
      {/* 🚀 هندسة الـ CSS للطباعة: فواصل صفحات حقيقية وإخفاء للواجهة */}
      <style dangerouslySetInnerHTML={{ __html: `
        @media print {
          @page { size: A4 portrait; margin: 15mm; }
          body { background: white !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
          .no-print, nav, footer, header, [class*="fixed"] { display: none !important; }
          .print-area-badges { display: block !important; width: 100% !important; background: white !important; }
          .page-break { page-break-after: always; break-after: page; }
        }
        @media screen {
          .print-area-badges { display: none !important; }
        }
      ...
      <AnimatePresence>
        { (isLoading) && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-slate-900/95 backdrop-blur-sm z-[100] flex flex-col items-center justify-center text-white">
            ...
            <h2 className="text-2xl font-black mb-2 animate-pulse text-center px-4">تجهيز غرفة العمليات...</h2>
          </motion.div>
        )}
      </AnimatePresence>

      <div className={`max-w-7xl mx-auto space-y-6 relative z-10 no-print ${isPrinting ? 'hidden' : 'block'}`}>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-lg bg-white rounded-[2rem] shadow-2xl p-6 sm:p-8 flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex-1 overflow-hidden flex flex-col min-h-[400px]">
                <div className="mb-4 shrink-0">
                   ...
                   <input type="text" className="w-full bg-indigo-50/50 border border-indigo-200 rounded-xl py-3 px-4 text-sm font-black text-indigo-900 focus:outline-none focus:border-indigo-500 transition-colors shadow-inner" placeholder="اكتب المسمى الوظيفي..." value={customRoleTitle} onChange={(e) => setCustomRoleTitle(e.target.value)} />
                ...
                <div className="relative mb-4 shrink-0 mt-2">
                   ...
                   <input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3.5 pr-11 pl-4 text-sm font-bold text-slate-800 focus:outline-none focus:border-slate-400 transition-colors" placeholder="ابحث عن اسم المعلم أو الإداري..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                </div>
                ...
                <div className="pt-4 shrink-0 border-t border-slate-100 mt-4">
                  <button onClick={handleAssign} disabled={!selectedUserId || !customRoleTitle.trim() || isSubmitting} className={cn("w-full py-4 text-white font-black rounded-xl disabled:opacity-50 shadow-md flex items-center justify-center gap-2 transition-all active:scale-95", selectedBaseRole.bg.replace('/10', ''), "hover:opacity-90")}>
                    {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin"/> : <CheckCircle2 className="w-5 h-5" />} اعتماد التكليف الرسمي
                  </button>
                </div>
              </div>
            </motion.div>
      ...
      {/* ====================================================================================== */}
      {/* 🖨️ قوالب الطباعة الأصلية (Native CSS Print) - لن يتم قص أي صفحة بعد الآن! */}
      {/* ====================================================================================== */}
      <div className={`print-area-badges w-full bg-white ${isPrinting ? 'block' : 'hidden'}`} dir="rtl">
         {chunkArray(teamMembers, 6).map((chunk, pageIndex) => (
            <div key={pageIndex} className="page-break flex flex-wrap gap-8 justify-center items-start pt-8" style={{ height: '277mm' }}>
