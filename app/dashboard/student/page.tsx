...
export default function StudentDashboard() {
  ...
  if (isChecking) {
    return (
      <div className="flex h-[80vh] items-center justify-center bg-slate-50/50 font-cairo">
        <div className="flex flex-col items-center gap-4">
          <Loader2 className="w-14 h-14 text-indigo-600 animate-spin" />
          <p className="text-slate-500 font-bold animate-pulse tracking-widest">جاري التحقق من الهوية...</p>
        </div>
      </div>
    );
  }
  ...
  if (authRole !== 'student') {
    return <div className="p-10 text-center font-bold text-rose-600 min-h-[80vh] flex items-center justify-center bg-slate-50 font-cairo">هذه الصفحة مخصصة للطلاب فقط.</div>;
  }
    ...
    return (
      <div className="flex h-[80vh] items-center justify-center bg-slate-50/50 font-cairo">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-500 font-bold animate-pulse tracking-widest">جاري إعداد لوحتك الدراسية...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 sm:space-y-8 pb-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 overflow-x-hidden font-cairo pt-6" dir="rtl">
      
      {/* 🚀 الهيدر الفخم الموحد */}
      <div className="relative overflow-hidden rounded-[2rem] sm:rounded-[3rem] bg-gradient-to-r from-blue-700 via-indigo-600 to-violet-700 p-6 sm:p-12 text-white shadow-2xl shadow-indigo-200/50">
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 text-center sm:text-right">
            <Link href={`/students/${user.id}`} className="relative group shrink-0">
              <div className="h-28 w-28 sm:h-32 sm:w-32 rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden border-4 border-white/20 shadow-2xl bg-white/10 backdrop-blur-md flex items-center justify-center transition-transform duration-500 group-hover:scale-105 group-hover:rotate-3">
                {avatarUrl ? <img src={avatarUrl} alt={studentData?.users?.full_name} className="w-full h-full object-cover" /> : <span className="text-4xl sm:text-5xl font-black text-white/70 drop-shadow-md">{studentData?.users?.full_name?.charAt(0) || 'ط'}</span>}
              </div>
            ...
            <div className="pt-2">
              ...
              <h1 className="text-2xl sm:text-4xl lg:text-5xl font-black mb-2 tracking-tight drop-shadow-md leading-tight">
                مرحباً، {studentData?.users?.full_name?.split(' ')[0] || 'بطلنا'} 👋
              </h1>
              <p className="text-indigo-100 text-sm sm:text-base font-bold flex flex-wrap items-center justify-center sm:justify-start gap-2 mb-3">
              ...
              {/* 🚀 الزر الجديد: استعراض الملف الأكاديمي */}
              <Link href={`/students/${user.id}`} className="inline-flex items-center gap-2 text-indigo-100 hover:text-white font-bold text-sm bg-white/10 px-4 py-2 rounded-xl border border-white/10 transition-all hover:bg-white/20 shadow-sm">
          ...
          <div className="flex flex-row flex-wrap gap-3 sm:gap-4 justify-center lg:shrink-0">
            <div className="rounded-2xl sm:rounded-[2rem] bg-white/10 p-4 sm:p-6 backdrop-blur-md border border-white/20 flex flex-col items-center justify-center min-w-[130px] shadow-lg hover:bg-white/20 transition-colors">
              <p className="text-[10px] sm:text-xs text-indigo-200 uppercase tracking-widest font-black mb-1">نسبة الحضور</p>
              <p className="text-3xl sm:text-4xl font-black drop-shadow-md">{attendanceStats?.rate || 0}%</p>
            </div>
            <div className="rounded-2xl sm:rounded-[2rem] bg-white/10 p-4 sm:p-6 backdrop-blur-md border border-white/20 flex flex-col items-center justify-center min-w-[130px] shadow-lg hover:bg-white/20 transition-colors">
              <p className="text-[10px] sm:text-xs text-indigo-200 uppercase tracking-widest font-black mb-1">المتوسط العام</p>
              <p className="text-3xl sm:text-4xl font-black drop-shadow-md">{avgScore}%</p>
            </div>
          </div>
        </div>
          ...
          <div className="relative z-10 mt-10 pt-6 border-t border-white/20 w-full">
            ...
            <div className="flex gap-4 sm:gap-6 overflow-x-auto pb-4 custom-scrollbar mask-fade-edges">
              {myBadges.map((badgeEntry, index) => (
                <div key={badgeEntry.id || index} className="flex-shrink-0 bg-white/10 backdrop-blur-md rounded-[2rem] p-5 border border-white/20 flex items-center gap-5 w-[22rem] hover:bg-white/20 transition-all duration-300 hover:shadow-2xl hover:shadow-white/10 group cursor-default">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs font-bold text-indigo-200 line-clamp-2 mt-1 leading-tight" title={badgeEntry.reason}>{badgeEntry.reason || 'تقديراً للجهود'}</p>
                    <p className="text-[10px] text-white/50 mt-2 bg-black/20 w-fit px-2 py-1 rounded-lg">بتاريخ: {safeFormat(badgeEntry.granted_at, 'd MMM yyyy')}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* 🚀 إنذار الغياب المتقدم (تنبيه الطالب الشخصي) */}
      <AnimatePresence>
        {fullDaysAbsent > 0 && (
          <motion.div initial={{ opacity: 0, y: -20, height: 0 }} animate={{ opacity: 1, y: 0, height: 'auto' }} className="relative overflow-hidden rounded-[2rem] sm:rounded-[2.5rem] bg-gradient-to-r from-rose-600 via-red-600 to-rose-700 p-6 sm:p-8 text-white shadow-2xl shadow-rose-500/30 border-2 border-rose-400/50">
              ...
              <div className="flex items-center gap-4 sm:gap-6 w-full lg:w-auto">
                ...
                <div>
                  <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-black/20 backdrop-blur-sm text-[10px] sm:text-xs font-black uppercase tracking-widest mb-2 border border-white/10">
                    <ShieldAlert className="w-3.5 h-3.5 text-yellow-400" /> إنذار إداري مسجل
                  </div>
                  ...
                  <p className="text-rose-100 text-xs sm:text-sm font-bold leading-relaxed max-w-xl">حسب لائحة السلوك والمواظبة، تم احتساب وتجميع الحصص التي تغيبت عنها لتعادل <span className="bg-white/20 px-1.5 py-0.5 rounded text-white mx-1">{fullDaysAbsent} أيام</span> فعلية. يرجى الالتزام بالدوام.</p>
                </div>
              </div>
      ...
      {isTenthGrade && !hasSelectedTrack && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="rounded-[2.5rem] bg-gradient-to-br from-amber-50 to-orange-50 border-2 border-amber-200 p-8 shadow-xl shadow-amber-100/50">
          <div className="flex flex-col md:flex-row items-center gap-8">
            <div className="p-5 bg-white rounded-[2rem] shadow-sm border border-amber-100 shrink-0"><Target className="h-12 w-12 text-amber-500 animate-pulse" /></div>
            <div className="flex-1 text-center md:text-right">
              ...
              <p className="text-slate-600 font-bold text-sm sm:text-base leading-relaxed">يرجى اختيار المسار الأكاديمي (علمي أو أدبي) الذي ترغب في دراسته في الصف الحادي عشر.</p>
            </div>
            <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto shrink-0">
              <button onClick={() => handleTrackSelection('scientific')} className="px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black text-base shadow-xl shadow-indigo-200 hover:bg-indigo-700 transition-all hover:scale-105 active:scale-95 w-full sm:w-auto">المسار العلمي</button>
              <button onClick={() => handleTrackSelection('literary')} className="px-8 py-4 bg-emerald-500 text-white rounded-2xl font-black text-base shadow-xl shadow-emerald-200 hover:bg-emerald-600 transition-all hover:scale-105 active:scale-95 w-full sm:w-auto">المسار الأدبي</button>
            </div>
          </div>
        </motion.div>
      )}
      ...
      {isTenthGrade && hasSelectedTrack && (
        <div className="rounded-[2rem] bg-emerald-50/50 border border-emerald-100 p-6 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-emerald-100 rounded-2xl shrink-0"><CheckCircle2 className="h-7 w-7 text-emerald-600" /></div>
            <div>
              <p className="text-lg font-black text-emerald-900">تم اعتماد مسارك الأكاديمي</p>
          ...
          <p className="text-[10px] text-emerald-600/70 font-black uppercase tracking-widest bg-white px-3 py-1.5 rounded-xl">تم الاختيار في {safeFormat(studentData.track_selection_date, 'd MMMM yyyy')}</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 lg:gap-6">
        {[
          ...
          <Link key={idx} href={item.href} className="group">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} className={`p-6 rounded-[2rem] bg-white border border-slate-100 shadow-sm hover:shadow-xl transition-all flex flex-col items-center justify-center gap-4 group-hover:-translate-y-1 h-full`}>
              <div className={`p-4 bg-${item.color}-50 rounded-2xl group-hover:bg-${item.color}-500 transition-colors duration-500`}><item.icon className={`h-8 w-8 text-${item.color}-500 group-hover:text-white transition-colors duration-500`} /></div>
              <span className={`font-black text-slate-700 group-hover:text-${item.color}-600 transition-colors text-sm text-center`}>{item.label}</span>
            </motion.div>
          </Link>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 items-start">
        ...
        <div className="lg:col-span-2 space-y-6 lg:space-y-8 w-full">
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden hover:shadow-lg transition-all relative">
            ...
            <div className="p-6 lg:p-8 border-b border-slate-100/50 flex items-center justify-between bg-white/50 relative z-10 gap-4">
              <h2 className="text-xl sm:text-2xl font-black text-slate-900 flex items-center gap-3">
                <div className="p-3 bg-sky-50 rounded-2xl border border-sky-100 shadow-inner"><Clock className="h-6 w-6 text-sky-600" /></div> جدول حصص اليوم
              </h2>
              <Link href="/dashboard/student/schedule" className="text-sm font-bold text-sky-600 hover:text-sky-700 hover:bg-sky-100 transition-colors px-4 py-2 bg-sky-50 rounded-xl shadow-sm border border-sky-100 shrink-0">الجدول الكامل</Link>
            </div>
            
            <div className="p-6 lg:p-8 relative z-10 bg-slate-50/30 overflow-x-hidden">
              {todaysSchedule.length > 0 ? (
                  ...
                  {todaysSchedule.map((item, i) => {
                    ...
                    return (
                      <div key={i} className={cn("relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group", current ? "is-active z-20" : "z-10")}>
                        <div className={cn("flex items-center justify-center w-12 h-12 rounded-2xl border-4 border-white shadow-md shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 transition-all duration-500", current ? "bg-sky-500 text-white scale-125 ring-4 ring-sky-100" : next ? "bg-amber-400 text-white" : "bg-white text-slate-400 border-slate-200")}>
                          {current ? <Play className="h-5 w-5 animate-pulse ml-1" /> : <span className="text-base font-black">{item.period}</span>}
                        </div>
                        <div className={cn("w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-5 rounded-3xl border transition-all duration-500", current ? "bg-gradient-to-br from-sky-50 to-white border-sky-200 shadow-xl shadow-sky-100/50 scale-[1.02]" : next ? "bg-amber-50 border-amber-200 shadow-md" : "bg-white border-slate-100 shadow-sm hover:shadow-md hover:border-sky-200")}>
                          <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-3 mb-3">
                            <h3 className={cn("text-lg font-black transition-colors truncate pl-2", current ? "text-sky-900" : next ? "text-amber-900" : "text-slate-800")}>{item.subjects?.name}</h3>
                            <div className="flex flex-wrap items-center gap-1.5 shrink-0">
                              {current && <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-sky-500 text-[10px] font-bold text-white shadow-md"><div className="w-1.5 h-1.5 rounded-full bg-white animate-ping" /> الحصة الآن</span>}
                              {next && !current && <span className="px-2.5 py-1 rounded-full bg-amber-200 text-amber-800 text-[10px] font-bold shadow-sm">الحصة القادمة</span>}
                              <span className={cn("text-xs font-black px-2.5 py-1 rounded-xl shadow-sm border", current ? "bg-white text-sky-700 border-sky-100" : "bg-slate-50 text-slate-500 border-slate-200")}>الحصة {item.period}</span>
                            </div>
                          </div>
                          <div className="flex flex-wrap items-center justify-between pt-3 border-t border-slate-100/80 gap-3">
                            <p className={cn("text-sm font-bold flex items-center gap-2", current ? "text-sky-700" : "text-slate-600")}><GraduationCap className="h-4 w-4 opacity-70 shrink-0" /><span className="truncate">أ. {item.teachers?.users?.full_name || 'غير محدد'}</span></p>
                            {(() => {
                              const periodInfo = periods.find(p => p.period_number === item.period);
                              if (periodInfo?.start_time && periodInfo?.end_time) {
                                return <span className={cn("text-[11px] font-black tracking-widest flex items-center gap-1.5 bg-white px-2 py-1 rounded-lg border shadow-sm shrink-0", current ? "text-sky-600 border-sky-100" : "text-slate-400 border-slate-100")} dir="ltr"><Clock className="w-3 h-3 shrink-0" />{periodInfo.start_time.substring(0, 5)} - {periodInfo.end_time.substring(0, 5)}</span>;
                              }
                            })()}
                          </div>
                        </div>
                ...
                <div className="text-center py-16 bg-white rounded-[2rem] border border-dashed border-slate-200 shadow-sm">
                  <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-slate-50 mb-4 shadow-inner border border-slate-100"><Calendar className="h-10 w-10 text-slate-300" /></div>
                  <h3 className="text-xl font-black text-slate-900 mb-2">لا توجد حصص اليوم</h3>
                  <p className="text-sm text-slate-500 font-medium">استمتع بيومك! ليس لديك أي حصص مجدولة لهذا اليوم.</p>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Performance Chart */}
          <div className="rounded-[2.5rem] bg-white/80 backdrop-blur-xl p-6 lg:p-8 shadow-sm border border-slate-100 hover:shadow-lg transition-all relative overflow-hidden">
            ...
            <div className="mb-8 flex items-center justify-between relative z-10">
              <h2 className="text-xl sm:text-2xl font-black text-slate-900 flex items-center gap-3">
                <div className="p-3 bg-indigo-50 rounded-2xl border border-indigo-100 shadow-inner"><TrendingUp className="h-6 w-6 text-indigo-600" /></div>
              ...
              {unlockedGrades.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={unlockedGrades.map(g => ({ ...g, displayTitle: g.exam?.title || 'اختبار', displayScore: g.score || 0 })).reverse()}>
                    <defs>
                      <linearGradient id="colorScore" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#4f46e5" stopOpacity={0.3}/><stop offset="95%" stopColor="#4f46e5" stopOpacity={0}/></linearGradient>
                    </defs>
                    ...
                    <Tooltip contentStyle={{borderRadius: '1rem', border: '1px solid #e2e8f0', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)', fontWeight: 'bold', fontSize: '12px'}} itemStyle={{color: '#4f46e5', fontWeight: '900'}} />
        ...
        <div className="space-y-6 lg:space-y-8 w-full">
          ...
          {/* Recent Grades */}
          <div className="rounded-[2.5rem] bg-white/80 backdrop-blur-xl p-6 shadow-sm border border-slate-100 hover:shadow-lg transition-all relative overflow-hidden">
            ...
            <div className="mb-6 flex items-center justify-between relative z-10 gap-3">
              <h2 className="text-xl font-black text-slate-900 flex items-center gap-3">
                <div className="p-2.5 bg-emerald-50 rounded-xl border border-emerald-100 shadow-inner shrink-0"><Award className="h-5 w-5 text-emerald-600" /></div> آخر النتائج
              </h2>
              <Link href="/student/performance" className="text-xs font-bold text-emerald-600 hover:text-white flex items-center gap-1 bg-emerald-50 px-3 py-1.5 rounded-lg hover:bg-emerald-600 transition-colors shadow-sm border border-emerald-100">السجل الكامل <ChevronLeft className="h-4 w-4" /></Link>
            </div>
            ...
            <div className="space-y-4 relative z-10">
              {recentGrades.length > 0 ? (
                recentGrades.slice(0,4).map((grade) => {
                  ...
                  return (
                    <div key={grade.id} className={`flex items-center justify-between p-4 rounded-2xl border-2 transition-all ${isLocked ? 'bg-slate-50 border-slate-100' : 'bg-white border-slate-100 shadow-sm hover:shadow-md hover:border-emerald-200 group'}`}>
                      <div className="flex items-center gap-4 min-w-0">
                        <div className={`h-12 w-12 rounded-xl flex items-center justify-center shadow-sm shrink-0 transition-colors ${isLocked ? 'bg-slate-200 text-slate-500' : grade.score >= 50 ? 'bg-emerald-50 text-emerald-500 group-hover:bg-emerald-500 group-hover:text-white' : 'bg-red-50 text-red-500 group-hover:bg-red-500 group-hover:text-white'}`}>
                        ...
                        <div className="min-w-0 pr-1">
                          ...
                          <p className="text-[10px] font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-md inline-block truncate max-w-full">{grade.exam?.subject?.name}</p>
                        </div>
                      </div>
                      <div className="text-right flex flex-col items-end justify-center shrink-0 pl-1">
                        {isLocked ? (
                          <><span className="text-[10px] font-black text-slate-500 bg-white shadow-sm border border-slate-200 px-2 py-1 rounded-md flex items-center gap-1 mb-1"><Lock className="w-3 h-3" /> محجوبة</span><p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">{safeFormat(grade.completed_at, 'd MMM')}</p></>
                        ) : (
                          <><p className={`text-lg sm:text-xl font-black flex items-baseline gap-1 ${grade.score >= 50 ? 'text-emerald-600' : 'text-red-600'}`}>{grade.score} <span className="text-[10px] font-bold opacity-60">/ {grade.exam?.max_score || 100}</span></p><p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-1">{safeFormat(grade.completed_at, 'd MMM')}</p></>
                        )}
                      </div>
                    </div>
                  );
                ...
                <div className="text-center py-8 text-slate-500 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200 font-bold text-sm">لا توجد نتائج اختبارات حالياً</div>
              )}
            </div>
          </div>
          ...
          <div className="rounded-[2.5rem] bg-white/80 backdrop-blur-xl p-6 shadow-sm border border-slate-100 hover:shadow-lg transition-all relative overflow-hidden">
            <div className="mb-6 flex items-center justify-between relative z-10">
              <h2 className="text-xl font-black text-slate-900 flex items-center gap-3">
                <div className="p-2.5 bg-amber-50 rounded-xl border border-amber-100 shadow-inner"><Target className="h-5 w-5 text-amber-500" /></div> واجبات مطلوبة
              </h2>
            </div>
            <div className="space-y-4 relative z-10">
              {upcomingAssignments.length > 0 ? (
                  ...
                  <Link href={`/assignments/${assignment.id}`} key={assignment.id} className="block group">
                    <div className="p-5 rounded-2xl border-2 border-slate-100 hover:border-amber-300 hover:shadow-md transition-all bg-white flex flex-col justify-between h-full">
                      <div className="flex items-start justify-between mb-3 gap-2">
                        ...
                        <span className="text-[10px] font-black px-2 py-1 bg-amber-50 text-amber-700 border border-amber-200 rounded-lg whitespace-nowrap shrink-0 flex items-center gap-1.5"><Clock className="w-3 h-3" /> {safeFormat(assignment.due_date, 'd MMM')}</span>
                      </div>
                ...
                <div className="text-center py-8 text-slate-400 font-bold bg-slate-50 rounded-2xl border border-dashed border-slate-200 text-sm">لا توجد واجبات مطلوبة حالياً</div>
              )}
            </div>
          </div>
          ...
          <div className="rounded-[2.5rem] bg-white/80 backdrop-blur-xl p-6 shadow-sm border border-slate-100 hover:shadow-lg transition-all relative overflow-hidden">
            <div className="mb-6 flex items-center justify-between relative z-10">
              <h2 className="text-xl font-black text-slate-900 flex items-center gap-3">
                <div className="p-2.5 bg-rose-50 rounded-xl border border-rose-100 shadow-inner"><Bell className="h-5 w-5 text-rose-500" /></div> اختبارات قادمة
              </h2>
            </div>
            <div className="space-y-4 relative z-10">
              {upcomingExams.length > 0 ? (
                upcomingExams.map((exam) => (
                  <Link href={`/exams/take/${exam.id}`} key={exam.id} className="block group">
                    <div className="p-5 rounded-2xl border-2 border-slate-100 hover:border-rose-300 hover:shadow-md transition-all bg-white flex flex-col justify-between h-full">
                      ...
                      <p className="text-[10px] sm:text-xs font-bold text-slate-500 bg-slate-50 px-2 py-1 rounded-md inline-block w-fit border border-slate-100 mb-4">{exam.subject?.name}</p>
                ...
                <div className="text-center py-8 text-slate-400 font-bold bg-slate-50 rounded-2xl border border-dashed border-slate-200 text-sm">لا توجد اختبارات مجدولة حالياً</div>
              )}
            </div>
          </div>
