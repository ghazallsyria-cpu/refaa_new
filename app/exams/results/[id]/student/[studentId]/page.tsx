  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-10 pb-24" dir="rtl">
      ...
      <div className="glass-card p-10 rounded-[40px] border-t-[16px] border-t-indigo-600 shadow-2xl relative overflow-hidden bg-white">
        ...
        <h1 className="text-4xl font-black text-slate-900 mb-8">{exam?.title}</h1>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 relative z-10">
          ...
          <div className="bg-slate-50 p-6 rounded-[28px] border border-slate-100">
            ...
            <span className="text-xl font-black text-slate-800">{exam?.subject_name}</span>
          </div>
          <div className="bg-slate-50 p-6 rounded-[28px] border border-slate-100">
            ...
            <span className="text-xl font-black text-emerald-600">{attempt?.score}%</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-8">
        ...
        {answers.map((ans, idx) => (
          <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} key={ans.id} className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-xl relative overflow-hidden transition-all hover:shadow-indigo-50">
            <div className={`absolute top-0 right-0 w-3 h-full ${ans.is_correct ? 'bg-emerald-500' : 'bg-red-500'}`} />
            <div className="flex justify-between items-start mb-6"><span className="text-xs font-black text-slate-400 uppercase tracking-widest bg-slate-50 px-4 py-2 rounded-full">سؤال {idx + 1}</span><div className={`px-4 py-2 rounded-xl text-sm font-black ${ans.is_correct ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>الدرجة: {ans.points_earned || 0} / {ans.question?.points || 0}</div></div>
              ...
              <div className="flex-1 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className={`p-6 rounded-[28px] border ${ans.is_correct ? 'bg-emerald-50/30 border-emerald-100' : 'bg-red-50/30 border-red-100'}`}>
                    ...
                    <p className="text-lg font-black">{ans.selected_option_id ? ans.question?.options?.find((o: any) => o.id === ans.selected_option_id)?.content : ans.text_answer || 'لم يتم الإجابة'}</p>
                  </div>
                  {!ans.is_correct && <div className="p-6 rounded-[28px] bg-slate-50 border border-slate-100 shadow-inner"><p className="text-[10px] font-black uppercase text-slate-400 mb-2">الإجابة الصحيحة</p><p className="text-lg font-black text-indigo-600">{ans.question?.options?.find((o: any) => o.is_correct)?.content || 'تتطلب تصحيحاً يدوياً'}</p></div>}
                </div>
              </div>
              {ans.question?.media_url && <div className="relative w-full md:w-64 aspect-square rounded-3xl overflow-hidden border-2 border-slate-50 bg-slate-50"><Image src={ans.question.media_url} alt="Question" fill className="object-contain" unoptimized /></div>}
            </div>
          </motion.div>
        ))}
      </div>
