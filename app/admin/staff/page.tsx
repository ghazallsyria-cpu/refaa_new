...
import { 
  ...
  CheckCircle2, AlertCircle, Building2, UserCog, Mail, Phone, MapPin
} from 'lucide-react';
...
export default function StaffManagementPage() {
  ...
  const openModal = (staff: any = null) => {
    if (staff) {
      ...
      setIsEditing(false);
      setFormData({ id: '', full_name: '', national_id: '', email: '', phone: '', job_category: '', job_title: '', scope_stage: 'الكل' });
    }
    setShowModal(true);
  };
      ...
      <div className="relative overflow-hidden rounded-[2.5rem] sm:rounded-[3rem] bg-gradient-to-r from-slate-900 via-indigo-900 to-slate-900 p-8 sm:p-12 text-white shadow-2xl mb-8">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6 sm:gap-8">
          <div className="space-y-3">
            ...
            <p className="text-indigo-100 text-sm sm:text-base font-bold opacity-90 max-w-2xl">إدارة الكوادر القيادية، الأخصائيين، والموظفين الماليين الذين يشكلون العمود الفقري للمدرسة.</p>
          </div>
                  ...
                  {staffInCat.map(member => (
                    <div key={member.id} className={`bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 hover:shadow-xl hover:border-${styles.color}-300 transition-all duration-300 group`}>
                      ...
                      <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => openModal(member)} className="flex-1 py-2 bg-slate-50 hover:bg-indigo-50 text-slate-600 hover:text-indigo-600 text-xs font-black rounded-xl transition-colors flex items-center justify-center gap-1"><Edit className="w-3.5 h-3.5"/> تعديل</button>
      ...
      <AnimatePresence>
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-[3rem] w-full max-w-2xl shadow-2xl overflow-hidden my-8 border border-white/20">
              <div className="p-8 sm:p-10 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <div>
                  <h2 className="text-2xl font-black text-slate-900 flex items-center gap-3">
                    ...
                    {isEditing ? 'تعديل بيانات الكادر' : 'تسجيل كادر جديد'}
                  </h2>
                  <p className="text-sm font-bold text-slate-500 mt-2">قم بتحديد الفئة الوظيفية والصلاحيات بدقة.</p>
                </div>
              ...
              <form onSubmit={handleSubmit} className="p-8 sm:p-10 space-y-6">
                ...
                <div className="p-6 bg-indigo-50/50 rounded-3xl border border-indigo-100 space-y-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-black text-indigo-700 uppercase flex items-center gap-2"><MapPin className="w-4 h-4"/> النطاق والصلاحية</label>
                ...
                <div className="pt-6 border-t border-slate-100 flex flex-col sm:flex-row-reverse gap-3">
                  <button type="submit" disabled={submitting} className="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white font-black text-base px-10 py-4 rounded-2xl shadow-xl shadow-indigo-200 transition-all active:scale-95 disabled:opacity-50">
