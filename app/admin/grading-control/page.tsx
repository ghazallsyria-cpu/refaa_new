...
import { ShieldAlert, Activity, Users, CheckCircle2, Clock, Loader2, Power, AlertCircle, RefreshCw } from 'lucide-react';
    ...
    try {
      ...
      if (teacherSections && users && sections && subjects) {
        ...
        setStats({
          ...
          pending: progressArray.length - submittedCount
        });
      }
    } catch (error) {
  ...
  const handleToggle = async (field: string, currentValue: boolean) => {
    ...
    try {
      ...
      const dbFieldMap: Record<string, string> = {
        'p1_cw_active': 'grading_p1_cw_active',
        'p1_ex_active': 'grading_p1_ex_active',
        'p2_cw_active': 'grading_p2_cw_active',
        'p2_ex_active': 'grading_p2_ex_active',
      };

      const { error } = await supabase
        .from('school_settings')
        .update({ [dbFieldMap[field]]: newValue })
        .eq('id', settings.id);

      if (error) throw error;

      setSettings(prev => ({ ...prev, [field]: newValue }));
      setStatus({ type: 'success', msg: 'تم تحديث صلاحيات الإدخال بنجاح وتطبيقها على جميع المعلمين!' });
    } catch (error) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="glass-panel p-6 rounded-[2rem] border border-white/10 bg-[#0f1423]/80">
              <h2 className="text-xl font-black text-white mb-6 flex items-center gap-2">
                <Power className="w-5 h-5 text-indigo-400" /> قواطع الإدخال
              </h2>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 bg-black/40 rounded-xl border border-white/5">
                  ...
                  <button 
                    ...
                    onClick={() => handleToggle('p1_cw_active', settings.p1_cw_active)}
                    className={`relative w-12 h-6 rounded-full transition-colors ${settings.p1_cw_active ? 'bg-emerald-500' : 'bg-slate-700'}`}
                  >
                ...
                <div className="flex items-center justify-between p-4 bg-black/40 rounded-xl border border-white/5">
                  ...
                  <button 
                    disabled={toggleLoading}
                    onClick={() => handleToggle('p1_ex_active', settings.p1_ex_active)}
                    className={`relative w-12 h-6 rounded-full transition-colors ${settings.p1_ex_active ? 'bg-emerald-500' : 'bg-slate-700'}`}
                  >
                ...
                <div className="flex items-center justify-between p-4 bg-black/40 rounded-xl border border-white/5">
                  ...
                  <button 
                    disabled={toggleLoading}
                    onClick={() => handleToggle('p2_cw_active', settings.p2_cw_active)}
                    className={`relative w-12 h-6 rounded-full transition-colors ${settings.p2_cw_active ? 'bg-emerald-500' : 'bg-slate-700'}`}
                  >
                ...
                <div className="flex items-center justify-between p-4 bg-black/40 rounded-xl border border-white/5">
                  ...
                  <button 
                    disabled={toggleLoading}
                    onClick={() => handleToggle('p2_ex_active', settings.p2_ex_active)}
                    className={`relative w-12 h-6 rounded-full transition-colors ${settings.p2_ex_active ? 'bg-emerald-500' : 'bg-slate-700'}`}
                  >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="glass-panel p-5 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 text-center">
                ...
                <p className="text-xs font-bold text-slate-300">كشوفات اعتُمدت</p>
              </div>
          ...
          <div className="lg:col-span-2">
            <div className="glass-panel p-6 sm:p-8 rounded-[2rem] border border-white/10 bg-[#0f1423]/80 h-full">
              <h2 className="text-xl font-black text-white mb-6 flex items-center gap-2">
                <Activity className="w-5 h-5 text-amber-400" /> رادار إنجاز المعلمين
              </h2>
                ...
                <table className="w-full text-right">
                  <thead>
                    <tr className="border-b border-white/10">
                      ...
                      <th className="p-3 text-slate-400 font-bold text-sm">المادة</th>
                      <th className="p-3 text-slate-400 font-bold text-sm">الصف والشعبة</th>
                      <th className="p-3 text-slate-400 font-bold text-sm text-center">الحالة</th>
                    </tr>
                  </thead>
                  <tbody>
                    {teacherProgress.length > 0 ? (
                      teacherProgress.map((item) => (
                        <tr key={item.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                          ...
                          <td className="p-4 text-center">
                            {item.isSubmitted ? (
                              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/10 text-emerald-400 text-xs font-black rounded-lg border border-emerald-500/20">
                                <CheckCircle2 className="w-4 h-4" /> تم الاعتماد
                              </span>
                            ) : (
                              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-500/10 text-amber-400 text-xs font-black rounded-lg border border-amber-500/20">
                                <Clock className="w-4 h-4" /> قيد الإدخال
                              </span>
                            )}
                          </td>
                        </tr>
                      ...
                      <tr>
                        <td colSpan={4} className="p-10 text-center text-slate-500 font-bold">
                          لا يوجد تكليفات مسجلة في التشكيلات المدرسية للمرحلة الثانوية.
                        </td>
                      </tr>
                    )}
                  </tbody>
