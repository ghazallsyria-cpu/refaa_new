...
export default function AdminExcusesPage() {
  ...
  // 🚀 حالة جديدة لحفظ التواريخ التي حددها المدير للاعتماد
  ...
  // 🚀 فتح النافذة وتحميل التواريخ المطلوبة تلقائياً في مربعات الاختيار
  const openExcuseModal = (excuse: any) => {
    setSelectedExcuse(excuse);
    setRejectNote('');
    // استخراج التواريخ ووضعها افتراضياً كمختارة
  ...
  const handleApprove = async (excuse: any) => {
    if (selectedDatesToApprove.length === 0) {
      alert('يرجى تحديد تاريخ واحد على الأقل ليتم اعتماده.');
      return;
    }
    ...
    // 🚀 نرسل التواريخ المحددة من الإدارة إلى الهوك
  ...
  return (
    <div className="min-h-screen bg-[#090b14] text-slate-200 pb-32 font-cairo" dir="rtl">
        ...
        <div className="bg-[#131836]/60 backdrop-blur-2xl p-8 sm:p-10 rounded-[3rem] border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              ...
              <p className="text-slate-400 font-bold mt-3 text-sm sm:text-base">راجع تقارير الغياب، استعلم عن التواريخ واعتمدها ليقوم النظام بتحديث السجل الأم وإشعار الطالب.</p>
            </div>
      ...
      {/* 🚀 نافذة مراجعة العذر (Modal) - مع حل تجمد الموبايل */}
      <AnimatePresence>
        {selectedExcuse && (
          <Dialog.Root open={!!selectedExcuse} onOpenChange={(open) => !open && setSelectedExcuse(null)}>
            <Dialog.Portal>
              ...
              {/* 🚀 هنا تم إضافة overflow-y-auto للنافذة الرئيسية لمنع التجميد على الجوال */}
              <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#131836] border border-white/10 rounded-[2.5rem] w-[95%] max-w-4xl max-h-[85vh] overflow-y-auto custom-scrollbar shadow-[0_0_60px_rgba(0,0,0,0.9)] z-50 flex flex-col md:flex-row" dir="rtl">
                ...
                <div className="md:w-1/2 p-6 sm:p-8 flex flex-col bg-[#131836]">
                  ...
                  <div className="space-y-6 flex-1">
                    ...
                    {/* 🚀 قسم مربعات الاختيار (الاستعلام والاعتماد) للإدارة */}
