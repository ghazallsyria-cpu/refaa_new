...
export default function QuizBuilder() {
  ...
  const addQuestion = useCallback((type: QuestionType | 'file' | 'file_upload') => {
    const newQuestion: any = { ...createQuestion(type as QuestionType), type: type, is_required: true };
    if (type === 'true_false') {
  ...
  useEffect(() => { setIsAdmin(userRole === 'admin' || userRole === 'management'); }, [userRole]);
  ...
  const fetchInitialData = useCallback(async () => {
    try {
      if (!isNew) {
        ...
        setExam({
          ...examData, section_ids: examData.section_ids || [],
          settings: { shuffle_questions: fetchedSettings.shuffle_questions ?? false, shuffle_options: fetchedSettings.shuffle_options ?? false, show_results_immediately: fetchedSettings.show_results_immediately ?? true, allow_backtracking: fetchedSettings.allow_backtracking ?? true, prevent_tab_switch: fetchedSettings.prevent_tab_switch ?? false, prevent_copy: fetchedSettings.prevent_copy ?? true }
        });
  ...
  const updateQuestion = (id: string, updates: Partial<any>) => { setQuestions(questions.map(q => q.id === id ? { ...q, ...updates } : q)); };
  ...
  const duplicateQuestion = (id: string) => {
    const question = questions.find(q => q.id === id);
    if (question) {
      const duplicated = { ...question, id: crypto.randomUUID(), options: question.options?.map((o: any) => ({ ...o, id: crypto.randomUUID() })) || [] };
  ...
  const addOption = (questionId: string) => {
    setQuestions(questions.map(q => {
      if (q.id === questionId) return { ...q, options: [...(q.options || []), { id: crypto.randomUUID(), content: '', is_correct: false }] };
      return q;
    }));
  };
    ...
    setQuestions(questions.map(q => {
      if (q.id === questionId) {
        const newOptions = (q.options || []).map((o: any) => {
          if (o.id === optionId) {
            if (updates.is_correct && (q.type === 'multiple_choice' || q.type === 'true_false')) return { ...o, ...updates };
            return { ...o, ...updates };
          }
          if (updates.is_correct && (q.type === 'multiple_choice' || q.type === 'true_false')) return { ...o, is_correct: false };
          return o;
        });
  ...
  const deleteOption = (questionId: string, optionId: string) => {
    setQuestions(questions.map(q => {
      if (q.id === questionId) return { ...q, options: (q.options || []).filter((o: any) => o.id !== optionId) };
      return q;
    }));
  };

  const handleSave = async () => {
    if (!exam.title || !exam.subject_id) { showNotification('error', 'يرجى إدخال عنوان الاختبار والمادة'); return; }
    const totalPoints = questions.reduce((sum, q) => sum + (Number(q.points) || 0), 0);
    const maxScore = Number(exam.max_score) || 0;
    if (totalPoints !== maxScore) { showNotification('error', `مجموع درجات الأسئلة (${totalPoints}) يجب أن يساوي الدرجة الكلية للاختبار (${maxScore})`); return; }

    setSaving(true);
    try {
      let finalTeacherId = exam.teacher_id;
      if (userRole === 'teacher' && user) finalTeacherId = user.id;
      ...
      const examPayload = { ...exam, teacher_id: finalTeacherId, max_score: maxScore, total_marks: maxScore };
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 pb-24 relative">
      <AnimatePresence>
        {notification && (
          <motion.div initial={{ opacity: 0, y: -20, x: '-50%' }} animate={{ opacity: 1, y: 0, x: '-50%' }} exit={{ opacity: 0, y: -20, x: '-50%' }} className={`fixed top-6 left-1/2 z-50 px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-4 border backdrop-blur-md ${ notification.type === 'success' ? 'bg-emerald-50/90 text-emerald-800 border-emerald-200' : 'bg-red-50/90 text-red-800 border-red-200' }`}>
            <div className="h-8 w-8 rounded-full bg-white/50 flex items-center justify-center">{notification.type === 'success' ? <Check className="h-5 w-5" /> : <AlertCircle className="h-5 w-5" />}</div>
      ...
      <header className="sticky top-0 z-40 glass-card border-b border-white/60 px-6 py-4 shadow-xl shadow-slate-200/20">
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            <button onClick={() => router.back()} className="h-12 w-12 flex items-center justify-center rounded-2xl bg-white border border-slate-100 text-slate-500 hover:text-indigo-600 hover:shadow-lg transition-all active:scale-95"><ArrowRight className="h-5 w-5" /></button>
            <div className="hidden sm:block space-y-0.5">
              <h1 className="text-xl font-black text-slate-900 tracking-tight truncate max-w-[300px]">{exam.title || 'اختبار جديد'}</h1>
              <div className="flex items-center gap-2"><div className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" /><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">جاري الحفظ تلقائياً</p></div>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className={`hidden lg:flex items-center gap-3 px-5 py-3 rounded-2xl border font-black transition-all ${ questions.reduce((sum, q) => sum + (Number(q.points) || 0), 0) === (Number(exam.max_score) || 0) ? 'bg-emerald-50/50 border-emerald-100 text-emerald-600' : 'bg-amber-50/50 border-amber-100 text-amber-600' }`}>
            ...
            <Dialog.Root>
              <Dialog.Trigger asChild><button className="h-12 w-12 flex items-center justify-center rounded-2xl bg-white border border-slate-100 text-slate-600 hover:text-indigo-600 hover:shadow-lg transition-all active:scale-95"><Settings className="h-5 w-5" /></button></Dialog.Trigger>
              <Dialog.Portal>
                ...
                <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-white rounded-[40px] p-10 shadow-2xl z-50 animate-in fade-in zoom-in-95 duration-300 border border-white/20" dir="rtl">
                  <div className="flex items-center justify-between mb-10">
                    <div><Dialog.Title className="text-3xl font-black text-slate-900 tracking-tight">إعدادات الاختبار</Dialog.Title><p className="text-slate-500 font-bold">تخصيص تجربة الاختبار للطلاب</p></div>
                  ...
                  <div className="space-y-10">
                      ...
                      <div className="space-y-4">
                        {[
                          ...
                          <div key={setting.key} className="flex items-center justify-between p-4 rounded-3xl bg-slate-50/50 border border-slate-100">
                            <div><p className="text-sm font-black text-slate-800 tracking-tight">{setting.label}</p><p className="text-xs text-slate-500 font-bold">{setting.desc}</p></div>
                            <Switch.Root checked={!!(exam.settings as any)?.[setting.key]} onCheckedChange={(val) => setExam({ ...exam, settings: { ...(exam.settings || {}), [setting.key]: val } as any })} className="w-14 h-8 bg-slate-200 rounded-full relative data-[state=checked]:bg-indigo-600 transition-all outline-none cursor-pointer shadow-inner">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                      <div className="space-y-3"><label className="text-xs font-black text-slate-400 uppercase tracking-widest">مدة الاختبار</label><div className="relative"><Clock className="absolute right-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400" /><input type="number" value={exam.duration} onChange={(e) => setExam({...exam, duration: parseInt(e.target.value) || 0})} className="w-full pr-12 pl-4 py-4 rounded-2xl bg-slate-50 border-0 ring-1 ring-inset ring-slate-100 focus:ring-2 focus:ring-indigo-600 outline-none font-black text-slate-900 transition-all" /></div></div>
                      <div className="space-y-3"><label className="text-xs font-black text-slate-400 uppercase tracking-widest">عدد المحاولات</label><div className="relative"><HelpCircle className="absolute right-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400" /><input type="number" value={exam.max_attempts} onChange={(e) => setExam({...exam, max_attempts: parseInt(e.target.value) || 0})} className="w-full pr-12 pl-4 py-4 rounded-2xl bg-slate-50 border-0 ring-1 ring-inset ring-slate-100 focus:ring-2 focus:ring-indigo-600 outline-none font-black text-slate-900 transition-all" /></div></div>
                      <div className="space-y-3"><label className="text-xs font-black text-slate-400 uppercase tracking-widest">الدرجة الكلية</label><div className="relative"><Hash className="absolute right-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400" /><input type="number" value={exam.max_score} onChange={(e) => setExam({...exam, max_score: parseInt(e.target.value) || 0})} className="w-full pr-12 pl-4 py-4 rounded-2xl bg-slate-50 border-0 ring-1 ring-inset ring-slate-100 focus:ring-2 focus:ring-indigo-600 outline-none font-black text-slate-900 transition-all" /></div></div>
                    </div>
                  </div>
                </Dialog.Content>
              </Dialog.Portal>
            </Dialog.Root>
            <button onClick={() => setExam({...exam, status: exam.status === 'published' ? 'draft' : 'published'})} className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-black transition-all active:scale-95 border ${exam.status === 'published' ? 'bg-emerald-50 text-emerald-600 border-emerald-200 hover:bg-emerald-100' : 'bg-amber-50 text-amber-600 border-amber-200 hover:bg-amber-100'}`}><CheckCircle className="h-5 w-5" /> <span>{exam.status === 'published' ? 'منشور' : 'مسودة'}</span></button>
            <button onClick={handleSave} disabled={saving} className="flex items-center gap-3 bg-indigo-600 text-white px-8 py-3 rounded-2xl hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-200 font-black disabled:opacity-50 active:scale-95">{saving ? <div className="h-5 w-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : <Save className="h-5 w-5" />}<span>{exam.status === 'published' ? 'حفظ ونشر' : 'حفظ كمسودة'}</span></button>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-6 py-12 space-y-10">
        <div className="glass-card rounded-[40px] border-t-[16px] border-t-indigo-600 border border-white/60 shadow-2xl shadow-slate-200/50 p-10 space-y-8 relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-10 border-t border-slate-100">
            {isAdmin && (
              <div className="space-y-3"><label className="text-xs font-black text-slate-400 uppercase tracking-widest block">المعلم</label><select value={exam.teacher_id || ''} onChange={(e) => setExam({...exam, teacher_id: e.target.value})} className="w-full px-5 py-4 rounded-2xl bg-slate-50 border-0 ring-1 ring-inset ring-slate-100 focus:ring-2 focus:ring-indigo-600 outline-none font-bold text-slate-700 transition-all appearance-none cursor-pointer"><option value="">اختر المعلم</option>{teachers.map((t: any) => <option key={t.id} value={t.id}>{t.full_name}</option>)}</select></div>
            )}
            <div className="space-y-3"><label className="text-xs font-black text-slate-400 uppercase tracking-widest block">الدرجة العظمى</label><input type="number" value={exam.max_score} onChange={(e) => setExam({...exam, max_score: parseInt(e.target.value) || 0})} className="w-full px-5 py-4 rounded-2xl bg-slate-50 border-0 ring-1 ring-inset ring-slate-100 focus:ring-2 focus:ring-indigo-600 outline-none font-bold text-slate-700 transition-all appearance-none" /></div>
            <div className="space-y-3"><label className="text-xs font-black text-slate-400 uppercase tracking-widest block">تاريخ الاختبار</label><input type="date" value={exam.exam_date} onChange={(e) => setExam({...exam, exam_date: e.target.value})} className="w-full px-5 py-4 rounded-2xl bg-slate-50 border-0 ring-1 ring-inset ring-slate-100 focus:ring-2 focus:ring-indigo-600 outline-none font-bold text-slate-700 transition-all appearance-none cursor-pointer" /></div>
            <div className="space-y-3"><label className="text-xs font-black text-slate-400 uppercase tracking-widest block">وقت البداية</label><input type="time" value={exam.start_time} onChange={(e) => setExam({...exam, start_time: e.target.value})} className="w-full px-5 py-4 rounded-2xl bg-slate-50 border-0 ring-1 ring-inset ring-slate-100 focus:ring-2 focus:ring-indigo-600 outline-none font-bold text-slate-700 transition-all appearance-none cursor-pointer" /></div>
            <div className="space-y-3"><label className="text-xs font-black text-slate-400 uppercase tracking-widest block">وقت النهاية</label><input type="time" value={exam.end_time} onChange={(e) => setExam({...exam, end_time: e.target.value})} className="w-full px-5 py-4 rounded-2xl bg-slate-50 border-0 ring-1 ring-inset ring-slate-100 focus:ring-2 focus:ring-indigo-600 outline-none font-bold text-slate-700 transition-all appearance-none cursor-pointer" /></div>
            <div className="space-y-3"><label className="text-xs font-black text-slate-400 uppercase tracking-widest block">المادة</label><select value={exam.subject_id} onChange={(e) => setExam({...exam, subject_id: e.target.value})} className="w-full px-5 py-4 rounded-2xl bg-slate-50 border-0 ring-1 ring-inset ring-slate-100 focus:ring-2 focus:ring-indigo-600 outline-none font-bold text-slate-700 transition-all appearance-none cursor-pointer"><option value="">اختر المادة</option>{subjects.map((s: any) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
            <div className="space-y-3 md:col-span-3"><label className="text-xs font-black text-slate-400 uppercase tracking-widest block">الشعب المستهدفة</label><div className="grid grid-cols-2 md:grid-cols-4 gap-3"><label className="flex items-center gap-2 p-3 rounded-xl bg-slate-50 border border-slate-100 cursor-pointer"><input type="checkbox" checked={exam.section_ids?.length === sections.length && sections.length > 0} onChange={(e) => setExam({...exam, section_ids: e.target.checked ? sections.map((s: any) => s.id) : []})} className="rounded text-indigo-600 focus:ring-indigo-500" /><span className="text-sm font-bold text-slate-700">الجميع</span></label>{sections.map((s: any) => (<label key={s.id} className="flex items-center gap-2 p-3 rounded-xl bg-slate-50 border border-slate-100 cursor-pointer"><input type="checkbox" checked={exam.section_ids?.includes(s.id)} onChange={(e) => setExam({...exam, section_ids: e.target.checked ? [...(exam.section_ids || []), s.id] : (exam.section_ids || []).filter(id => id !== s.id)})} className="rounded text-indigo-600 focus:ring-indigo-500" /><span className="text-sm font-bold text-slate-700 truncate">{s.name}</span></label>))}</div></div>
          </div>
        </div>
                    ...
                    <div className="w-full md:w-64 space-y-3">
                      ...
                      <select 
                        ...
                        onChange={(e) => {
                           ...
                           if (['multiple_choice', 'checkbox', 'true_false', 'multi_select'].includes(type as string)) {
                              if (!q.options || q.options.length === 0) {
                                  updates.options = type === 'true_false' 
                                    ? [{ id: crypto.randomUUID(), content: 'صح', is_correct: true }, { id: crypto.randomUUID(), content: 'خطأ', is_correct: false }]
