...
import { 
  ...
  UserMinus, CalendarDays, FileText
} from 'lucide-react';
      ...
      return (
        <div className="min-h-screen bg-rose-50 p-6 flex flex-col items-center justify-center font-cairo" dir="rtl">
          <div className="bg-white p-8 rounded-3xl shadow-2xl w-full max-w-2xl border-4 border-rose-500 text-center">
            ...
            <p className="text-slate-600 font-bold mb-6 text-sm">تم اصطياد الخطأ، الرجاء تصوير هذه الشاشة للمبرمج:</p>
...
// =========================================================================
// 2. 🚀 التطبيق الرئيسي (لجان الامتحانات والإحصائيات)
// =========================================================================
function ExamCommitteesControl() {
  ...
  useEffect(() => {
     if(uniqueExamDates.length > 0 && !activeExamDate) {
        setActiveExamDate(uniqueExamDates[0]);
        fetchHeadsByDate(uniqueExamDates[0]);
     }
  }, [uniqueExamDates]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 🚀 جلب تاريخ الامتحان مع بيانات المراقبة
  ...
  const handleToggleExemption = async (tId: string, currentStatus: boolean) => {
    try {
       ...
       alert('حدث خطأ. تأكد من إضافة عمود is_excluded_from_exams في جدول teachers.');
       fetchData(); 
    }
  };
  ...
  // 🚀 إضافة المراقب مع دمج تاريخ الاختبار
  const handleAddInvigilator = async () => {
    ...
    // فحص هل المعلم يراقب في لجنة أخرى نفس اليوم؟
    ...
    try { 
      await supabase.from('committee_invigilators').insert({ 
         ...
         exam_date: activeExamDate, // 🚀 حفظ التاريخ
         status: 'pending' 
      }); 
      setIsAssignModalOpen(false); setSelectedTeacherId(''); setTeacherSearchTerm(''); fetchData(); 
    } catch (error) { alert('خطأ في التكليف، تأكد من إضافة عمود exam_date في قاعدة البيانات'); }
  };
  ...
  const sortedAndFilteredTeachers = teachers
    ...
    .sort((a, b) => {
       const aCount = getTeacherTotalAssignments(String(a?.id)).length;
       const bCount = getTeacherTotalAssignments(String(b?.id)).length;
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-10 font-cairo pb-24" dir="rtl">
      ...
      <div className="max-w-7xl mx-auto space-y-6 relative">
        <div className="bg-white rounded-[2rem] p-6 sm:p-8 shadow-sm border border-slate-200">
          ...
          {/* 🚀 فلتر اليوم الامتحاني (يظهر فقط إذا كان هناك تواريخ) */}
          ...
          {/* 🚀 الإحصائية اليومية المجمعة (الجديدة) */}
            ...
            <div className="flex-1 overflow-hidden flex flex-col min-h-[400px]">
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2 pr-2 border border-slate-100 rounded-2xl p-2 bg-slate-50/50">
                 {teachers.filter(t => String(t?.full_name || '').includes(teacherSearchTerm) || String(t?.subjectsStr || '').includes(teacherSearchTerm)).map((t, index) => {
                    ...
                    // 🚀 فلترة مهام هذا المعلم لليوم المحدد فقط
