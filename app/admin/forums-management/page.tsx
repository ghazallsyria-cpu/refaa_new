    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-24 p-4 sm:p-6 lg:p-8 font-sans" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
        ...
        <div className="flex gap-3">
           <Link href="/forums" className="px-6 py-4 rounded-2xl bg-slate-50 text-slate-600 font-black text-sm flex items-center gap-2 hover:bg-slate-100 transition-all border border-slate-200">
             <ArrowLeft className="w-4 h-4" /> معاينة المنتدى
           </Link>
           <button onClick={() => handleOpenCatModal()} className="px-6 py-4 rounded-2xl bg-indigo-600 text-white font-black text-sm flex items-center gap-2 shadow-xl shadow-indigo-200 hover:bg-indigo-700 active:scale-95 transition-all">
             <Plus className="w-5 h-5" /> قسم رئيسي جديد
           </button>
        </div>
      </div>
      ...
      <div className="flex gap-2 p-1.5 bg-white border border-slate-200 rounded-3xl w-fit shadow-sm">
        <button onClick={() => setActiveTab('structure')} className={`px-8 py-3 rounded-2xl font-black text-sm transition-all flex items-center gap-2 ${activeTab === 'structure' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:bg-slate-50'}`}>
          <FolderTree className="w-4 h-4" /> هيكل المنتديات
        </button>
        <button onClick={() => setActiveTab('library')} className={`px-8 py-3 rounded-2xl font-black text-sm transition-all flex items-center gap-2 ${activeTab === 'library' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:bg-slate-50'}`}>
          <ImageIcon className="w-4 h-4" /> مكتبة الوسائط
        </button>
      </div>
          ...
          <div className="space-y-6">
            {loading ? (
              ...
              <div className="text-center py-24 bg-white rounded-[3rem] border-2 border-dashed border-slate-200">
                <LayoutGrid className="w-20 h-20 text-slate-200 mx-auto mb-4" />
                <p className="text-slate-400 font-bold">لا توجد أقسام مبرمجة حالياً.</p>
              </div>
            ) : (
              structuredCategories.map((main) => (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} key={main.id} className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden">
                  <div className="p-6 bg-slate-50/80 border-b border-slate-200 flex items-center justify-between gap-4">
                      ...
                      <div key={sub.id} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-between group hover:bg-white hover:border-indigo-200 hover:shadow-md transition-all">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-xl overflow-hidden bg-white border border-slate-200 shrink-0 relative flex items-center justify-center">
                             {sub.icon ? <img src={sub.icon} alt={sub.name} className="w-full h-full object-cover" /> : <Hash className="w-5 h-5 text-slate-300"/>}
                          </div>
                          <div className="min-w-0">
                            <h4 className="font-black text-slate-800 text-sm truncate">{sub.name}</h4>
                            <div className="flex gap-1.5 mt-1">
                               {sub.target_classes && sub.target_classes.length > 0 ? <Target className="w-3 h-3 text-amber-500" /> : <Globe className="w-3 h-3 text-emerald-500" />}
                               <span className="text-[9px] font-bold text-slate-400">
                                 {sub.target_classes && sub.target_classes.length > 0 ? 'فصول محددة' : 'متاح للجميع'}
                               </span>
                            </div>
                          </div>
                        </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-[3rem] shadow-2xl w-full max-w-4xl border border-white/20 my-auto overflow-hidden">
              ...
              <form onSubmit={handleSaveCategory} className="p-6 sm:p-10 space-y-8 bg-white max-h-[75vh] overflow-y-auto custom-scrollbar">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-6">
                    <div>
                      <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-3 pr-2">اسم القسم</label>
                      <input type="text" required value={catForm.name} onChange={e => setCatForm({...catForm, name: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 font-black outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all" placeholder="مثال: قسم مادة الفيزياء" />
                    </div>

                    <div>
                      <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-3 pr-2">الوصف</label>
                      <textarea rows={3} value={catForm.description} onChange={e => setCatForm({...catForm, description: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 font-bold outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all resize-none" placeholder="نبذة مختصرة عن المواضيع..." />
                    </div>

                    <div>
                      <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-3 pr-2">التصنيف الأب</label>
                      <select value={catForm.parent_id} onChange={e => setCatForm({...catForm, parent_id: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 font-black outline-none appearance-none cursor-pointer">
                        <option value="none">🌟 قسم رئيسي</option>
                        {structuredCategories.filter(m => m.id !== editingCat?.id).map(m => (
                          <option key={m.id} value={m.id}>↳ يتبع لـ: {m.name}</option>
                        ))}
                      </select>
                    </div>
                  </div>

                  {/* Right Column: Icon Logic */}
                  <div className="space-y-6">
                    ...
                    <div className="bg-slate-50 rounded-[2.5rem] border border-slate-200 p-8 flex flex-col items-center justify-center text-center gap-6 relative group/icon">
                       <div className="h-28 w-28 rounded-[2.5rem] bg-white border-2 border-slate-100 shadow-inner flex items-center justify-center relative overflow-hidden">
                          {catForm.icon_url ? <img src={catForm.icon_url} alt="Icon" className="w-full h-full object-cover p-2" /> : <ImageIcon className="w-12 h-12 text-slate-200" />}
                       </div>
                       
                       <div className="flex flex-col gap-3 w-full">
                          <button type="button" onClick={() => setShowAssetPicker(true)} className="w-full py-3.5 bg-indigo-600 text-white font-black text-xs rounded-2xl shadow-lg shadow-indigo-100 flex items-center justify-center gap-2 hover:bg-indigo-700 transition-all active:scale-95">
                             <LayoutGrid className="w-4 h-4" /> اختيار من مكتبة المواد
                          </button>
                          <div className="flex items-center gap-2">
                             <div className="h-px bg-slate-200 flex-1"></div>
                             <span className="text-[10px] font-black text-slate-400 uppercase">أو</span>
                             <div className="h-px bg-slate-200 flex-1"></div>
                          </div>
                          <button type="button" onClick={() => setCatForm({...catForm, icon_url: ''})} className="w-full py-3 bg-white text-rose-500 font-black text-xs rounded-2xl border border-rose-100 hover:bg-rose-50 transition-all">
                   ...
                   <div className="space-y-6">
                      <div>
                        ...
                        <select value={catForm.post_perm} onChange={e => setCatForm({...catForm, post_perm: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 font-black outline-none">
                          <option value="all">الجميع (حسب الفصول المحددة)</option>
                          <option value="teachers_admin">المعلمون والإدارة فقط</option>
                          <option value="admin_only">الإدارة فقط (لأغراض النشر الرسمي)</option>
                        </select>
                      </div>
                      <div>
                ...
                <div className="flex gap-4 pt-8 border-t border-slate-100">
                  <button type="submit" disabled={isSubmitting} className="flex-1 bg-slate-900 hover:bg-black text-white py-5 rounded-[1.5rem] font-black flex justify-center items-center gap-3 shadow-2xl active:scale-95 transition-all">
                      {isSubmitting ? <Loader2 className="animate-spin w-6 h-6" /> : <Save className="w-6 h-6" />} {editingCat ? 'حفظ التغييرات النهائية' : 'اعتماد وبناء القسم'}
                  </button>
      ...
      {/* 🚀 Asset Picker Overlay (Internal Library Browser) */}
      <AnimatePresence>
        {showAssetPicker && (
          <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 sm:p-12 bg-indigo-950/80 backdrop-blur-xl">
             <motion.div initial={{ opacity: 0, y: 100 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-[3rem] shadow-2xl w-full max-w-4xl p-8 sm:p-12 overflow-hidden flex flex-col h-[80vh]">
                <div className="flex justify-between items-center mb-10 border-b border-slate-100 pb-6">
                   <div>
                      <h3 className="text-2xl font-black text-slate-900 tracking-tight">اختر أيقونة من المكتبة</h3>
                      <p className="text-sm font-bold text-slate-500">اختر الصورة التي تريد ربطها بهذا القسم.</p>
                   </div>
                   <button onClick={() => setShowAssetPicker(false)} className="w-12 h-12 bg-slate-50 rounded-full flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-all shadow-sm border border-slate-100"><XCircle className="w-7 h-7" /></button>
                </div>
                <div className="flex-1 overflow-y-auto pr-4 custom-scrollbar">
                   <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-6">
                      {assets.map(asset => (
                        <div 
                           key={asset.id} 
                           onClick={() => { setCatForm({...catForm, icon_url: asset.url}); setShowAssetPicker(false); }}
                           className={`group flex flex-col items-center cursor-pointer transition-all ${catForm.icon_url === asset.url ? 'scale-105' : 'hover:scale-105'}`}
                        >
                           <div className={`w-full aspect-square rounded-3xl bg-slate-50 border-4 transition-all flex items-center justify-center relative ${catForm.icon_url === asset.url ? 'border-indigo-600 ring-4 ring-indigo-50 shadow-xl shadow-indigo-100' : 'border-white group-hover:border-indigo-100 shadow-sm'}`}>
          ...
          <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg p-8 sm:p-10 border border-white/20">
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-xl font-black text-slate-900">إضافة أصل جديد للمكتبة</h2>
              ...
              <form onSubmit={handleSaveAsset} className="space-y-6">
                ...
                <div>
                  ...
                  <ImageUpload onUploadSuccess={(url) => setAssetForm({...assetForm, url})} initialImageUrl={assetForm.url} label="ارفع شعار المادة الرسمي" />
                </div>
                <button type="submit" disabled={isSubmitting || !assetForm.url} className="w-full bg-indigo-600 text-white py-4 rounded-xl font-black shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
                  {isSubmitting ? <Loader2 className="animate-spin w-5 h-5" /> : <Save className="w-5 h-5" />} حفظ في المكتبة
                </button>
              </form>
            </motion.div>
          </div>
