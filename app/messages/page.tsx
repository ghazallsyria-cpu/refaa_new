...
export default function MessagesPage() {
  ...
  useEffect(() => {
    ...
    // 🚀 1. العد السلبي للجروبات (بدون أي طلبات للسيرفر)
  ...
  useEffect(() => {
    if (activeThread && messages.length >= 0) {
      ...
      // 🚀 تمت إزالة دالة markAsRead من هنا بالكامل لمنع أي حلقة مفرغة (Infinite Loop)
  ...
  // 🛡️ دالة مسؤولة عن فتح المحادثة وتحديد الرسائل كمقروءة فقط بضغطة زر
  const handleOpenThread = (thread: any, type: 'group' | 'private') => {
    setActiveThread(thread);
    
    // فلترة الرسائل غير المقروءة لهذه المحادثة فقط
    ...
    // تحديث قاعدة البيانات مرة واحدة فقط عند الضغط
  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-5rem)] lg:h-[calc(100dvh-6rem)] max-w-[1600px] mx-auto font-cairo text-slate-200 relative overflow-hidden" dir="rtl">
      ...
      <div className={cn("glass-panel overflow-hidden flex flex-1 min-h-0 mx-0 lg:mx-8 relative z-10 bg-[#0f1423]/60", activeThread ? "rounded-none lg:rounded-[2.5rem] lg:border lg:border-white/10 lg:shadow-[0_20px_50px_rgba(0,0,0,0.5)]" : "rounded-t-[2.5rem] lg:rounded-[2.5rem] border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)]")}>
        ...
        {/* 🚀 نافذة الدردشة - إصلاح تصميم الجوال */}
        <div className={cn("flex-col transition-all duration-300", !activeThread ? "hidden lg:flex lg:flex-1 items-center justify-center bg-[#090b14] lg:bg-transparent z-[70] lg:z-auto" : "flex fixed inset-0 z-[100] bg-[#090b14] lg:static lg:flex-1 h-[100dvh] lg:h-auto overflow-hidden")}>
           {!activeThread ? (
