...
import { 
  ...
  ArrowLeft, Star, Users, Crown, Compass, Newspaper, Video, ChevronLeft, BellRing, Megaphone, ArrowUpRight
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#02040a] text-slate-200 font-cairo overflow-x-hidden selection:bg-amber-500 selection:text-slate-900" dir="rtl">
          ...
          <div className="flex overflow-x-auto gap-8 px-4 sm:px-6 lg:px-8 pb-16 snap-x snap-mandatory hide-scrollbar" dir="rtl">
            {studioItems.map((media, index) => (
              <motion.div 
                ...
                key={media.id} 
                className="relative shrink-0 w-[85vw] sm:w-[600px] h-[400px] sm:h-[500px] rounded-[3rem] overflow-hidden snap-center group border border-white/10 bg-[#0f1423] shadow-2xl"
              >
                {playingVideoId === media.id ? (
                  <div className="absolute inset-0 z-40 bg-black rounded-[3rem] overflow-hidden">
                    <button onClick={(e) => { e.stopPropagation(); setPlayingVideoId(null); }} className="absolute top-6 left-6 z-50 w-12 h-12 bg-black/40 hover:bg-rose-500 text-white rounded-full flex items-center justify-center backdrop-blur-md transition-colors border border-white/20 shadow-xl">✕</button>
                  ...
                  <div className="absolute inset-0 w-full h-full cursor-pointer" onClick={() => { if (media.media_type === 'video') setPlayingVideoId(media.id); }}>
                    ...
                    <div className="absolute top-8 left-8 z-10 px-5 py-2.5 rounded-2xl bg-black/50 backdrop-blur-xl border border-white/10 text-white text-xs font-black flex items-center gap-2 shadow-xl pointer-events-none">
                    ...
                    <div className="absolute bottom-0 left-0 w-full p-8 sm:p-12 z-10 translate-y-4 group-hover:translate-y-0 transition-transform duration-500 pointer-events-none">
        ...
        <section className="py-32 relative z-10 bg-[#02040a] border-t border-white/5">
          ...
          <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="flex items-center gap-6 mb-20 border-b border-white/5 pb-10">
              ...
              <div>
                <h2 className="text-4xl sm:text-6xl font-black text-white tracking-tight">المجلة <span className="text-transparent bg-clip-text bg-gradient-to-l from-emerald-400 to-teal-200">الأكاديمية</span></h2>
      ...
      <section className="py-32 relative z-10 bg-[#02040a] border-t border-white/5 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
           ...
           <p className="text-xl text-slate-400 font-bold mb-12">انضم الآن إلى مجتمع مدرستك في منصة التعليم الرقمي الأقوى.</p>
