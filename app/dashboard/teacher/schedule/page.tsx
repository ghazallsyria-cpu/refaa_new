...
import { Calendar, Clock, BookOpen, Users, Zap, ArrowRight, Loader2, AlertCircle } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion'; // 🚀 تم إصلاح مسار الاستيراد
import { format } from 'date-fns';
import { arSA } from 'date-fns/locale'; // 🚀 استيراد اللغة العربية لتنسيق الوقت (ص/م)
...
export default function TeacherSchedulePage() {
  ...
  const fetchTeacherSchedule = useCallback(async () => {
    // 🚀 لا نطلب البيانات إلا بعد التأكد من أن المستخدم معلم أو إداري
  ...
  return (
    ...
    >
      {/* 🚀 زر العودة */}
      <div className="mb-2">
        <Link href="/dashboard/teacher" className="flex items-center gap-2 text-slate-500 hover:text-indigo-600 font-bold bg-white/80 backdrop-blur-md px-5 py-2.5 rounded-2xl shadow-sm border border-slate-200 transition-all w-fit group">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 mt-1 font-bold">عرض كامل للحصص الدراسية المسندة إليك خلال الأسبوع</p>
          </div>
        </div>
        <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-100 shadow-sm w-full md:w-auto">
          ...
          <div>
            ...
            {/* 🚀 إضافة دعم اللغة العربية للتوقيت (ص/م) */}
      ...
      {periods.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-[3rem] border border-dashed border-slate-300 shadow-sm">
           ...
           <p className="text-slate-500 font-bold">لم تقم الإدارة برفع أوقات الحصص أو إسناد جدول لك حتى الآن.</p>
        </div>
      ) : (
                      ...
                      return (
                        <td key={`${day.id}-${period.id}`} className="p-3 border-l border-slate-100 h-36 align-top min-w-[170px]">
                          {cellData ? (
                            ...
                            >
                              {isCurrent && (
                                <div className="absolute top-0 right-0 p-1.5 bg-white/20 rounded-bl-xl backdrop-blur-sm">
                              ...
                              <div className="space-y-1">
                                <div className={`flex items-center gap-1.5 mb-1.5 ${isCurrent ? 'text-indigo-100' : 'text-slate-400'}`}>
                                  ...
                                  <span className="text-[10px] font-black uppercase tracking-widest">المادة</span>
                                </div>
