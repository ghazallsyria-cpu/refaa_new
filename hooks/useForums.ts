...
import { 
  ...
  Loader2, Sparkles, BookOpen, Layers, Globe, Target, Save, XCircle, Image as ImageIcon, Trash2, Lock, ShieldAlert, Edit2
} from 'lucide-react';
...
export default function ForumsPage() {
  ...
  const [editingCategoryId, setEditingCategoryId] = useState<string | null>(null); // 🚀 حالة التعديل
  ...
  // 🚀 دالة فتح نافذة التعديل مع تعبئة البيانات السابقة
  ...
  const handleCreateCategory = async (e: React.FormEvent) => {
    ...
    if (editingCategoryId) {
      result = await updateCategory(editingCategoryId, payload); // 🚀 تحديث بدلاً من إنشاء
    } else {
      ...
      <div className="relative group h-full">
        {isAdmin && (
            <div className="absolute top-2 left-2 z-20 flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); openEditModal(cat); }} className="bg-white/80 backdrop-blur-sm p-2 rounded-lg text-indigo-500 hover:bg-indigo-100 transition-all shadow-sm border border-indigo-100" title="تعديل القسم">
                    <Edit2 className="w-4 h-4" />
                </button>
                <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleDeleteCategory(cat.id, cat.icon); }} className="bg-white/80 backdrop-blur-sm p-2 rounded-lg text-rose-500 hover:bg-rose-100 transition-all shadow-sm border border-rose-100" title="حذف القسم">
      ...
      <div className="bg-gradient-to-br from-indigo-900 via-blue-900 to-slate-900 pt-12 pb-24 px-4 sm:px-6 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div className="shrink-0 flex flex-col gap-4 w-full md:w-auto">
            ...
            {isAdmin && (
              <button onClick={() => { resetForm(); setIsModalOpen(true); }} className="flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-3.5 rounded-2xl font-black shadow-lg shadow-emerald-500/20 transition-all active:scale-95 border border-emerald-400 text-sm">
              ...
              <div className="flex items-center justify-between mb-6 pb-5 border-b border-slate-100">
                ...
                {isAdmin && (
                    <div className="flex items-center gap-2 shrink-0">
                        <button onClick={() => openEditModal(mainCat)} className="p-3 text-indigo-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-colors border border-transparent hover:border-indigo-100" title="تعديل القسم الرئيسي">
                            <Edit2 className="w-5 h-5" />
                        </button>
                        <button onClick={() => handleDeleteCategory(mainCat.id, mainCat.icon)} className="p-3 text-rose-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-colors border border-transparent hover:border-rose-100" title="حذف القسم الرئيسي">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-[2rem] shadow-2xl w-full max-w-xl border border-slate-100 my-auto">
              <div className="bg-slate-50 p-6 border-b border-slate-100 flex justify-between">
                <div>
                    <h2 className="text-xl font-black text-slate-900">{editingCategoryId ? 'تعديل بيانات القسم' : 'إضافة قسم للمنتدى'}</h2>
                    <p className="text-xs font-bold text-slate-500">{editingCategoryId ? 'قم بتحديث الإعدادات والصلاحيات.' : 'يمكنك إنشاء قسم رئيسي أو تفريعه.'}</p>
                </div>
              ...
              <form onSubmit={handleCreateCategory} className="p-6 space-y-5">
                ...
                {/* 🚀 القائمة المنسدلة بدون أي تغيير في منطقك الأساسي الأصلي */}
                <div>
                ...
                <div className="flex gap-3 pt-2">
                  <button type="submit" disabled={isSubmitting} className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-3.5 rounded-xl font-black text-sm flex justify-center gap-2">{isSubmitting ? <Loader2 className="animate-spin" /> : <Save />} {editingCategoryId ? 'حفظ التعديلات' : 'اعتماد القسم'}</button>
