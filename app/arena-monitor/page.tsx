      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-200 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="w-full md:w-auto min-w-[300px]">
            <select 
              className="w-full bg-slate-50 border border-slate-200 text-slate-700 font-black p-3.5 rounded-xl outline-none focus:border-indigo-500 shadow-inner"
              onChange={(e) => {
        ...
        {selectedAssignment && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-4">
                <div className="w-12 h-12 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center"><CheckCircle2 className="w-6 h-6"/></div>
                <div>
              ...
              <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-4">
                <div className="w-12 h-12 bg-amber-50 text-amber-600 rounded-full flex items-center justify-center"><Target className="w-6 h-6"/></div>
                <div>
            ...
            <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
              <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                ...
                <button onClick={() => fetchProgress(selectedAssignment)} className="text-indigo-600 bg-indigo-50 hover:bg-indigo-100 px-4 py-2 rounded-xl text-xs font-black flex items-center gap-2 transition-colors">
                      ...
                      studentsProgress.map((student) => (
                        <tr key={student.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                          <td className="p-4 flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-100 to-purple-100 flex items-center justify-center text-indigo-700 font-black">
                              {student.student_name.charAt(0)}
                            </div>
                            <span className="truncate max-w-[150px]">{student.student_name}</span>
                          </td>
                          ...
                          <td className="p-4">
                            <div className="flex items-center justify-center gap-2">
                              <span className="flex items-center gap-1 text-emerald-600 bg-emerald-50 px-2 py-1 rounded-lg"><CheckCircle2 className="w-3 h-3"/> {student.correct_score}</span>
                              <span className="flex items-center gap-1 text-rose-600 bg-rose-50 px-2 py-1 rounded-lg"><XCircle className="w-3 h-3"/> {student.wrong_score}</span>
                            </div>
                          </td>
                          <td className="p-4">
                            {student.teacher_feedback ? (
                              <span className="text-[10px] bg-indigo-50 text-indigo-600 px-2 py-1 rounded-lg flex items-center gap-1 w-fit"><CheckCircle2 className="w-3 h-3"/> تم إرسال ملاحظة</span>
                            ) : (
                              <span className="text-[10px] text-slate-400 border border-slate-200 px-2 py-1 rounded-lg">لا توجد ملاحظات</span>
                            )}
                          </td>
                          <td className="p-4 text-center">
                            <button onClick={() => openFeedbackModal(student)} className="bg-white border border-slate-200 hover:border-indigo-400 hover:text-indigo-600 p-2 rounded-xl text-slate-500 transition-all shadow-sm">
      ...
      <AnimatePresence>
        {feedbackModalOpen && selectedStudent && (
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-[2rem] shadow-2xl z-50 overflow-hidden">
              
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-indigo-50/50">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-indigo-100 text-indigo-600 rounded-xl"><Sparkles className="w-5 h-5"/></div>
                  <div>
                    <h3 className="font-black text-slate-800">رسالة للمعلم</h3>
                    <p className="text-xs font-bold text-slate-500">إلى: {selectedStudent.student_name}</p>
                  </div>
                </div>
                <button onClick={() => setFeedbackModalOpen(false)} className="p-2 text-slate-400 hover:text-slate-700 bg-white rounded-full shadow-sm"><X className="w-5 h-5"/></button>
              </div>

              <div className="p-6 space-y-4">
                <div className="bg-slate-50 border border-slate-200 p-4 rounded-2xl flex justify-between items-center text-sm font-bold">
                  <span>إنجاز الطالب: <span className="text-indigo-600">{selectedStudent.percentage}%</span></span>
                  <span className="text-emerald-600">{selectedStudent.correct_score} صح</span>
                  <span className="text-rose-600">{selectedStudent.wrong_score} خطأ</span>
                </div>
                
                <label className="block text-xs font-black text-slate-500 mb-1">اكتب ملاحظتك التوجيهية أو التشجيعية:</label>
                <textarea 
                  value={feedbackText} 
                  onChange={(e) => setFeedbackText(e.target.value)} 
                  placeholder="مثال: أداء ممتاز يا بطل! راجع قانون نيوتن الثاني وركز في الإشارات السلبية..."
                  className="w-full h-32 bg-white border border-slate-200 p-4 rounded-xl font-bold text-slate-700 outline-none focus:border-indigo-500 resize-none shadow-inner leading-relaxed"
                ></textarea>
              </div>

              <div className="p-6 pt-0 flex gap-3">
                <button onClick={() => setFeedbackModalOpen(false)} className="flex-1 py-3 bg-slate-100 text-slate-600 font-black rounded-xl hover:bg-slate-200 transition-colors">إلغاء</button>
                <button onClick={saveFeedback} disabled={savingFeedback} className="flex-[2] py-3 bg-indigo-600 text-white font-black rounded-xl hover:bg-indigo-700 active:scale-95 transition-all shadow-lg shadow-indigo-200 flex items-center justify-center gap-2 disabled:opacity-70">
