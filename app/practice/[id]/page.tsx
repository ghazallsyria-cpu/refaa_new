...
const TypewriterReveal = ({ htmlContent }: { htmlContent: string }) => {
  ...
  return (
    <div className="relative">
      ...
      <AnimatePresence>
        {!revealed && (
          <motion.div initial={{ opacity: 1 }} animate={{ opacity: [1, 0, 1], y: [0, 50, 100] }} exit={{ opacity: 0 }} transition={{ duration: 3.5, ease: "linear" }} className="absolute left-0 w-full h-[2px] bg-indigo-400/50 shadow-[0_0_10px_rgba(99,102,241,0.8)] z-10" />
        )}
      </AnimatePresence>
    </div>
  );
          ...
          {failedQsForPDF.map((q, idx) => (
            <div key={q.id} className="bg-white border-2 border-slate-200 rounded-3xl p-8 shadow-sm">
              <div className="flex items-center gap-3 mb-6 border-b-2 border-slate-100 pb-4">
                <div className="bg-rose-100 text-rose-700 w-10 h-10 flex items-center justify-center rounded-xl font-black text-lg shrink-0">{idx + 1}</div>
                <h3 className="text-xl font-black text-slate-800">نص السؤال:</h3>
              </div>
              <div className="prose prose-slate max-w-none font-bold text-slate-800 mb-8 text-lg" dangerouslySetInnerHTML={{ __html: renderHTMLWithMath(q.content_html) }} />
              <div className="bg-indigo-50/50 border-2 border-indigo-200 rounded-2xl p-6">
                <h3 className="font-black text-indigo-800 mb-4 flex items-center gap-2 text-lg"><BrainCircuit className="w-6 h-6" /> تحليل الإجابة النموذجية:</h3>
                {q.model_answer_html && q.model_answer_html.trim() !== '' ? (
                  <div className="prose prose-indigo max-w-none font-bold text-indigo-950 text-lg leading-loose" dangerouslySetInnerHTML={{ __html: renderHTMLWithMath(q.model_answer_html) }} />
          ...
          <AnimatePresence mode="wait">
            {!isFinished && currentQ ? (
                  ...
                  {currentQ.type === 'essay' && !showHint && (
                    <div className="mt-8">
                      {isOfficial ? (
                        ...
                        <div className="text-center bg-slate-50 p-6 rounded-2xl border border-slate-200 border-dashed">
                  ...
                  {!isOfficial && showHint && (() => {
                    ...
                    return (
                      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="mt-8 overflow-hidden rounded-2xl border-2 border-indigo-200 bg-white shadow-lg">
                        <div className="flex items-center justify-between bg-gradient-to-r from-indigo-600 to-violet-600 px-5 py-3 text-white"><div className="flex items-center gap-2 font-black text-sm"><BrainCircuit className="w-5 h-5 animate-pulse" /><span>المساعد الذكي يكتب لك الشرح الآن...</span></div><Sparkles className="w-4 h-4 animate-spin-slow opacity-70" /></div>
                        <div className="p-6 bg-indigo-50/30 min-h-[100px]">
                          {extractedImages.length > 0 && (
                            <div className="mb-6 p-4 bg-white/60 rounded-xl border border-indigo-100 shadow-sm flex flex-col items-center gap-4"><p className="text-[10px] font-black text-indigo-400 w-full text-right border-b border-indigo-50 pb-2 uppercase tracking-widest">صورة مرجعية من السؤال:</p>{extractedImages.map((imgHtml, idx) => ( <div key={idx} dangerouslySetInnerHTML={{ __html: imgHtml }} className="max-w-full rounded-lg overflow-hidden shadow-sm" /> ))}</div>
                          )}
                        ...
                        <div className="px-6 py-3 bg-white border-t border-indigo-100 flex justify-center"><span className="text-[10px] font-black text-indigo-400 uppercase tracking-tighter">برمجة وتطوير ايهاب جمال غزال - مدرس الفيزياء</span></div>
                      </motion.div>
                    );
                  })()}
                </div>
                  ...
                  isMCQ ? (
                    <AnimatePresence mode="wait">
                      {isSuccess ? (
                        ...
                        <motion.div key="wrong" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-3"><div className="bg-rose-100 text-rose-800 font-black py-3 px-4 rounded-xl flex items-center justify-center gap-2 border-2 border-rose-300 text-center shadow-sm"><RefreshCcw className="w-5 h-5" /> {randomEncourageMsg}</div>{currentQ.model_answer_html && !showHint && ( <button onClick={() => setShowHint(true)} className="w-full bg-indigo-100 text-indigo-700 font-black py-3 rounded-xl hover:bg-indigo-200 transition-colors flex items-center justify-center gap-2 border border-indigo-300 shadow-sm"><BrainCircuit className="w-5 h-5" /> تحليل الإجابة (المساعد الذكي)</button> )}</motion.div>
                      ) : <div key="idle" className="w-full bg-slate-200/70 text-slate-500 font-black py-4 rounded-xl flex items-center justify-center gap-2 border border-slate-200 shadow-inner">اختر إجابة للتقدم</div>}
                    </AnimatePresence>
                  ) : currentQ.type === 'essay' ? (
                    showHint ? (
                      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-3 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm"><p className="text-center text-sm font-black text-slate-700">تقييم ذاتي: هل إجابتك صحيحة؟</p><div className="flex gap-2"><button onClick={() => handleSelfEvaluation(true)} className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white font-black py-3 rounded-xl flex flex-col items-center gap-1 active:scale-95 transition-all shadow-md"><CheckCircle2 className="w-5 h-5" /> نعم، أتقنتها!</button><button onClick={() => handleSelfEvaluation(false)} className="flex-1 bg-rose-500 hover:bg-rose-600 text-white font-black py-3 rounded-xl flex flex-col items-center gap-1 active:scale-95 transition-all shadow-md"><RefreshCcw className="w-5 h-5" /> لا، أخطأت</button></div></motion.div>
              ...
              <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[2rem] shadow-2xl border border-slate-200 p-6 sm:p-8 text-center relative overflow-hidden h-full flex flex-col justify-center">
                ...
                <p className="text-slate-500 font-bold mb-6">{isOfficial ? 'تم حفظ إجاباتك وإرسالها للمعلم بنجاح.' : (mode === 'retake_errors' ? 'لقد واجهت نقاط ضعفك بقوة.' : 'لقد أكملت التدريب.')}</p>
                ...
                <div className="flex flex-col gap-3">
                  {!isPreviewMode && failedQuestionIds.size > 0 && !isOfficial && ( <button onClick={generatePDF} disabled={isGeneratingPDF} className="w-full bg-emerald-50 text-emerald-700 border-2 border-emerald-200 font-black py-4 rounded-xl hover:bg-emerald-100 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50">{isGeneratingPDF ? <>جاري تجهيز الملف... <span className="animate-spin text-xl">⏳</span></> : <>تحميل ملخص أخطائي (مع الشرح PDF) <FileText className="w-5 h-5" /></>}</button> )}
