...
export default function TeacherDashboard() {
  ...
  // ✅ الإصلاح: todaysSchedule مشتق من schedule بشكل صحيح
  ...
  // ============================================================
  // 🌟 منطق تحديد الشاشة — يقرأ الثلاثة مفاتيح بشكل صحيح
  // ============================================================
  ...
  // ✅ قراءة الثلاثة مفاتيح من platform_settings
  const isSuspenseGlobal  = platformSettings?.results_suspense_mode    === true;
  const isMiddlePublished = platformSettings?.results_published_middle  === true;
  const isHighPublished   = platformSettings?.results_published_high    === true;

  // المرحلة 2: وضع الترقب + النتائج لم تُصدر بعد
  const showMiddleSchoolEndYear   = isStrictlyMiddleSchoolTeacher && isSuspenseGlobal && !isMiddlePublished;
  const showHighSchoolExamMode    = isHighSchoolTeacher           && isSuspenseGlobal && !isHighPublished;

  // المرحلة 3: تم إصدار النتائج (احتفال)
  const showMiddleResultsCelebration = isStrictlyMiddleSchoolTeacher && isMiddlePublished;
  const showHighResultsCelebration   = isHighSchoolTeacher           && isHighPublished;

  // الوضع الافتراضي: لا شيء من الأعلى مفعّل
  const showRegularDashboard = !showMiddleSchoolEndYear && !showHighSchoolExamMode
                             && !showMiddleResultsCelebration && !showHighResultsCelebration;
    ...
    <motion.div initial="hidden" animate="visible" variants={containerVariants} className="min-h-screen relative bg-[#02040a] text-slate-100 pb-32 overflow-x-hidden font-sans pt-2 sm:pt-6 print:bg-white print:text-black print:p-0" dir="rtl">
      ...
      <div className="space-y-6 sm:space-y-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 print:w-full print:px-0">
        ...
        <AnimatePresence mode="wait">
          ...
          {/* ============================================================ */}
          {/* 🎓 شاشة 1 — معلمو المتوسطة: نهاية العام (ترقب + لم تُصدر بعد) */}
          {/* ============================================================ */}
          ...
          {/* ============================================================ */}
          {/* 🎯 شاشة 2 — معلمو الثانوية: غرفة عمليات (ترقب + لم تُصدر بعد) */}
          {/* ============================================================ */}
          ...
          {/* ============================================================ */}
          {/* 🎉 شاشة 3 — احتفال إصدار نتائج المتوسطة                      */}
          {/* ============================================================ */}
          {showMiddleResultsCelebration && (
            <motion.div key="middle-results-celebration" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="relative overflow-hidden rounded-[2.5rem] glass-panel p-10 sm:p-16 border-emerald-500/40 text-center shadow-[0_0_60px_rgba(16,185,129,0.2)] flex flex-col items-center justify-center min-h-[70vh]">
               <div className="absolute inset-0 bg-gradient-to-br from-emerald-900/20 via-transparent to-teal-900/20 pointer-events-none"></div>

               <motion.div
                  animate={{ rotate: [0, -10, 10, -10, 0], scale: [1, 1.1, 1] }}
                  transition={{ duration: 0.6, delay: 0.3 }}
                  className="w-32 h-32 bg-emerald-500/10 rounded-full flex items-center justify-center border-2 border-emerald-500/40 shadow-[0_0_50px_rgba(16,185,129,0.4)] mb-8 z-10"
               >
                  <PartyPopper className="w-16 h-16 text-emerald-400 drop-shadow-lg" />
               </motion.div>

               <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 text-xs font-black mb-6 z-10 shadow-inner uppercase tracking-widest">
                  <CheckCircle2 className="w-4 h-4" /> تم إصدار النتائج الرسمياً
               </div>

               <h2 className="text-4xl sm:text-6xl font-black text-white mb-6 drop-shadow-xl z-10 tracking-tight leading-tight">
                  نتائج المتوسطة<br/><span className="text-emerald-400">أُعلنت الآن! 🎊</span>
               </h2>

               <p className="text-emerald-100 font-bold text-lg sm:text-xl max-w-2xl leading-relaxed z-10 bg-[#02040a]/60 backdrop-blur-md p-8 rounded-3xl border border-emerald-500/20 shadow-inner mb-8">
                  أستاذي الفاضل <strong className="text-emerald-400">({teacherData?.users?.full_name})</strong>،
                  تم إصدار النتائج النهائية لطلاب المرحلة المتوسطة. يمكن للطلاب المسددين الاطلاع على نتائجهم الآن من لوحة التحكم الخاصة بهم.
                  <br/><br/>
                  <span className="text-sm text-slate-300">جزاكم الله خير الجزاء على عطائكم طوال هذا العام المبارك. 🌸</span>
               </p>
          ...
          {/* ============================================================ */}
          {/* 🎓 شاشة 4 — احتفال إصدار نتائج الثانوية                      */}
          {/* ============================================================ */}
          {showHighResultsCelebration && (
            <motion.div key="high-results-celebration" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="relative overflow-hidden rounded-[2.5rem] glass-panel p-10 sm:p-16 border-amber-500/40 text-center shadow-[0_0_60px_rgba(245,158,11,0.2)] flex flex-col items-center justify-center min-h-[70vh]">
               <div className="absolute inset-0 bg-gradient-to-br from-amber-900/20 via-transparent to-yellow-900/20 pointer-events-none"></div>

               <motion.div
                  animate={{ y: [0, -15, 0], rotate: [0, 5, -5, 0] }}
                  transition={{ duration: 1, delay: 0.3, repeat: 2 }}
                  className="w-32 h-32 bg-amber-500/10 rounded-full flex items-center justify-center border-2 border-amber-500/40 shadow-[0_0_50px_rgba(245,158,11,0.4)] mb-8 z-10"
               >
                  <GraduationCap className="w-16 h-16 text-amber-400 drop-shadow-lg" />
               </motion.div>

               <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-amber-500/20 border border-amber-500/30 text-amber-300 text-xs font-black mb-6 z-10 shadow-inner uppercase tracking-widest">
                  <Award className="w-4 h-4" /> اكتملت مهمة اللجان
               </div>

               <h2 className="text-4xl sm:text-6xl font-black text-white mb-6 drop-shadow-xl z-10 tracking-tight leading-tight">
                  نتائج الثانوية<br/><span className="text-amber-400">أُعلنت الآن! 🏆</span>
               </h2>

               <p className="text-amber-100 font-bold text-lg sm:text-xl max-w-2xl leading-relaxed z-10 bg-[#02040a]/60 backdrop-blur-md p-8 rounded-3xl border border-amber-500/20 shadow-inner mb-8">
                  أستاذي القدير <strong className="text-amber-400">({teacherData?.users?.full_name})</strong>،
                  انتهت مهام اللجان وتم إصدار النتائج النهائية لطلاب المرحلة الثانوية. يمكن للطلاب المسددين الاطلاع على نتائجهم الآن.
                  <br/><br/>
                  <span className="text-sm text-slate-300">شكراً لجهودكم الاستثنائية في فترة الاختبارات. دمتم عطاءً ونوراً للأجيال. ✨</span>
               </p>
          ...
          {/* ============================================================ */}
          {/* 🏫 شاشة 5 — الداشبورد العادي (الوضع الافتراضي)               */}
          {/* ============================================================ */}
                         ...
                         <div className="glass-panel border-rose-500/30 p-6 sm:p-8 rounded-[2rem] flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden shadow-[0_10px_30px_-10px_rgba(244,63,94,0.2)] z-20">
                           ...
                           <div className="flex items-start gap-5 relative z-10 w-full md:w-auto">
                             ...
                             <div>
                               ...
                               <p className="text-sm font-bold text-slate-300 mb-4 leading-relaxed">أستاذي الكريم، لقد انتهى وقت الحصص التالية ولم تقم بتسجيل غياب الطلاب لها حتى الآن:</p>
                             ...
                             return (
                               <div key={i} className={cn("relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group", current ? "is-active z-20" : "z-10")}>
                                 ...
                                 <div className={cn(
                                   ...
                                   current ? "bg-emerald-500/10 border-emerald-500/40 shadow-[0_0_30px_rgba(16,185,129,0.15)] scale-[1.02] ring-1 ring-emerald-500/20" 
                                   : isPast ? "bg-[#02040a]/20 border-white/5 opacity-60 grayscale" 
                                   : next ? "bg-amber-500/5 border-amber-500/20" 
                                   ...
                                   <div className={`absolute top-0 right-0 w-1 h-full ${current ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.8)]' : isPast ? 'bg-slate-800' : next ? 'bg-amber-400' : 'bg-white/10'}`}></div>
                                   <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-2 sm:gap-3 mb-3 sm:mb-4 pr-2">
                                     ...
                                     <div className="flex flex-wrap items-center gap-1.5 shrink-0">
                                       {current && <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-[9px] sm:text-[10px] font-bold text-emerald-300 shadow-inner"><div className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" /> تعمل الآن</span>}
                                       ...
                                       <span className={cn("text-[10px] sm:text-xs font-black px-2.5 py-1 rounded-xl shadow-inner border whitespace-nowrap", current ? "bg-emerald-500/20 text-emerald-300 border-emerald-500/20" : isPast ? "bg-transparent text-slate-500 border-slate-800" : "bg-white/5 text-slate-300 border-white/10")}>الحصة {item.period}</span>
                                     </div>
                                   </div>
                       ...
                       {recentExams.length > 0 ? (
                         recentExams.map((exam, idx) => (
                           <div key={idx} className="p-5 sm:p-6 hover:bg-white/5 transition-colors group">
                             ...
                             <p className="text-[10px] sm:text-xs font-bold text-slate-300 mb-3 sm:mb-4 bg-white/5 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg inline-block border border-white/5 shadow-inner">{exam.subject_name} • {exam.section_name}</p>
                       ...
                       {recentAssignments.length > 0 ? (
                         recentAssignments.map((assignment, idx) => (
                           <div key={idx} className="p-5 sm:p-6 hover:bg-white/5 transition-colors group">
                             ...
                             <p className="text-[10px] sm:text-xs font-bold text-slate-300 mb-3 sm:mb-4 bg-white/5 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg inline-block border border-white/5 shadow-inner">{assignment.subject_name} • {assignment.section_name || 'توزيع ذكي'}</p>
                             <div className="flex gap-2 sm:gap-3">
                           ...
                           return (
                             <Link href={`/messages?id=${msg.id}`} key={i} className={`flex gap-3 sm:gap-4 p-4 sm:p-6 transition-all group relative ${isUnread ? 'bg-indigo-500/10 hover:bg-indigo-500/20 border-l-4 border-l-indigo-400' : 'hover:bg-white/5 border-l-4 border-l-transparent'}`}>
                               ...
                               <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl sm:rounded-2xl bg-[#02040a]/60 backdrop-blur-md border border-white/10 flex-shrink-0 flex items-center justify-center font-black text-base sm:text-lg text-emerald-400 shadow-inner group-hover:scale-110 transition-transform overflow-hidden relative z-10">
                                 {msg.sender?.avatar_url ? <img src={msg.sender.avatar_url} alt={msg.sender.full_name} className="w-full h-full object-cover" /> : msg.sender?.full_name?.charAt(0) || 'م'}
                               </div>
