...
export default function StudentsPage() {
  ...
  // 🧠 معالجة البيانات وتجاوز أخطاء TypeScript الصارمة
  ...
  const stats = useMemo(() => {
    const tenthGradeStudents = students.filter((s: any) => {
    ...
    return {
      ...
      scientific: tenthGradeStudents.filter((s: any) => s.next_year_track === 'scientific').length,
      literary: tenthGradeStudents.filter((s: any) => s.next_year_track === 'literary').length,
      unassigned: students.filter((s: any) => !s.section_id).length,
    };
  }, [students]);

  const handleAddSubmit = async () => {
    try {
      if (!addForm.full_name || !addForm.national_id) return alert('يرجى تعبئة الحقول الإلزامية (الاسم والرقم المدني)');
      await addStudent(addForm);
      alert('تمت الإضافة بنجاح');
      setShowAddModal(false);
      setAddForm({ full_name: '', national_id: '', email: '', phone: '', section_id: '', parent_id: '' });
    } catch (error: any) { alert(error.message || 'حدث خطأ أثناء إضافة الطالب'); }
  };
      ...
      <div className="relative z-10 max-w-[1600px] mx-auto space-y-8 pb-20 px-4 sm:px-6 lg:px-8 pt-8">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button onClick={() => alert('ميزة الاستيراد الجماعي للطلاب قيد التطوير وستتوفر قريباً!')} className="inline-flex items-center gap-2 rounded-2xl bg-indigo-50 px-5 py-3 text-sm font-black text-indigo-700 shadow-sm border border-indigo-100 hover:bg-indigo-100 transition-all active:scale-95">
        ...
        <div className="bg-white p-4 rounded-[2rem] shadow-sm border border-slate-200 flex flex-col md:flex-row items-center gap-4">
          ...
          <div className="flex gap-2 w-full md:w-auto">
            <select value={selectedSection} onChange={(e) => { setSelectedSection(e.target.value); setCurrentPage(1); }} className="flex-1 md:w-48 bg-slate-50 border-0 rounded-xl py-3.5 px-4 font-bold text-slate-700 focus:ring-2 focus:ring-indigo-500 cursor-pointer">
              ...
              {sections.map(s => { const classData = Array.isArray(s.classes) ? s.classes[0] : s.classes; return <option key={s.id} value={s.id}>{classData?.name} - {s.name}</option>; })}
            </select>
        ...
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <tr key={student.id} className="hover:bg-slate-50/80 transition-colors group">
                      <td className="whitespace-nowrap py-4 pr-8 pl-4">
                        <div className="flex items-center gap-4">
                          <div className="h-10 w-10 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center font-black text-sm border border-indigo-100 group-hover:scale-110 transition-transform">{userData?.full_name?.charAt(0) || 'ط'}</div>
                          <div><p className="font-black text-slate-900">{userData?.full_name || 'غير معروف'}</p><p className="text-[10px] text-slate-400 font-bold">{userData?.email || 'لا يوجد بريد'}</p></div>
                        </div>
                      </td>
                      <td className="whitespace-nowrap px-4 py-4 text-sm font-bold text-slate-600 font-mono">{student.national_id}</td>
                      <td className="whitespace-nowrap px-4 py-4">
                        <div className="flex flex-col"><span className="text-sm font-black text-slate-900">{classData?.name || 'بدون صف'}</span><span className="text-[10px] font-bold text-indigo-500">{secData?.name || 'بدون شعبة'}</span></div>
                      </td>
                      <td className="whitespace-nowrap px-4 py-4">
                        {classData?.level === 10 || classData?.name?.includes('العاشر') ? (student.next_year_track ? (<span className={`px-2.5 py-1 rounded-lg text-[10px] font-black ${student.next_year_track === 'scientific' ? 'bg-blue-50 text-blue-600' : 'bg-purple-50 text-purple-600'}`}>{student.next_year_track === 'scientific' ? 'علمي' : 'أدبي'}</span>) : <span className="text-[10px] font-bold text-slate-400">لم يحدد</span>) : <span className="text-slate-300">-</span>}
                      </td>
                      <td className="whitespace-nowrap px-4 py-4 text-sm font-bold text-slate-600">{parentUserData?.full_name || 'غير مسجل'}</td>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between border-t border-slate-200 bg-white px-6 py-4">
              <p className="text-sm font-bold text-slate-500">إظهار <span className="font-black text-slate-900">{((currentPage - 1) * itemsPerPage) + 1}</span> إلى <span className="font-black text-slate-900">{Math.min(currentPage * itemsPerPage, filteredStudents.length)}</span> من أصل <span className="font-black text-slate-900">{filteredStudents.length}</span></p>
          ...
          <div className="bg-white w-full max-w-2xl rounded-[2rem] shadow-2xl overflow-hidden my-8">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div>
                 ...
                 <p className="text-xs font-bold text-slate-500 mt-1 flex items-center gap-1"><AlertCircle className="w-3 h-3 text-indigo-500"/>تلميح: لإضافة مجموعة طلاب، انتظر ميزة "الاستيراد عبر Excel" قريباً.</p>
              </div>
