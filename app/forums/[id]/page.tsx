...
import { 
  ...
  Pin, Lock, User, Clock, Send, XCircle, ShieldCheck
} from 'lucide-react';
...
export default function CategoryPage() {
  ...
  // دالة لتنظيف المحتوى وعرض مقتطف (Snippet) بدون أكواد HTML للمعاينة
            ...
            {filteredTopics.map((topic) => {
              ...
              <Link key={topic.id} href={`/forums/topic/${topic.id}`} className="block hover:bg-slate-50 transition-colors group p-5 sm:p-6">
                <div className="flex items-start gap-4">
                  {/* 🚀 عرض الصورة الشخصية أو الأيقونة بناءً على دور الكاتب */}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-3 sm:gap-4 text-[10px] sm:text-xs font-bold text-slate-500">
                      {/* 🚀 شارة التوثيق الذهبية للمعلمين */}
                      <span className={`flex items-center gap-1.5 px-2 py-1 rounded-md ${isStaff ? 'bg-gradient-to-r from-amber-100 to-amber-50 text-amber-700 border border-amber-200 shadow-sm' : 'bg-slate-100 text-slate-600 border border-slate-200'}`}>
                         {isStaff && <ShieldCheck className="w-3.5 h-3.5" />}
                         {topic.author_name} {isStaff && '(معلم)'}
                      </span>
