    ...
    return withCache('admin_stats', async () => {
      try {
        ...
        const attendanceRate = totalAttendanceCount > 0
          ? Math.round((presentAttendanceCount / totalAttendanceCount) * 100) : 0;
  ...
  const fetchStudentDashboardData = useCallback(async (forceRefresh = false) => {
    ...
    return withCache(`student_dashboard_${user.id}`, async () => {
      try {
        ...
        if (!student) {
          return {
            student: { id: user.id, users: { full_name: 'حساب طالب غير مكتمل (راجع الإدارة)' } },
            assignments: [], exams: [], attendanceRate: 0, grades: [], todaysSchedule: [], periods: []
          };
        }
        ...
        const [
          { data: assignments }, { data: exams }, { data: attendance }, { data: grades }, { data: todaysSchedule }, { data: periods }
        ] = await Promise.all([
        ...
        return {
          student, assignments: assignments || [], exams: exams || [], attendanceRate, grades: grades || [], todaysSchedule: todaysSchedule || [], periods: periods || []
        };
      } catch (error) {
  ...
  const fetchTeacherDashboardData = useCallback(async (forceRefresh = true) => { 
    ...
    return withCache(`teacher_dashboard_${user.id}`, async () => {
      try {
        ...
        const { data: teacherSections } = await supabase.from('teacher_sections').select('section_id, section:sections(id, name, class_id, classes(id, name), students(count))').eq('teacher_id', teacher.id).limit(5000); 
        ...
        const [
          ...
          { data: attendanceRecords } // 🚀 الجلب الجديد لسجلات الحضور للمعلم
        ] = await Promise.all([
          ...
          supabase.from('schedules').select('id, day_of_week, period, start_time, end_time, subjects(name), sections(name, classes(name))').eq('teacher_id', teacher.id).order('day_of_week').order('period').limit(5000),
          supabase.from('class_periods').select('*').order('period_number').limit(100),
        ...
        // 🚀 العملية الحسابية الذكية لاستخراج نسبة الحضور والغياب لطلاب المعلم
        ...
        const exams = (recentExams || []).map((e: any) => {
          ...
          return { ...e, subject_name: subj?.name || 'غير محدد', section_name: sec ? `${Array.isArray(sec.classes) ? sec.classes[0]?.name : sec.classes?.name || ''} - ${sec.name}` : 'غير محدد' };
        });
        ...
        const assignments = (recentAssignments || []).map((a: any) => {
          ...
          return { ...a, subject_name: subj?.name || 'غير محدد', section_name: sec ? `${Array.isArray(sec.classes) ? sec.classes[0]?.name : sec.classes?.name || ''} - ${sec.name}` : 'غير محدد' };
        });
        ...
        return { 
          ...
          stats: { 
            ...
            avgAttendance, // 🚀 الأرقام الحقيقية تُرسل هنا
            absenceRate    // 🚀 الأرقام الحقيقية تُرسل هنا
          } 
        };
      } catch (error) {
