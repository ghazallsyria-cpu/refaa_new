    ...
    const fetchTeacherSpecificData = async () => {
      ...
      try {
        ...
        if (userRole === 'admin' || userRole === 'management') {
          ...
          // جلب مواد المعلم فقط
          const { data: teacherSubjs } = await supabase
    ...
    <div className="min-h-screen bg-slate-50/50 pb-24" dir="rtl">
      ...
      <main className="max-w-4xl mx-auto px-6 py-12 space-y-10">
        <div className="glass-card rounded-[40px] border-t-[16px] border-t-indigo-600 p-10 space-y-10 shadow-2xl bg-white">
                  ...
                  {formData.sections.length > 0 ? (
                    <div className="flex flex-col gap-1">
                      {formData.sections.map((s: any) => {
                        ...
                        return (
                          <label key={s.id} className={`flex items-center gap-4 p-4 rounded-2xl cursor-pointer transition-all hover:bg-slate-50 ${isSelected ? 'bg-indigo-50/50' : ''}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 border-t border-slate-100 pt-10"><div className="space-y-2"><label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2 block">المدة (دقيقة)</label><input type="number" value={exam.duration} onChange={(e) => setExam({...exam, duration: parseInt(e.target.value)})} className="w-full p-4 rounded-2xl bg-slate-50 border-0 ring-1 ring-slate-100 font-bold focus:ring-2 focus:ring-indigo-600 outline-none transition-all" /></div><div className="space-y-2"><label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2 block">تاريخ الاختبار</label><input type="date" value={exam.exam_date} onChange={(e) => setExam({...exam, exam_date: e.target.value})} className="w-full p-4 rounded-2xl bg-slate-50 border-0 ring-1 ring-slate-100 font-bold focus:ring-2 focus:ring-indigo-600 outline-none transition-all" /></div><div className="space-y-2"><label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2 block">وقت البدء - النهاية</label><div className="flex gap-2"><input type="time" value={exam.start_time} onChange={(e) => setExam({...exam, start_time: e.target.value})} className="w-1/2 p-4 rounded-2xl bg-slate-50 border-0 ring-1 ring-slate-100 font-bold text-center focus:ring-2 focus:ring-indigo-600 outline-none transition-all" /><input type="time" value={exam.end_time} onChange={(e) => setExam({...exam, end_time: e.target.value})} className="w-1/2 p-4 rounded-2xl bg-slate-50 border-0 ring-1 ring-slate-100 font-bold text-center focus:ring-2 focus:ring-indigo-600 outline-none transition-all" /></div></div></div>
        </div>
