    ...
    try {
      ...
      if (role === 'teacher') {
        ...
        if (studentProfile?.section_id) {
          ...
          const sectionAssignmentIds =
            assignmentSections?.map((as: any) => as.assignment_id) || [];
          ...
          if (!contentType || contentType === 'exam') {
            ...
            examsData = (examsRes.data || []).map((exam: any) => {
              const attempt = examAttemptsRes.data?.find(
                (a: any) => a.exam_id === exam.id
              );
          ...
          if (!contentType || contentType === 'assignment') {
            const [assignmentsRes, assignmentSubmissionsRes] =
              await Promise.all([
                supabase
                  .from('assignments')
                  .select(
                  ...
                  .or(
                    `section_id.eq.${sectionId}${
                      sectionAssignmentIds.length > 0
                        ? `,id.in.(${sectionAssignmentIds.join(',')})`
                        : ''
                    }`
                  )
                  .eq('status', 'published'),
            ...
            assignmentsData = (assignmentsRes.data || []).map(
              (assignment: any) => {
                const submission = assignmentSubmissionsRes.data?.find(
