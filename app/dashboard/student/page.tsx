...
import { 
  ...
  XCircle, AlertTriangle, PieChart, ArrowRight
} from 'lucide-react';
  ...
  if (loading) return (
    <div className="flex h-[80vh] items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        ...
        <p className="text-slate-400 font-bold animate-pulse">جاري تحميل بياناتك...</p>
      </div>
    </div>
  );
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 pb-12 max-w-7xl mx-auto px-4" dir="rtl">
      ...
      <div className="relative overflow-hidden rounded-[45px] bg-gradient-to-br from-indigo-700 via-indigo-600 to-violet-800 p-10 text-white shadow-2xl">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="space-y-4">
            ...
            <h1 className="text-4xl md:text-5xl font-black tracking-tight">مرحباً، {data?.student?.users?.full_name || 'عزيزي الطالب'} 👋</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          {/* Detailed Attendance - Bento Design */}
          <div className="bg-white p-8 rounded-[40px] shadow-xl border border-slate-50">
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-2xl font-black flex items-center gap-3">
                <PieChart className="text-indigo-600" /> ملخص الحضور والغياب
              </h2>
              <span className="text-[10px] font-black bg-emerald-50 text-emerald-600 px-3 py-1 rounded-full uppercase tracking-tighter">بيانات حقيقية</span>
            </div>
            
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: 'حضور', val: data?.presentCount, icon: CheckCircle2, color: 'text-emerald-600', bg: 'bg-emerald-50', sub: 'أيام التواجد' },
                { label: 'غياب', val: data?.absentCount, icon: XCircle, color: 'text-red-600', bg: 'bg-red-50', sub: 'بدون عذر' },
                { label: 'تأخير', val: data?.lateCount, icon: Clock, color: 'text-amber-600', bg: 'bg-amber-50', sub: 'بعد الوقت' },
                { label: 'جزئي', val: data?.partialCount, icon: AlertTriangle, color: 'text-blue-600', bg: 'bg-blue-50', sub: 'مغادرة مبكرة' }
              ].map((s, i) => (
          ...
          {/* Performance Chart - Fixed shadow property */}
          <div className="bg-white p-8 rounded-[40px] shadow-xl border border-slate-50">
            <h2 className="text-2xl font-black mb-8 flex items-center gap-3">
              <TrendingUp className="text-indigo-600" /> منحنى المستوى الأكاديمي
            </h2>
              ...
              {data?.grades?.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={[...data.grades].reverse()}>
                    ...
                    {/* تم تصحيح الخطأ هنا من shadow إلى boxShadow */}
                    <Tooltip contentStyle={{ borderRadius: '20px', border: 'none', boxShadow: '0 20px 25px -5px rgb(0 0 0 / 0.1)' }} />
                    <Area type="monotone" dataKey="score" stroke="#4f46e5" fill="#4f46e5" fillOpacity={0.1} strokeWidth={4} />
                  </AreaChart>
                </ResponsiveContainer>
              ) : <div className="h-full flex items-center justify-center text-slate-300 font-bold bg-slate-50 rounded-[30px] border-2 border-dashed border-slate-100">لم يتم رصد نتائج اختبارات بعد</div>}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white p-6 rounded-[35px] shadow-xl border border-slate-50">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-lg font-black flex items-center gap-2"><Calendar className="text-indigo-600" /> جدول اليوم</h2>
              <Link href="/dashboard/student/schedule" className="text-[10px] font-black text-indigo-600 underline">الجدول الكامل</Link>
            </div>
            <div className="space-y-3">
              {data?.todaysSchedule?.length > 0 ? data.todaysSchedule.map((lesson: any, i: number) => (
                <div key={i} className="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-between group hover:bg-white hover:border-indigo-200 transition-all">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-black text-slate-900 group-hover:text-indigo-600 transition-colors">{lesson.subjects?.name}</p>
                      <p className="text-[10px] text-slate-400 font-bold line-clamp-1">{lesson.teachers?.users?.full_name || 'معلم المادة'}</p>
                    </div>
                  </div>
                  <div className="text-[9px] font-black text-slate-400" dir="ltr">{lesson.start_time?.substring(0, 5) || '--:--'}</div>
                </div>
              )) : <p className="text-center py-8 text-slate-400 font-bold text-xs bg-slate-50/50 rounded-2xl border border-dashed border-slate-100">لا توجد حصص مجدولة</p>}
            </div>
          </div>
          ...
          <div className="bg-slate-900 p-8 rounded-[40px] text-white shadow-xl relative overflow-hidden group">
            <div className="relative z-10">
              ...
              <Link href="/assignments" className="w-full bg-white text-slate-900 py-4 rounded-2xl font-black text-center flex items-center justify-center gap-2 shadow-lg hover:bg-indigo-50 transition-all active:scale-95">
                تصفح المهام <ArrowRight size={18} />
              </Link>
            </div>
