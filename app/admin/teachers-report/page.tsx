...
export default function TeachersMonitorPage() {
  ...
  const getSchoolTime = () => {
    ...
    return new Date(utc + (3 * 3600000)); // توقيت الكويت/السعودية (+3)
  };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // 🚀 خوارزمية التدقيق المتقدمة (Advanced Audit Engine)
      const results: TeacherMonitor[] = (teachersData as any[]).map((teacher: any) => {
        // جدول المعلم لليوم فقط
        const teacherSchedules = allSchedules?.filter(s => s.teacher_id === teacher.id && s.day_of_week === dbDay) || [];
        const total = teacherSchedules.length;

        // سجلات الغياب التي رصدها المعلم اليوم
        const teacherAttendance = allAttendance?.filter(a => a.teacher_id === teacher.id && a.date === todayStr) || [];
        
        // 🚀 استخراج الحصص الفريدة التي تم رصدها (لأن كل طالب له سجل، نحن نريد عدّ "الحصة" ككل)
        const uniqueRecordedSlots = new Set(teacherAttendance.map(a => `${a.section_id}-${a.period}`)).size;
        
        // تحديد المسجل (لا يمكن أن يتجاوز الإجمالي)
        ...
        const lastRecorded = teacherAttendance.length > 0
          ? [...teacherAttendance].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())[0].created_at
          : null;
