...
import { 
  ...
  TrendingUp, TrendingDown, Trophy, AlertTriangle, X, Power, Trash2, Settings, Plus, Layers, UserCircle, CheckCircle2
} from 'lucide-react';
...
const StarDisplay = ({ val }: { val: number }) => (
  <div className="flex items-center gap-1 bg-amber-500/10 text-amber-500 px-2 py-1 rounded-lg font-black text-sm border border-amber-500/20 shadow-inner w-fit">
    <Star className="w-3.5 h-3.5 fill-amber-500 drop-shadow-sm" /> {val.toFixed(1)}
  </div>
);
...
export default function StudentEvaluationsDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (settingsData) {
         ...
         setCriteria(settingsData.evaluation_criteria || ["المحور العلمي", "المحور الإداري", "المحور الإنساني"]);
      }
      ...
      const formattedRaw = (data || []).map((ev: any) => {
         ...
         // حساب المتوسط لهذا التقييم (يدعم الـ JSON الديناميكي والنظام القديم)
         let avg = 0;
         if (ev.dynamic_ratings && Object.keys(ev.dynamic_ratings).length > 0) {
            const vals = Object.values(ev.dynamic_ratings).map(Number);
            avg = vals.reduce((a, b) => a + b, 0) / vals.length;
         } else {
            avg = ((ev.scientific_rating||0) + (ev.management_rating||0) + (ev.humanity_rating||0)) / 3;
         }
      ...
      const finalStats = Object.values(groupedData).map((t: any) => ({
        ...t, overall_avg: t.sum_score / t.total_evals
  ...
  const saveSettings = async () => {
     ...
     try {
       await supabase.from('platform_settings').update({
          ...
          // إبقاء المفتاح القديم مفعلاً إذا كان أي من المرحلتين تعمل (للتوافق)
  ...
  const deleteEvaluation = async (id: string, studentName: string) => {
      ...
      try {
          ...
          if (error) throw error;
          // تحديث الواجهة بدون إعادة التحميل من السيرفر
          setAllRawEvaluations(prev => prev.filter(ev => ev.id !== id));
          fetchData(); // لإعادة حساب المتوسطات
    ...
    <div className="min-h-screen bg-slate-50 p-4 sm:p-6 md:p-8 font-cairo text-slate-800 pb-32" dir="rtl">
      ...
      <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8 relative">
        ...
        <div className="bg-white rounded-[2rem] p-6 sm:p-10 border border-slate-200 shadow-sm relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
              ...
              <p className="text-slate-500 font-bold text-sm flex items-center gap-4">
                 <span>حالة المتوسط: <strong className={isMiddleActive ? 'text-emerald-500' : 'text-rose-500'}>{isMiddleActive ? 'بوابة مفتوحة' : 'مغلقة'}</strong></span>
                 <span>حالة الثانوي: <strong className={isHighActive ? 'text-emerald-500' : 'text-rose-500'}>{isHighActive ? 'بوابة مفتوحة' : 'مغلقة'}</strong></span>
              </p>
            </div>
        ...
        <div className="flex bg-white p-2 rounded-2xl border border-slate-200 shadow-sm w-fit mx-auto sm:mx-0">
           ...
           <button onClick={() => setActiveTab('details')} className={`px-6 py-3 rounded-xl font-black text-sm transition-all flex items-center gap-2 ${activeTab === 'details' ? 'bg-indigo-50 text-indigo-600 shadow-inner border border-indigo-100' : 'text-slate-500 hover:bg-slate-50'}`}><List className="w-4 h-4"/> سجل الفحص والرقابة (تتبع الطلاب)</button>
        </div>
                    ...
                    <div key={t.teacher_id} className="bg-white border border-slate-200 rounded-3xl p-5 hover:border-indigo-300 transition-colors shadow-sm flex flex-col group">
                      <div className="flex items-start justify-between mb-4 border-b border-slate-100 pb-4">
                          <div className="flex items-center gap-3">
                            <div className="w-12 h-12 rounded-full bg-slate-100 border-2 border-white shadow-md overflow-hidden shrink-0">{t.avatar ? <img src={t.avatar} className="w-full h-full object-cover"/> : <UserCircle className="w-full h-full text-slate-400"/>}</div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden">
             <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex flex-col lg:flex-row items-center justify-between gap-4">
                 ...
                 <div className="flex gap-3 w-full lg:w-auto">
                    <div className="relative flex-1 lg:w-64"><div className="absolute inset-y-0 right-0 pr-4 flex items-center"><Search className="h-4 w-4 text-slate-400" /></div><input type="text" className="w-full border rounded-xl py-2.5 pr-11 pl-4 text-sm font-bold bg-white" placeholder="اسم المعلم..." value={searchTeacher} onChange={(e) => setSearchTeacher(e.target.value)} /></div>
                    <div className="relative flex-1 lg:w-64"><div className="absolute inset-y-0 right-0 pr-4 flex items-center"><Layers className="h-4 w-4 text-slate-400" /></div><input type="text" className="w-full border rounded-xl py-2.5 pr-11 pl-4 text-sm font-bold bg-white" placeholder="الفصل (مثال: 12 علمي)..." value={searchClass} onChange={(e) => setSearchClass(e.target.value)} /></div>
                 </div>
             </div>
                ...
                <table className="w-full text-right border-collapse">
                   <thead>
                      <tr className="bg-slate-100 text-slate-500 text-xs font-black uppercase tracking-widest">
                         <th className="p-4 rounded-r-xl">الطالب والفصل</th>
                         <th className="p-4">المعلم والمادة</th>
                         <th className="p-4">التقييم (النجوم)</th>
                         ...
                         <tr key={ev.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors group">
                            ...
                            <td className="p-4">
                               <p className="font-bold text-slate-700 text-sm flex items-center gap-2">
                                 <div className="w-6 h-6 rounded-full bg-slate-200 overflow-hidden shrink-0">{ev.teacher_avatar ? <img src={ev.teacher_avatar} className="w-full h-full object-cover"/> : <UserCircle className="w-full h-full text-slate-400"/>}</div>
                                 {ev.teacher_name}
                               </p>
                               <p className="text-[10px] text-slate-500 font-bold mt-1 pr-8">{ev.subject}</p>
                            </td>
                            <td className="p-4">
                               <div className="flex flex-col gap-1">
                                 {ev.dynamic_ratings ? Object.entries(ev.dynamic_ratings).map(([key, val]) => (
                                    <div key={key} className="flex justify-between items-center text-[10px] font-bold bg-amber-50 px-2 py-0.5 rounded border border-amber-100">
                                    ...
                                    <div className="text-[10px] font-bold bg-slate-100 px-2 py-1 rounded text-slate-500">تقييم بنظام قديم: المتوسط {ev.avg_score.toFixed(1)}</div>
                                 )}
                               </div>
                            </td>
                            <td className="p-4 text-xs font-bold text-slate-600 italic whitespace-pre-wrap">
                               {ev.feedback ? `"${ev.feedback}"` : <span className="text-slate-400">بدون تعليق</span>}
                            </td>
                            <td className="p-4 text-center">
                               <button onClick={() => deleteEvaluation(ev.id, ev.student_name)} className="p-2 bg-rose-50 text-rose-500 rounded-xl hover:bg-rose-500 hover:text-white transition-all shadow-sm border border-rose-200 opacity-50 group-hover:opacity-100" title="حذف هذا التقييم">
                                  <Trash2 className="w-4 h-4" />
                               </button>
                            </td>
                         </tr>
                      ))}
                      {filteredRaw.length === 0 && <tr><td colSpan={5} className="p-10 text-center text-slate-500 font-bold bg-slate-50">لا يوجد سجلات مطابقة للبحث.</td></tr>}
                   </tbody>
                </table>
             </div>
          </motion.div>
      ...
      {/* 🚀 نافذة إعدادات البوابة والبنود */}
      <AnimatePresence>
        {isSettingsOpen && (
          <Dialog.Root open={isSettingsOpen}>
            <Dialog.Portal>
              ...
              <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white border border-slate-200 rounded-[2rem] w-[95%] max-w-xl shadow-2xl z-50 p-6 sm:p-8" dir="rtl">
                 <div className="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
                    <div className="flex items-center gap-3">
                       <div className="w-10 h-10 bg-slate-800 rounded-xl flex items-center justify-center text-white"><Settings className="w-5 h-5"/></div>
                       <div>
                         <Dialog.Title className="text-lg font-black text-slate-800">إعدادات بوابة التقييم</Dialog.Title>
                         <p className="text-[10px] font-bold text-slate-500 mt-1">التحكم بالمراحل الدراسية وبنود الاستبيان</p>
                       </div>
                    </div>
                    <button onClick={() => setIsSettingsOpen(false)} className="text-slate-400 hover:text-rose-500 bg-slate-50 p-2 rounded-full transition-colors active:scale-90"><X className="w-5 h-5"/></button>
                 </div>
                 ...
                 <div className="space-y-6">
                    ...
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-4">
                       ...
                       <div className="flex gap-4">
                          <button onClick={() => setIsMiddleActive(!isMiddleActive)} className={cn("flex-1 py-3 rounded-xl font-black text-sm transition-all border flex flex-col items-center gap-1 shadow-sm", isMiddleActive ? "bg-emerald-50 border-emerald-400 text-emerald-700" : "bg-white border-slate-200 text-slate-400")}>
                             <span>المرحلة المتوسطة</span>
                             <span className="text-[10px]">{isMiddleActive ? 'مُشغلة' : 'موقفة'}</span>
                          </button>
                          <button onClick={() => setIsHighActive(!isHighActive)} className={cn("flex-1 py-3 rounded-xl font-black text-sm transition-all border flex flex-col items-center gap-1 shadow-sm", isHighActive ? "bg-emerald-50 border-emerald-400 text-emerald-700" : "bg-white border-slate-200 text-slate-400")}>
                    ...
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
                       ...
                       <div className="flex gap-2 mb-4">
                          <input type="text" value={newCriterion} onChange={e=>setNewCriterion(e.target.value)} placeholder="أضف بند تقييم جديد..." className="flex-1 bg-white border border-slate-300 rounded-xl px-3 py-2 text-sm font-bold outline-none focus:border-indigo-500" />
                          <button onClick={addCriterion} className="bg-slate-800 text-white px-4 rounded-xl font-black hover:bg-slate-900 transition-colors"><Plus className="w-5 h-5"/></button>
                       </div>
                       ...
                       <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar pr-1">
                          {criteria.map((c, i) => (
                             <div key={i} className="flex justify-between items-center bg-white p-2.5 rounded-xl border border-slate-200 shadow-sm">
                                ...
                                <button onClick={() => removeCriterion(c)} className="text-rose-400 hover:text-rose-600 bg-rose-50 p-1.5 rounded-lg transition-colors"><Trash2 className="w-4 h-4"/></button>
                             </div>
                          ))}
                          {criteria.length === 0 && <p className="text-center text-xs font-bold text-rose-500 py-2">لا توجد بنود! لن يظهر الاستبيان للطلاب.</p>}
                       </div>
                    </div>
                 </div>

                 <button onClick={saveSettings} disabled={isSavingSettings} className="w-full mt-6 py-4 bg-slate-800 hover:bg-slate-900 text-white font-black rounded-xl shadow-lg transition-all flex justify-center items-center gap-2 active:scale-95 disabled:opacity-50">
              ...
              <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white border border-slate-200 rounded-[2rem] w-[95%] max-w-xl shadow-2xl z-50 p-6 sm:p-8" dir="rtl">
                <div className="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
                  <div className="flex items-center gap-3">
                     ...
                     <div><Dialog.Title className="text-lg font-black text-slate-800">صندوق الأسرار</Dialog.Title><p className="text-[10px] font-bold text-slate-500 mt-1">الخاصة بالمعلم: <span className="text-indigo-600">{selectedTeacher.name}</span></p></div>
                  </div>
