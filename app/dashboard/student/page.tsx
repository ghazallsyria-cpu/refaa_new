...
import { 
  ...
  Ticket, Timer, FileKey, Download, ShieldCheck, ScrollText, Coins, Send, PartyPopper, Wallet
} from 'lucide-react';
...
export default function StudentDashboard() {
  ...
  // 🌟 حالات إعدادات المنصة والحاجب المالي 🌟
    ...
    const loadDashboardData = async () => {
      ...
      try {
        ...
        if (data && data.student) {
          ...
          if (studentId) {
              const [ badgesRes, gradesRes, absentCountRes, totalCountRes, excusesRes, trackRes, docsRes, settingsRes, resultRes, stdFinanceRes ] = await Promise.all([
                ...
                // 🌟 التأكد الدقيق من الحالة المالية للطالب من الجدول الأساسي لتجاوز الكاش
  ...
  const handleAddDate = () => { if (!currentDateInput) return; if (excuseForm.absent_dates.includes(currentDateInput)) return; setExcuseForm(prev => ({ ...prev, absent_dates: [...prev.absent_dates, currentDateInput].sort() })); };
  ...
  const handleSubmitExcuse = async () => {
    ...
    try {
      ...
      if (error) throw error;
      alert('تم تقديم العذر بنجاح!'); setIsExcuseModalOpen(false); setExcuseForm({ absent_dates: [format(new Date(), 'yyyy-MM-dd')], duration_type: 'full_day', target_periods: [], reason: '', attachment_url: '', cloudinary_public_id: '' }); 
      window.location.reload(); 
  ...
  if (isChecking || loading || !mounted) return <div className="flex h-screen items-center justify-center bg-transparent"><Loader2 className="w-12 h-12 animate-spin text-blue-500 drop-shadow-md" /></div>;
  if (authRole !== 'student') return <div className="flex h-screen items-center justify-center bg-transparent text-white font-bold">وصول مقيد. للطلاب فقط.</div>;
  ...
  // تصنيف المرحلة الأكاديمية للطالب
  ...
  const isHighSchool = !isMiddleSchool; // عاشر، حادي عشر، ثاني عشر
  ...
  // 🌟 منطق إظهار النتائج (Suspense & Results Logic) 🌟
  const isSuspenseMode = platformSettings?.results_suspense_mode;
  ...
  const hideOldContent = isSuspenseMode || isMyResultPublished;
  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={containerVariants} className="min-h-screen relative bg-transparent text-slate-100 pb-32 font-sans pt-2 sm:pt-6" dir="rtl">
      <div className="space-y-6 sm:space-y-8 max-w-7xl mx-auto relative z-10">
        ...
        <motion.div variants={itemVariants} className="relative overflow-hidden rounded-[2rem] sm:rounded-[3rem] glass-panel p-6 sm:p-10 border-blue-500/30 group shadow-lg">
          ...
          <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-8">
            ...
            {/* إخفاء الإحصائيات إذا تم تفعيل الحجب أو الترقب */}
            {!hideOldContent && (
        ...
        {/* 🌟🌟 نظام الترقب وإعلان النتائج (The Results Nexus) 🌟🌟 */}
        <AnimatePresence mode="wait">
          {isSuspenseMode && !isMyResultPublished && (
             <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative overflow-hidden rounded-[2.5rem] glass-panel p-10 sm:p-16 border-amber-500/30 text-center shadow-[0_0_50px_rgba(245,158,11,0.15)] flex flex-col items-center justify-center min-h-[40vh]">
                <div className="absolute inset-0 bg-gradient-to-t from-amber-900/20 to-transparent"></div>
                <div className="w-20 h-20 bg-amber-500/10 rounded-[2rem] flex items-center justify-center border border-amber-500/30 shadow-[0_0_30px_rgba(245,158,11,0.3)] mb-6 animate-[bounce_3s_infinite] z-10">
                   <Clock className="w-10 h-10 text-amber-400 drop-shadow-md" />
                </div>
                <h2 className="text-3xl sm:text-5xl font-black text-white mb-4 drop-shadow-xl z-10">ترقبوا إعلان النتائج قريباً!</h2>
                <p className="text-amber-200/80 font-bold text-sm sm:text-lg max-w-xl leading-relaxed z-10">
                   اللجان تعمل على قدم وساق للتدقيق والمراجعة. سيتم إظهار نتيجتك النهائية هنا فور اعتمادها من الإدارة العليا. نتمنى لك التوفيق والنجاح.
                </p>
             </motion.div>
          )}

          {isMyResultPublished && (
             <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className={`relative overflow-hidden rounded-[2.5rem] glass-panel p-8 sm:p-12 text-center shadow-2xl flex flex-col items-center justify-center min-h-[40vh] ${isFinanciallyBlocked ? 'border-rose-500/40' : 'border-emerald-500/40'}`}>
                {isFinanciallyBlocked ? (
                   // الطالب عليه ذمم مالية (حجب النتيجة)
                   <>
                      <div className="absolute inset-0 bg-gradient-to-t from-rose-900/20 to-transparent"></div>
                      <div className="w-20 h-20 bg-rose-500/10 rounded-[2rem] flex items-center justify-center border border-rose-500/30 shadow-[0_0_30px_rgba(225,29,72,0.3)] mb-6 z-10">
                         <Lock className="w-10 h-10 text-rose-400 drop-shadow-md" />
                      </div>
                      <h2 className="text-2xl sm:text-4xl font-black text-white mb-4 drop-shadow-xl z-10">نتيجة {classNameStr} جاهزة ومُعتمدة</h2>
                      <div className="bg-[#02040a]/60 backdrop-blur-md p-6 rounded-2xl border border-rose-500/20 max-w-2xl z-10 text-right">
                         <p className="text-rose-200 font-bold text-sm sm:text-base leading-relaxed">
                            <strong className="text-rose-400 font-black">عزيزي ولي الأمر،</strong> نبارك لنجلكم إنهاء العام الدراسي. يرجى التكرم بتسوية الرصيد المالي المتبقي لدى قسم المحاسبة لتفعيل إصدار الشهادة الرقمية فوراً، ولتتمكنوا من استلام النسخة الورقية الرسمية المعتمدة من سكرتارية المدرسة.
                         </p>
                      </div>
                      <button className="mt-8 px-8 py-4 bg-gradient-to-r from-rose-600 to-rose-500 hover:from-rose-500 hover:to-rose-400 text-white rounded-xl font-black text-sm transition-all shadow-[0_0_20px_rgba(225,29,72,0.4)] flex items-center gap-2 z-10 active:scale-95 border border-rose-400">
                         <Wallet className="w-5 h-5" /> تسوية الرصيد إلكترونياً
                      </button>
                   </>
                ) : (
                   // الطالب مسدد ومصرح له بالنتيجة
                   <>
                      ...
                      <h2 className="text-xl sm:text-2xl font-black text-emerald-100 mb-2 drop-shadow-md z-10">ألف مبروك النجاح والتفوق!</h2>
                      <div className="text-6xl sm:text-8xl font-black text-white mb-6 drop-shadow-[0_0_30px_rgba(16,185,129,0.6)] z-10 tracking-tighter" dir="ltr">
                         {finalResult ? `${finalResult}%` : '---'}
                      </div>
                      <p className="text-emerald-200/80 font-bold text-sm sm:text-base max-w-xl leading-relaxed z-10 mb-8 bg-[#02040a]/40 p-4 rounded-xl border border-emerald-500/20 shadow-inner">
                      ...
                      <button className="px-8 py-4 bg-emerald-600 hover:bg-emerald-500 text-slate-900 rounded-xl font-black text-sm transition-all shadow-[0_0_30px_rgba(16,185,129,0.5)] flex items-center gap-2 z-10 active:scale-95 border border-emerald-400">
                         <Download className="w-5 h-5" /> تحميل إشعار النتيجة الرقمي (PDF)
                      </button>
                   </>
                )}
             </motion.div>
        ...
        {/* 🚀 إخفاء باقي عناصر لوحة الطالب إذا كان وضع الترقب أو النتائج مفعلاً 🚀 */}
        {!hideOldContent && (
           <>
              {/* الهوية الامتحانية */}
              <AnimatePresence>
              ...
              {/* الأوسمة والمجلس والإنذار والعداد */}
              {myBadges.length > 0 && (
      ...
      {/* 🚀 بوابة التقييم الإجبارية المستقلة عن الإخفاء */}
      {studentData?.id && (
