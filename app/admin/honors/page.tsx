...
import ImageUpload from '@/components/ImageUpload'; // 👈 استدعاء المكون الاحترافي الخاص بك (تأكد من المسار)
...
export default function AdminHonorsDashboard() {
  ...
  // 🚀 حالات جديدة للتعامل مع مكون الصور الخاص بك
  const [imageUrl, setImageUrl] = useState<string>(''); // سيستقبل الرابط الجاهز من المكون
  const [resetKey, setResetKey] = useState(0); // خدعة برمجية ذكية لإعادة تصفير المكون بعد الحفظ
  ...
  const handleAddStudent = async (e: React.FormEvent) => {
    ...
    // الحفظ في قاعدة البيانات مباشرة (الرابط أصبح جاهزاً من مكون ImageUpload)
    const { error } = await supabase.from('top_students').insert([{ 
      student_name: studentName, 
      grade_level: activeTab, 
      percentage: parseFloat(percentage),
      image_url: imageUrl || null // 👈 نأخذ الرابط النهائي هنا
    }]);
    ...
    if (!error) {
      ...
      setStudentName('');
      setPercentage('');
      setImageUrl(''); // تصفير الرابط
      setResetKey(prev => prev + 1); // 👈 تغيير الـ key يجبر مكون ImageUpload على العودة لشكله الأصلي (حذف الصورة السابقة)
      fetchStudents();
    } else {
          ...
          <div className="md:col-span-1">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 sticky top-8">
              ...
              <form onSubmit={handleAddStudent} className="space-y-4">
                ...
                {/* 🚀 استدعاء المكون الخاص بك هنا */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">صورة الطالب (اختياري)</label>
                  <ImageUpload 
                    key={resetKey} // هذه الخدعة تجعل المكون يفرغ نفسه تلقائياً بعد نجاح الإضافة
