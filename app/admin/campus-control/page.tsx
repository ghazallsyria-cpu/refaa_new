...
export default function CampusControlPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [studioRes, magazineRes, annRes, tickerRes, ribbonRes] = await Promise.all([
        ...
        supabase.from('school_ribbon').select('*').eq('id', 1).maybeSingle()
      ]);
  ...
  const handleSaveAnnounce = async (e: React.FormEvent) => { 
    ...
    try { 
      ...
      setAnnounceForm({ title: '', tag: 'إعلان عام' }); 
      fetchData(); 
      showToast('تم النشر', 'success'); 
    } catch (e) { 
      showToast('خطأ', 'error'); 
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 relative z-10">
            ...
            <div className="flex flex-wrap sm:flex-nowrap bg-black/40 backdrop-blur-md p-1.5 rounded-[1.5rem] border border-white/5 relative shadow-inner gap-1">
              {[
                ...
                { id: 'ribbon', icon: Bookmark, label: 'الوشاح' }
              ].map((tab) => (
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'studio' && (
              <motion.div key="studio" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-8">
                <div className="flex justify-between items-center">
                  ...
                  <button onClick={() => setIsStudioModalOpen(true)} className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-3.5 rounded-2xl font-black text-sm flex items-center gap-2 shadow-[0_0_20px_rgba(79,70,229,0.3)] transition-all active:scale-95"><Plus className="w-5 h-5"/> رفع ميديا</button>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                  {studioItems.length === 0 ? (
                    <div className="col-span-full py-20 text-center rounded-[3rem] border border-white/5 bg-[#0f1423]/30">
                      <p className="text-slate-400 font-black">فارغ حالياً.</p>
                    </div>
                  ) : (
                    studioItems.map((item) => (
                      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} key={item.id} className="group relative aspect-[4/5] rounded-[2rem] overflow-hidden bg-[#0f1423] border border-white/5 shadow-lg">
                        ...
                        <div className="absolute top-4 right-4 flex gap-2">
                          <span className="bg-black/50 backdrop-blur-md px-3 py-1.5 rounded-xl border border-white/10 text-[10px] font-black text-white shadow-sm flex items-center gap-1.5">{item.media_type === 'video' ? <Video className="w-3.5 h-3.5 text-amber-400" /> : <ImageIcon className="w-3.5 h-3.5 text-indigo-400" />} {item.media_type === 'video' ? 'فيديو' : 'صورة'}</span>
                        </div>
                        <button onClick={() => handleDeleteStudio(item)} className="absolute top-4 left-4 w-10 h-10 bg-rose-500/80 hover:bg-rose-500 text-white rounded-xl flex items-center justify-center backdrop-blur-md transition-all border border-white/10 active:scale-95 shadow-xl z-20"><Trash2 className="w-5 h-5" /></button>
            ...
            {activeTab === 'magazine' && (
              <motion.div key="magazine" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-8">
                <div className="flex justify-between items-center">
                  ...
                  <button onClick={() => setIsMagazineModalOpen(true)} className="bg-emerald-500 hover:bg-emerald-400 text-[#02040a] px-6 py-3.5 rounded-2xl font-black text-sm flex items-center gap-2 shadow-[0_0_20px_rgba(16,185,129,0.3)] transition-all active:scale-95"><Plus className="w-5 h-5"/> صياغة خبر</button>
                </div>
            ...
            {activeTab === 'announcements' && (
              <motion.div key="announcements" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-8">
                <div className="flex justify-between items-center">
                  <h2 className="text-2xl font-black text-white">إعلانات سريعة</h2>
                  <button onClick={() => setIsAnnounceModalOpen(true)} className="bg-rose-500 hover:bg-rose-400 text-white px-6 py-3.5 rounded-2xl font-black text-sm flex items-center gap-2 shadow-[0_0_20px_rgba(244,63,94,0.3)] transition-all active:scale-95"><Plus className="w-5 h-5" /> إضافة</button>
                </div>
            ...
            {activeTab === 'ticker' && (
              <motion.div key="ticker" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-8">
                <div className="flex justify-between items-center">
                  ...
                  <button onClick={() => setIsTickerModalOpen(true)} className="bg-amber-500 hover:bg-amber-400 text-black px-6 py-3.5 rounded-2xl font-black text-sm flex items-center gap-2 shadow-[0_0_20px_rgba(245,158,11,0.3)] transition-all active:scale-95"><Plus className="w-5 h-5" /> خبر عاجل</button>
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#02040a]/90 backdrop-blur-2xl">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#0f1423] w-full max-w-xl rounded-[3rem] border border-white/10 overflow-hidden shadow-2xl relative">
              ...
              <form onSubmit={handleSaveStudio} className="p-8 pt-4 space-y-6">
                <div onClick={() => !isUploading && fileInputRef.current?.click()} className={`w-full h-56 rounded-[2rem] border-2 border-dashed flex flex-col items-center justify-center cursor-pointer transition-all relative overflow-hidden ${studioForm.media_url ? 'border-emerald-500/50 bg-emerald-500/5' : 'border-indigo-500/30 bg-indigo-500/5 hover:bg-indigo-500/10'}`}>
                  ...
                  {isUploading ? (
                    ...
                    <div className="text-center z-10">
                      ...
                      <p className="text-emerald-400 font-black text-sm">تم التجهيز بنجاح</p>
                    </div>
                  ) : (
                    <div className="text-center z-10">
                      <UploadCloud className="w-14 h-14 text-indigo-400 mx-auto mb-3 opacity-80" />
                      <p className="text-white font-black text-sm">انقر لاختيار ملف</p>
                    </div>
                  )}
                ...
                <input type="text" value={studioForm.title} onChange={e => setStudioForm({...studioForm, title: e.target.value})} placeholder="العنوان (اختياري)" className="w-full bg-[#02040a] border border-white/5 rounded-2xl p-5 text-white font-bold outline-none focus:border-indigo-500/50" />
                <button type="submit" disabled={isSubmitting || !studioForm.media_url} className="w-full bg-indigo-600 hover:bg-indigo-500 text-white py-5 rounded-2xl font-black shadow-lg disabled:opacity-50">اعتماد ونشر للرئيسية</button>
              </form>
            </motion.div>
          </div>
        )}
              ...
              <form onSubmit={handleSaveMagazine} className="p-8 pt-4 space-y-6">
                <div onClick={() => !isUploading && fileInputRef.current?.click()} className={`w-full h-40 rounded-[2rem] border-2 border-dashed flex items-center justify-center cursor-pointer overflow-hidden relative ${magazineForm.cover_image ? 'border-emerald-500/50' : 'border-white/10 hover:border-white/20'}`}>
                  ...
                  {isUploading ? (
                    ...
                    <img src={magazineForm.cover_image} className="w-full h-full object-cover" />
                  ) : (
                    <div className="text-center">
                      <ImageIcon className="w-10 h-10 text-slate-500 mx-auto mb-2" />
                      <p className="text-slate-400 text-sm font-black">رفع صورة الغلاف</p>
                    </div>
                  )}
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <input type="text" value={magazineForm.title} onChange={e => setMagazineForm({...magazineForm, title: e.target.value})} placeholder="عنوان الخبر..." className="w-full bg-[#02040a] border border-white/5 rounded-2xl p-4 text-white font-bold outline-none focus:border-emerald-500/50" />
                  <input type="text" value={magazineForm.author_name} onChange={e => setMagazineForm({...magazineForm, author_name: e.target.value})} placeholder="الكاتب" className="w-full bg-[#02040a] border border-white/5 rounded-2xl p-4 text-white font-bold outline-none focus:border-emerald-500/50" />
                </div>
                <textarea rows={3} value={magazineForm.excerpt} onChange={e => setMagazineForm({...magazineForm, excerpt: e.target.value})} placeholder="مقتطف قصير..." className="w-full bg-[#02040a] border border-white/5 rounded-2xl p-4 text-white font-bold outline-none focus:border-emerald-500/50 resize-none" />
                <label className="flex items-center gap-3 cursor-pointer p-5 bg-[#02040a] rounded-2xl border border-white/5 hover:border-white/10 transition-colors">
                  ...
                  <span className="text-sm font-black text-white flex items-center gap-2"><Star className="w-4 h-4 text-emerald-400" /> تثبيت كخبر رئيسي</span>
                </label>
                <button type="submit" disabled={isSubmitting || !magazineForm.cover_image} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white py-5 rounded-2xl font-black shadow-lg disabled:opacity-50">نشر في المجلة</button>
              </form>
            </motion.div>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#02040a]/90 backdrop-blur-2xl">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#0f1423] w-full max-w-lg rounded-[3rem] border border-white/10 overflow-hidden shadow-2xl relative">
              ...
              <form onSubmit={handleSaveAnnounce} className="p-8 pt-4 space-y-5">
                <input type="text" value={announceForm.title} onChange={e => setAnnounceForm({...announceForm, title: e.target.value})} placeholder="نص الإعلان..." className="w-full bg-[#02040a] border border-white/5 rounded-2xl p-4 text-white font-bold outline-none focus:border-rose-500/50" required />
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#02040a]/90 backdrop-blur-2xl">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#0f1423] w-full max-w-lg rounded-[3rem] border border-white/10 overflow-hidden shadow-2xl relative">
              ...
              <form onSubmit={handleSaveTicker} className="p-8 pt-4 space-y-5">
                <textarea rows={4} value={tickerForm.content} onChange={e => setTickerForm({...tickerForm, content: e.target.value})} placeholder="الخبر العاجل..." className="w-full bg-[#02040a] border border-white/5 rounded-2xl p-4 text-white font-bold outline-none focus:border-amber-500/50 resize-none" required />
