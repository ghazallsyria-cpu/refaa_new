...
Heading1, Heading2, AlignRight, AlignCenter, AlignLeft, AlignJustify,
Palette, BarChart3, X, Calculator
} from 'lucide-react';

import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid } from 'recharts';

// ==========================================
// إضافة مخصصة للرسم البياني (Chart Node)
// ==========================================
  ...
  {isEditing ? (
    <div className="p-4 bg-white rounded-lg border border-slate-200" dir="ltr">
      ...
      <div className="mt-2 flex justify-end gap-2" dir="rtl">
        <button onClick={saveChartData} className="px-4 py-2 bg-indigo-600 text-white text-sm font-bold rounded hover:bg-indigo-700">حفظ</button>
...
<div className="border border-slate-200 rounded-[1.5rem] overflow-hidden bg-white shadow-sm focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-200 transition-all font-sans">
  ...
  <div className="bg-slate-50/95 backdrop-blur-md border-b border-slate-200 p-2 flex flex-wrap items-center gap-1 sticky top-0 z-20" dir="rtl">
    ...
    {/* الأحجام باستخدام العناوين بدلاً من الخطوط لتجنب أخطاء TypeScript */}
    <div className="flex items-center gap-0.5 border-l border-slate-300 pl-2 ml-1">
      <ToolbarButton icon={Heading1} onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} isActive={editor.isActive('heading', { level: 2 })} title="عنوان كبير" />
      <ToolbarButton icon={Heading2} onClick={() => editor.chain().focus().toggleHeading({ level: 3 }).run()} isActive={editor.isActive('heading', { level: 3 })} title="عنوان متوسط" />
    </div>
    ...
    <div className="flex items-center gap-0.5 border-l border-slate-300 pl-2 ml-1 relative">
      <button type="button" onClick={() => { setShowColorPicker(!showColorPicker); setShowMathUI(false); }} className={`p-2 rounded-lg ${showColorPicker ? 'bg-indigo-100 text-indigo-700' : 'text-slate-600 hover:bg-slate-200'}`} title="لون النص">
    ...
    <div className="flex items-center gap-0.5 border-l border-slate-300 pl-2 ml-1 relative">
      <ToolbarButton icon={LinkIcon} onClick={() => { setShowLinkInput(!showLinkInput); setShowMathUI(false); setShowColorPicker(false); }} isActive={editor.isActive('link') || showLinkInput} title="إضافة رابط" />
      ...
      <button type="button" onClick={() => { setShowMathUI(!showMathUI); setShowLinkInput(false); setShowColorPicker(false); }} className={`p-2 rounded-lg ${showMathUI ? 'bg-pink-100 text-pink-700' : 'text-slate-600 hover:bg-slate-200'}`} title="كتابة معادلات رياضية">
