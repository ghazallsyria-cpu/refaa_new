...
export default function AssignmentBuilder({ questions, onChange }: AssignmentBuilderProps) {
  ...
  const addQuestion = (type: string = 'text') => {
    ...
    if (type === 'comparison') {
      newQuestion.options = ['الطرف الأول', 'الطرف الثاني', 'وجه المقارنة الأول'];
    } else if (type === 'data_table') {
      // 🚀 الهيكلية الجديدة لجدول البيانات الديناميكي
      newQuestion.table = {
        headers: ['العنصر', 'C', 'H'],
        rows: [
          ['الكتل بالجرام', '', ''],
  ...
  // وظائف خيارات الخيارات المتعددة والمقارنة
  const addOption = (questionId: string, isAspect: boolean = false) => {
    ...
    if (question) {
      ...
      updateQuestion(questionId, { options: [...currentOptions, newLabel] });
    }
  };
  ...
  // 🚀 وظائف إدارة جدول البيانات الديناميكي (Data Table)
            ...
            >
              ...
              <div className="flex flex-col gap-6 pt-2 sm:pt-4">
                <div className="flex flex-col lg:flex-row gap-6 items-start">
                  ...
                  {/* إعدادات السؤال (القائمة المنسدلة) */}
                  {!isHeader && (
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            } else if (type === 'comparison' && !question.options) {
                ...
                {/* 🚀 إعدادات جدول البيانات الديناميكي (الجديد) */}
                        ...
                        <div key={`row-${rowIdx}`} className="flex border-b border-white/5 last:border-b-0 bg-[#02040a]/40 relative group hover:bg-[#0f1423]/50 transition-colors">
                          {row.map((cell: string, colIdx: number) => (
                            <div key={`cell-${rowIdx}-${colIdx}`} className="flex-1 min-w-[120px] border-l border-white/5 last:border-l-0 p-2">
                              {colIdx === 0 ? (
                                // الخلية الأولى في كل صف (عنوان الصف)
                                <input
                                  type="text" dir="auto"
                                  value={cell}
                                ...
                                // خلايا التعبئة (تترك فارغة أو بها نص)
                                <input
                                  type="text" dir="auto"
                                  value={cell}
                                  onChange={(e) => updateTableCell(question.id, rowIdx, colIdx, e.target.value)}
                ...
                {/* الشريط السفلي (النقاط، الإلزامية، والحذف) */}
