 ...
 * 1. 🎀 إدارة الوشاح (Ribbon - Preview Edition): رفع/إزالة البانر العمودي، 
 * وتمت هندسة الـ Preview ليتدلى مباشرة من سقف القسم ليعطي تصوراً نهائياً ودقيقاً.
 ...
 * * * 🛠️ هندسة العرض المحدثة (Ribbon Fix):
 * - تم حذف الصندوق الداكن الطافي الذي يحيط بالوشاح المرفوع.
 * - تم جعل الوشاح يتدلى مباشرة من "سقف" الحاوية، ويشغل الحجم الكامل (Preview).
 * - استخدام خاصية `clip-path` لقص الوشاح من الأسفل على شكل وسام.
 * * * @version     3.1.0 (Ribbon Preview Fix)
 * @date        مايو 2026
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12 relative z-10">
        ...
        {isLoading ? (
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'ribbon' && (
              <motion.div key="ribbon" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-8 relative">
                
                {/* 1. قسم التحكم العلوي (العنوان والأزرار) */}
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-[#0f1423] p-8 rounded-[2rem] border border-white/5 shadow-inner">
                  <div>
                    <h2 className="text-2xl font-black text-white flex items-center gap-3"><Bookmark className="text-rose-500" /> الوشاح المتدلي (التحكم)</h2>
                    <p className="text-slate-500 text-xs font-bold mt-1 max-w-xl leading-relaxed">بانر طولي يتدلى من أعلى الشاشة الرئيسية (للمناسبات والأعياد). العرض التقديمي أدناه يتدلى من سقف القسم لمعاينة الشكل النهائي بدقة.</p>
                  </div>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    {ribbonUrl && (
                      <button onClick={handleDeleteRibbon} className="w-14 h-14 bg-rose-500/10 hover:bg-rose-500/20 text-rose-500 rounded-2xl flex items-center justify-center backdrop-blur-xl border border-rose-500/20 shadow-inner active:scale-95 transition-all"><Trash2 className="w-6 h-6" /></button>
                    )}
                  </div>
                </div>

                {/* 2. قسم العرض التقديمي (Preview) - التدلي من "سقف" القسم المظلم */}
                <div className="flex justify-center bg-[#070b14] p-10 py-16 rounded-[2rem] border border-white/5 shadow-[inset_0_20px_50px_rgba(0,0,0,0.5)] relative overflow-hidden h-[600px]">
                  ...
                  {ribbonUrl ? (
                    <motion.div 
                      ...
                      className="relative w-48 sm:w-56 lg:w-64 h-full shadow-[0_20px_50px_rgba(0,0,0,0.8)] border-x border-b border-white/10"
                      style={{ 
                    ...
                    >
                      <img src={ribbonUrl} alt="Hanging Ribbon (Preview)" className="w-full h-full object-cover opacity-90" />
                      <div className="absolute inset-0 bg-gradient-to-b from-black/10 via-transparent to-black/60 mix-blend-overlay"></div>
                    </motion.div>
                  ) : (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 flex flex-col items-center justify-center opacity-30 text-center gap-4 px-10">
                      <Bookmark className="w-20 h-20 text-slate-700" />
                      <p className="text-slate-500 font-black text-xl leading-snug">لا يوجد وشاح معلق حالياً.<br /><span className="text-sm font-bold text-slate-600">ارفع صورة علم أو بانر طولي من الأعلى لظهوره هنا.</span></p>
                    </motion.div>
                  )}
                </div>
