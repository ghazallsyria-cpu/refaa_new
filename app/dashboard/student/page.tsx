...
const PLATFORM_SERVICES = [
  ...
  { id: 6, title: 'صندوق الرسائل', desc: 'تواصل آمن وفعال مع الإدارة', icon: MessagesSquare, color: 'rose', link: '/messages' },
];
...
export default function StudentDashboard() {
  ...
  if (isChecking || loading || !mounted) return <div className="flex h-screen items-center justify-center bg-[#02040a]"><Loader2 className="w-12 h-12 animate-spin text-blue-500 drop-shadow-md" /></div>;
  if (authRole !== 'student') return <div className="flex h-screen items-center justify-center bg-[#02040a] text-white font-bold">وصول مقيد. للطلاب فقط.</div>;
  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={containerVariants} className="min-h-screen relative bg-[#02040a] text-slate-100 pb-32 font-sans pt-2 sm:pt-6" dir="rtl">
      <div className="space-y-6 sm:space-y-8 max-w-7xl mx-auto relative z-10 px-4 sm:px-6 lg:px-8">

        {studentData?.id && <MemorialShieldDisplay userId={studentData.id} role="student" />}
           ...
           <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className={`relative overflow-hidden rounded-[2.5rem] glass-panel p-8 sm:p-12 text-center shadow-2xl flex flex-col items-center justify-center min-h-[50vh] ${isFinanciallyBlocked ? 'border-rose-500/40' : 'border-emerald-500/40'}`}>
              {isFinanciallyBlocked ? (
                 <>
                    ...
                    <button className="mt-8 px-8 sm:px-12 py-4 sm:py-5 bg-gradient-to-r from-rose-600 to-rose-500 hover:from-rose-500 hover:to-rose-400 text-white rounded-2xl font-black text-sm sm:text-base transition-all shadow-[0_0_30px_rgba(225,29,72,0.5)] flex items-center gap-3 z-10 active:scale-95 border border-rose-400">
                       <Wallet className="w-6 h-6" /> تسوية الرصيد إلكترونياً
                    </button>
                 </>
              ) : (
                 <>
                    ...
                    <button className="px-8 sm:px-10 py-4 sm:py-5 bg-emerald-600 hover:bg-emerald-500 text-slate-900 rounded-2xl font-black text-sm sm:text-base transition-all shadow-[0_0_30px_rgba(16,185,129,0.5)] flex items-center gap-2 z-10 active:scale-95 border border-emerald-400">
        ...
        {showMiddleSchoolSuspense && (
           <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative overflow-hidden rounded-[2.5rem] glass-panel p-10 sm:p-16 border-amber-500/30 text-center shadow-[0_0_50px_rgba(245,158,11,0.15)] flex flex-col items-center justify-center min-h-[50vh]">
        ...
        {showHighSchoolExamMode && (
          <div className="space-y-8">
            {/* Exam Hero Banner */}
            <motion.div variants={itemVariants} className="relative overflow-hidden rounded-[2rem] sm:rounded-[3rem] glass-panel p-8 sm:p-12 border-rose-500/30 group shadow-2xl">
              ...
              <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-8">
                <div className="flex flex-col sm:flex-row items-center gap-6 text-center sm:text-right">
                  <div className="h-28 w-28 sm:h-32 sm:w-32 rounded-[2.5rem] overflow-hidden border-2 border-rose-500/30 bg-[#0f1423]/80 backdrop-blur-xl flex items-center justify-center relative shadow-[0_0_30px_rgba(225,29,72,0.2)]">
                    {avatarUrl ? <img src={avatarUrl} alt="av" className="w-full h-full object-cover" /> : <span className="text-4xl font-black text-rose-400">{rawFullName.charAt(0)}</span>}
                  </div>
                  <div>
                    <div className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-rose-500/20 backdrop-blur-md border border-rose-500/30 text-xs font-black uppercase tracking-widest mb-3 shadow-inner text-rose-300">
                      <ShieldAlert className="w-3.5 h-3.5" /> <span>وضع التركيز للاختبارات</span>
                    </div>
                    <h1 className="text-3xl sm:text-5xl font-black mb-3 leading-tight drop-shadow-xl text-white">بالتوفيق يا {displayFirstName} 🎯</h1>
                    <p className="text-slate-300 font-bold text-sm sm:text-base drop-shadow-md max-w-xl leading-relaxed">أنت الآن في فترة الاختبارات النهائية. تم تجميد الواجبات والحصص اليومية لتتفرغ تماماً لمراجعة موادك، نثق بقدراتك!</p>
                  </div>
                </div>
              </div>
            </motion.div>
                  ...
                  <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="relative overflow-hidden rounded-[2rem] sm:rounded-[3rem] glass-panel p-6 sm:p-10 border-indigo-500/30 flex flex-col lg:flex-row items-center justify-between gap-8 group">
                    ...
                    <div className="w-full md:hidden bg-[#02040a]/60 backdrop-blur-xl border border-white/10 rounded-3xl p-5 shadow-inner flex items-center gap-4 relative z-10">
                        ...
                        <div className="shrink-0 text-center border-r border-white/10 pr-4">
                          ...
                          <p className="text-xl font-black text-indigo-400 tracking-widest drop-shadow-sm">{seatAllocation.seat_number}</p>
                        </div>
                    </div>
                  </motion.div>
              )}
            ...
            {/* Graduation Docs for 12th Grade - inside exam mode */}
            {isTwelfthGrade && (
                  ...
                  <div className="glass-panel border-fuchsia-500/30 rounded-[2.5rem] p-6 sm:p-10 shadow-[0_0_40px_rgba(192,38,211,0.1)] relative overflow-hidden group">
                      ...
                      <div className="flex flex-col md:flex-row items-center gap-6 mb-8 relative z-10 text-center md:text-right border-b border-white/5 pb-8">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-sm font-bold text-slate-300 leading-relaxed max-w-3xl opacity-90 drop-shadow-sm">مبارك تخرجك المرتقب! لراحتك وسرعة تقديمك للجامعات، وفرنا لك خدمة استخراج الوثائق وتصديقها من الوزارة عن طريق مندوب المدرسة.</p>
                        </div>
                      </div>
        ...
        {showRegularDashboard && (
          <>
            ...
            {/* Graduation Docs for 12th Grade - inside regular mode */}
            {isTwelfthGrade && (
                  ...
                  <div className="glass-panel border-fuchsia-500/30 rounded-[2.5rem] p-6 sm:p-10 shadow-[0_0_40px_rgba(192,38,211,0.1)] relative overflow-hidden group">
                      ...
                      <div className="flex flex-col md:flex-row items-center gap-6 mb-8 relative z-10 text-center md:text-right border-b border-white/5 pb-8">
                        ...
                        <div className="flex-1">
                            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-fuchsia-500/10 text-fuchsia-300 text-[10px] sm:text-xs font-black uppercase tracking-widest mb-2 border border-fuchsia-500/20 shadow-inner backdrop-blur-sm">خاص بخريجي الثاني عشر 🎓</div>
                            <h2 className="text-2xl sm:text-3xl font-black text-white mb-2 drop-shadow-xl">بوابة طلب الوثائق والتصديقات الرسمية</h2>
                            <p className="text-sm font-bold text-slate-300 leading-relaxed max-w-3xl opacity-90 drop-shadow-sm">مبارك تخرجك! لراحتك وسرعة تقديمك للجامعات، وفرنا لك خدمة استخراج الوثائق وتصديقها من الوزارة عن طريق مندوب المدرسة.</p>
                        </div>
                      </div>
                      <div className="bg-rose-500/10 backdrop-blur-md border border-rose-500/20 rounded-2xl p-4 sm:p-5 mb-8 relative z-10 flex items-start sm:items-center gap-4 text-rose-200 shadow-inner">
                        <AlertTriangle className="w-6 h-6 sm:w-8 sm:h-8 shrink-0 text-rose-400 animate-pulse drop-shadow-md" />
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                    <div className="flex items-start gap-4 sm:gap-6 w-full md:w-auto flex-1">
                      ...
                      <div>
                        <h3 className="text-xl sm:text-2xl font-black mb-2 tracking-tight flex items-center gap-2 drop-shadow-lg text-white">{warningTitle}</h3>
                        <p className="text-sm sm:text-base font-bold opacity-90 leading-relaxed max-w-2xl text-slate-200 drop-shadow-sm">{warningMessage}</p>
            ...
            {nextOfficialExam && (
              <motion.div variants={itemVariants} className="relative overflow-hidden rounded-[2rem] sm:rounded-[2.5rem] glass-panel p-6 sm:p-8 border-rose-500/20 flex flex-col md:flex-row gap-6 items-center justify-between text-center md:text-right group">
                ...
                <div className="relative z-10 w-full flex flex-col items-center justify-center text-center">
                    ...
                    <div className="text-4xl sm:text-5xl md:text-6xl font-black text-white drop-shadow-xl tracking-widest" dir="rtl">{countdownStr}</div>
                          ...
                          return (
                            <div key={i} className={cn("relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group", current ? "is-active z-20" : "z-10")}>
                              ...
                              <div className={cn("w-[calc(100%-3.5rem)] sm:w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 sm:p-5 rounded-[1.5rem] sm:rounded-3xl border transition-all duration-500 backdrop-blur-xl relative overflow-hidden shadow-inner cursor-default", 
                                ...
                                <div className={`absolute top-0 left-0 w-1 h-full ${current ? 'bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.8)]' : isPast ? 'bg-slate-800' : next ? 'bg-blue-400' : 'bg-white/10'}`}></div>
