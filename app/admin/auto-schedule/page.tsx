...
import { 
  ...
  Loader2, Save, X, CalendarDays, Clock, Users, Trash2, SlidersHorizontal, Layers, CheckSquare, Ban, Briefcase, UserCog, LayoutGrid, List, MousePointerClick, AlertOctagon, Repeat, Activity, XCircle, CheckCircle, CloudDownload, Video, Printer, FileDown, CheckSquare2, Square, ArrowLeftRight, Hand
} from 'lucide-react';
...
export default function AutoScheduleGenerator() {
  ...
  // حالات السحب والإفلات والتعديل
  const [isSwapMode, setIsSwapMode] = useState(false);
  const [swapSource, setSwapSource] = useState<{day: number, period_number: number, slot: any} | null>(null);
  const [draggedItem, setDraggedItem] = useState<{type: 'grid'|'unplaced', day?: number, period_number?: number, slot?: any, lesson?: any} | null>(null);
  ...
  useEffect(() => {
    if (generatedSchedules.length > 0 && !gridFilterId) {
      if (gridFilterType === 'section') {
        const firstSection = sections.find(s => generatedSchedules.some(gs => String(gs.section_id) === String(s.id)));
        if (firstSection) setGridFilterId(String(firstSection.id));
      } else {
        const firstTeacherId = generatedSchedules[0]?.teacher_id;
        if (firstTeacherId) setGridFilterId(String(firstTeacherId));
      }
    }
  ...
  const fetchMasterData = async () => {
    try {
      ...
      formattedSections.sort((a,b) => a.level - b.level || String(a.name).localeCompare(String(b.name)));
      ...
      if (cleanTsData && formattedSections.length > 0) {
        ...
        cleanTsData.forEach(ts => {
          const sec = formattedSections.find(s => String(s.id) === String(ts.section_id));
        ...
        subjClassList.sort((a, b) => String(a.class_name || '').localeCompare(String(b.class_name || '')) || String(a.subj_name || '').localeCompare(String(b.subj_name || '')));
        ...
        if (typeof window !== 'undefined') {
          const savedConstraintsStr = localStorage.getItem('auto_schedule_teacher_constraints_v3');
  ...
  const teacherWorkloads = useMemo(() => {
    ...
    rawTeacherAssignments.forEach(ts => {
      const section = sections.find(s => String(s.id) === String(ts.section_id));
      const key = section ? `${ts.subject_id}_${section.class_id}` : '';
      const quota = key && subjectQuotas[key] !== undefined ? Number(subjectQuotas[key]) : 0;
      
      if (quota > 0) {
        const tId = String(ts.teacher_id);
        const tName = ts.teachers?.users?.full_name || ts.teacher_name || 'معلم غير محدد';
        
        if (!loads[tId]) loads[tId] = { id: tId, name: tName, total: 0, details: [] };
        loads[tId].total += quota;
        const detailStr = `${ts.subjects?.name || 'مادة'} (${section?.class_name || 'صف'}): ${quota} حصص`;
        if (!loads[tId].details.includes(detailStr)) {
            loads[tId].details.push(detailStr);
        }
      }
    });
  ...
  const generateSchedule = async () => {
    ...
    const teacherAssignments = rawTeacherAssignments.map(ts => {
      ...
      const validSlots = allowedSlots.filter(s => parseInt(s.split('-')[1]) <= stageMaxDailyPeriods);
      const maxPossibleSlots = validSlots.length;
      ...
      return {
        ...
        teacher_name: ts.teachers?.users?.full_name || ts.teacher_name || 'معلم غير محدد',
        subject_name: ts.subjects?.name || 'مادة', class_name: section?.class_name, section_level: section?.level,
        stage: section?.stage || 'middle', 
    ...
    addLog(`📊 الميزانية: إجمالي الحصص المطلوبة رسمياً هو (${totalRequested}) حصة.`);

    let absoluteBestSchedule = [];
    let absoluteBestUnplaced = Array(1000).fill(null); 
    let absoluteBestFailedCount = 99999;
    
    const subjectFailureWeights: Record<string, number> = {};
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        ...
        const ignoreFairness = attempt > 30; 
        const relaxation = attempt > 20 ? 2 : 0; 
        const emergencyForce = attempt > 40; 

        const teacherMaxDailyLoad: Record<string, number> = {};
        Object.keys(teacherTotalQuotas).forEach(tId => {
           const assignment = teacherAssignments.find(ta => String(ta.teacher_id) === String(tId));
           const availableDaysCount = assignment ? assignment.available_days.length : 5;
           
           const hasVIP = teacherAssignments.some(ta => String(ta.teacher_id) === String(tId) && ta.isVIP);
           let finalDays = availableDaysCount;
           if (hasVIP && finalDays > 2) finalDays = 2;
           
           const total = teacherTotalQuotas[tId];
           let baseMax = Math.ceil(total / (finalDays || 1));
        ...
        const sortedAssignments = shuffleArray([...teacherAssignments]).sort((a, b) => {
          ...
          const failKeyA = `${a.teacher_id}_${a.section_id}_${a.subject_id}`;
          const failKeyB = `${b.teacher_id}_${b.section_id}_${b.subject_id}`;
          const failWeightA = subjectFailureWeights[failKeyA] || 0;
          const failWeightB = subjectFailureWeights[failKeyB] || 0;
          if (failWeightA !== failWeightB) return failWeightB - failWeightA;
        ...
        const totalAssignmentsLength = sortedAssignments.length;

        for (const assignment of sortedAssignments) {
          assignmentIndex++;
          
          if (assignmentIndex % 40 === 0) {
             setGenerationLogs(prev => [`⏳ محرك التعلم يبحث (محاولة ${attempt}/${MAX_ATTEMPTS})... تقدم: (${assignmentIndex}/${totalAssignmentsLength})`, ...prev.slice(0, 3)]);
             await new Promise(resolve => setTimeout(resolve, 0));
          }

          const section = sections.find(s => String(s.id) === String(assignment.section_id));
          if (!section) continue;

          const stageMaxPeriods = section.stage === 'middle' ? 5 : 6;
          const failKey = `${assignment.teacher_id}_${assignment.section_id}_${assignment.subject_id}`;
          ...
          for (let i = 0; i < remainingLessons; i++) {
            ...
            if (!isPlaced && !assignment.isVIP && attempt > 5) {
              for (const day of getBestDays()) {
                ...
                for (const period of dayPeriods) {
                  ...
                  const subjCountToday = finalSchedule.filter(s => String(s.section_id) === String(section.id) && String(s.day) === String(day) && String(s.subject_id) === String(assignment.subject_id)).length;
                  if (subjCountToday >= maxPerDay) continue; 
                  ...
                  if (secSlotIdx !== -1 && teachSlotIdx === -1) {
                    ...
                    if (!blockingSlot.isVIP) {
                      ...
                      if (!allowedSlotsZ || allowedSlotsZ.length === 0) {
                          allowedSlotsZ = [];
                      ...
                      for (const altDay of allowedDaysZ) {
                         ...
                         for (const altPeriod of altPeriods) {
                            ...
                            const zAssignment = teacherAssignments.find(ta => String(ta.teacher_id) === String(blockingSlot.teacher_id) && String(ta.subject_id) === String(blockingSlot.subject_id));
                            ...
                            if (safeSpreadForZ) {
                              finalSchedule[secSlotIdx].day = altDay;
                              finalSchedule[secSlotIdx].period_number = altPeriod.period_number;
                              finalSchedule[secSlotIdx].start_time = altPeriod.start_time;
                              finalSchedule[secSlotIdx].end_time = altPeriod.end_time;
                  ...
                  if (!swapped && teachSlotIdx !== -1 && secSlotIdx === -1 && attempt > 10) {
                     ...
                     if (!teacherBlockingSlot.isVIP) {
                         ...
                         if (!allowedSlotsW || allowedSlotsW.length === 0) {
                             allowedSlotsW = [];
                         ...
                         for (const altDay of allowedDaysW) {
                             ...
                             for (const altPeriod of altPeriods) {
                                 ...
                                 const wAssignment = teacherAssignments.find(ta => String(ta.teacher_id) === String(teacherBlockingSlot.teacher_id) && String(ta.subject_id) === String(teacherBlockingSlot.subject_id));
                                 ...
                                 finalSchedule[teachSlotIdx].day = altDay;
                                 finalSchedule[teachSlotIdx].period_number = altPeriod.period_number;
                                 finalSchedule[teachSlotIdx].start_time = altPeriod.start_time;
                                 finalSchedule[teachSlotIdx].end_time = altPeriod.end_time;
                                 
                                 teacherDailyLoad[teacherBlockingSlot.teacher_id][day]--;
                                 teacherDailyLoad[teacherBlockingSlot.teacher_id][altDay]++;

                                 commitPlacement(section, assignment, day, period);
                                 isPlaced = true; swapped = true; break;
                             }
                         }
                     }
                  }
            ...
            if (!isPlaced) {
              failedPlacements++;
              unplacedQueue.push({
                 id: safeGenerateId(), section_id: section.id, section_name: section.full_name,
                 subject_id: assignment.subject_id, subject_name: assignment.subject_name,
                 teacher_id: assignment.teacher_id, teacher_name: assignment.teacher_name,
        ...
        if (failedPlacements < absoluteBestFailedCount) {
           ...
           addLog(`💡 تطور ملحوظ: الذكاء الاصطناعي وجد مساراً أفضل (المتبقي: ${failedPlacements} حصص).`);
        }
        if (failedPlacements === 0) {
            addLog(`🎯 تم إيجاد الجدول المثالي والخالي من التعارضات في المحاولة ${attempt}!`);
            break;
        }
    } 
    ...
    absoluteBestSchedule.sort((a, b) => Number(a.day) - Number(b.day) || Number(a.period_number) - Number(b.period_number));
    ...
    addLog(`✅ إحصائية رياضية: تم تسكين ${absoluteBestSchedule.length} حصة من أصل ${totalRequested} مطلوبة.`);
    setGenerating(false);
  };
  ...
  const handleDrop = (e: any, targetDay: number, targetPeriod: number, targetSlot: any) => {
      e.preventDefault();
      ...
      if (draggedItem.type === 'grid' && draggedItem.slot) {
          const sourceSlot = draggedItem.slot;
          if (String(targetDay) === String(draggedItem.day) && String(targetPeriod) === String(draggedItem.period_number)) {
          ...
          const pDataSource = periods.find(p => p.stage === sourceSlot.stage && String(p.period_number) === String(draggedItem.period_number));
          ...
          if (targetSlot) {
              ...
              if (!allowedB.includes(`${draggedItem.day}-${draggedItem.period_number}`)) {
                  alert(`❌ لا يمكن التبديل: المعلم (${targetSlot.teacher_name}) غير متاح في يوم ${getDayName(draggedItem.day)} الحصة ${draggedItem.period_number}.`);
                  setDraggedItem(null); return;
              }
          ...
          if (targetSlot) {
              ...
              newSchedules[bIndex] = {
                  ...newSchedules[bIndex],
                  day: draggedItem.day,
                  period_number: draggedItem.period_number,
      ...
      else if (draggedItem.type === 'unplaced' && draggedItem.lesson) {
          const lesson = draggedItem.lesson;
          
          const section = sections.find(s => String(s.id) === String(lesson.section_id));
          const stageMaxPeriods = section?.stage === 'middle' ? 5 : 6;
          ...
          let allowedSlots = teacherConstraints[lesson.teacher_id];
          if (!allowedSlots || allowedSlots.length === 0) {
              allowedSlots = [];
              workingDays.forEach(d => dynamicPeriods.forEach(p => allowedSlots.push(`${d}-${p}`)));
          }
          if (!allowedSlots.includes(`${targetDay}-${targetPeriod}`)) {
              alert(`❌ المعلم غير متاح في هذا الوقت بناءً على مصفوفة قيوده!`);
              setDraggedItem(null); return;
          }

          const occupantIndex = generatedSchedules.findIndex(s => String(s.section_id) === String(lesson.section_id) && String(s.day) === String(targetDay) && String(s.period_number) === String(targetPeriod));
          let newSchedules = [...generatedSchedules];
          let newUnplaced = unplacedLessons.filter(l => String(l.id) !== String(lesson.id));

          if (occupantIndex !== -1) {
            const occupant = newSchedules[occupantIndex];
            const confirmSwap = confirm(`هذه الحصة مشغولة بمادة (${occupant.subject_name}). هل تريد إزاحتها للانتظار وتسكين مادتك مكانها؟`);
            if (!confirmSwap) { setDraggedItem(null); return; }
          ...
          newSchedules.push({
            id: safeGenerateId(),
            section_id: lesson.section_id, section_name: lesson.section_name,
            subject_id: lesson.subject_id, subject_name: lesson.subject_name,
            teacher_id: lesson.teacher_id, teacher_name: lesson.teacher_name,
            day: targetDay, period_number: targetPeriod,
            start_time: pDataTarget.start_time, end_time: pDataTarget.end_time, stage: lesson.stage,
    ...
    try {
      ...
      const formatted = rawSlots.map(slot => {
        ...
        return {
          ...
          teacher_name: String(tName),
          subject_name: String(sName),
          stage: slot.stage || section?.stage || 'middle',
          zoom_link: zLink
        };
      });

      formatted.sort((a, b) => Number(a.day) - Number(b.day) || Number(a.period_number) - Number(b.period_number));
        ...
        <div className="bg-gradient-to-r from-indigo-900 to-slate-900 rounded-[2rem] p-6 md:p-8 text-white shadow-xl flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden relative">
          ...
          <div className="relative z-10">
            ...
            <p className="text-slate-300 font-bold max-w-xl text-sm md:text-base">
              الآن يمكنك استدعاء الجداول السحابية السابقة، توليد جداول جديدة، التعديل بنظام السحب والإفلات التفاعلي، وطباعتها كلها.
            </p>
          </div>
        </div>
          ...
          <div className="xl:col-span-1 space-y-6">
              ...
              <div className="space-y-6 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar pb-4">
                ...
                {sortedClassNames.map(className => {
                  ...
                  <div key={className} className="space-y-3">
                    <h4 className="font-black text-sm text-indigo-900 bg-indigo-50/80 px-3 py-2.5 rounded-lg border border-indigo-100 flex items-center justify-between">
                      <div className="flex items-center gap-2"><Layers className="w-4 h-4 text-indigo-500" /> {className}</div>
                      <span className={`text-[10px] px-2 py-1 rounded-md ${totalQuotasForClass > MAX_WEEKLY ? 'bg-rose-500 text-white animate-pulse' : 'bg-white text-indigo-700 font-bold'}`} title={`السعة القصوى للأسبوع هي ${MAX_WEEKLY} حصة`}>
                         {totalQuotasForClass}/{MAX_WEEKLY}
                      </span>
                    </h4>
                    ...
                    <div key={data.id} className={`bg-white border p-3 rounded-xl flex flex-col md:flex-row md:items-center justify-between gap-3 transition-colors shadow-sm ${hasCustomConstraints ? 'border-amber-300 bg-amber-50/30' : 'border-slate-200'}`}>
                      <div className="flex items-center justify-between md:justify-start gap-3 w-full md:w-auto">
                        <div className="flex flex-col">
                          <span className="font-bold text-sm md:text-base text-slate-700">{data.name}</span>
                      ...
                      <button onClick={() => openTeacherConstraintsModal(data.id, data.name)} className={`w-full md:w-auto px-4 py-2.5 rounded-xl flex items-center justify-center gap-2 shrink-0 transition-all active:scale-95 shadow-sm border ${hasCustomConstraints ? 'bg-amber-100 text-amber-700 border-amber-200 hover:bg-amber-200' : 'bg-slate-100 text-slate-600 border-slate-200 hover:bg-indigo-100 hover:text-indigo-600 hover:border-indigo-200'}`}>
            ...
            <div className="bg-white p-5 md:p-6 rounded-3xl shadow-sm border border-slate-200">
              ...
              <div className="mt-6 bg-slate-900 rounded-2xl p-4 h-40 overflow-y-auto font-mono text-[10px] text-slate-300 shadow-inner flex flex-col-reverse custom-scrollbar">
                {generationLogs.length === 0 ? <span className="text-center opacity-50 m-auto">محرك الذكاء بانتظار الإطلاق...</span> : generationLogs.map((log, i) => <div key={i} className={`mb-1 border-b border-white/5 pb-1 ${log.includes('❌') || log.includes('⚠️') ? 'text-rose-400' : log.includes('✅') || log.includes('🎉') || log.includes('🔄') || log.includes('🧹') || log.includes('💡') || log.includes('🎯') || log.includes('📊') ? 'text-emerald-400' : ''}`}>{'>'} {log}</div>)}
              </div>
            </div>
                            ...
                            <tbody>
                              {workingDays.map((day) => (
                                <tr key={day} className="border-b border-slate-300 last:border-b-0 break-inside-avoid">
                                  <td className="p-4 font-black text-slate-900 bg-slate-100 border-l border-slate-300 pointer-events-none">{getDayName(day)}</td>
                                  {dynamicPeriods.map(p => {
                                    const slot = generatedSchedules.find(s => String(s.day) === String(day) && String(s.period_number) === String(p) && (gridFilterType === 'section' ? String(s.section_id) === String(gridFilterId) : String(s.teacher_id) === String(gridFilterId)));
